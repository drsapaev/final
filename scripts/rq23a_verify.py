"""Self-contained RQ-23.a effective-settings report verification runner (lessons 48/49).

One process provisions a FRESH disposable PostgreSQL (pgserver, new
pgdata dir per run), proxies its unix socket onto a free 127.0.0.1 TCP
port, and runs pytest in-process. Sets RQ23A_PG_ADMIN_URL so
test_queue_profile_lifecycle.py reuses its scratch-provisioning path.
"""
...
def main() -> int:
    argv = sys.argv[1:]
    if not argv:
        print("usage: rq23a_verify.py <pytest args...>", file=sys.stderr)
        return 2

    pgdata = tempfile.mkdtemp(prefix="rq23a_pgdata_")
    ...
    try:
        ...
        proxy.shutdown()
        server.cleanup()  # pgserver API: graceful stop of this instance
