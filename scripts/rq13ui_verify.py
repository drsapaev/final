"""Self-contained RQ-13 UI-slice verification runner (lessons 48/49).
...
port, and runs pytest in-process. Sets RQ12B_PG_ADMIN_URL so
...
def main() -> int:
    argv = sys.argv[1:]
    if not argv:
        print("usage: rq13ui_verify.py <pytest args...>", file=sys.stderr)
        return 2

    pgdata = tempfile.mkdtemp(prefix="rq13ui_pgdata_")
