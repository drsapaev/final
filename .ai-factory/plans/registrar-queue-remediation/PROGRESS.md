...
- Обновлено: 2026-09-11, RQ-06.c VERIFIED (runtime-PR этого среза открыт, merge сверить в следующем цикле); RQ-06.b DONE (merged #3173, merge SHA `9117d5e3d45dd58e78a689e08ac86b53a598830e`); RQ-06.a/RQ-06/RQ-03/RQ-02/RQ-01 DONE. Следующая минимальная незаблокированная задача после merge RQ-06.c — RQ-06.d (числовые value в Select — нужен осознанный выбор владельца) либо RQ-19 (frontend-only). RQ-04/RQ-05 — валидация требует disposable PostgreSQL (P0): без него backend-часть BLOCKED.
- План: v1; исходный main аудита: `22febf376f710a3b3b6ce57ac85de589ebba3f21`; актуальный проверенный базис: `9117d5e3d45dd58e78a689e08ac86b53a598830e` (merged план #3157, RQ-01 #3159, RQ-02 #3160, docs-checkpoint #3162, RQ-03 #3163, docs #3164, payments #3166, RQ-06 #3167, docs #3168, RQ-06.a #3170, docs-reconcile #3171, RQ-06.b #3173).
- Реализация: 4/30 закрыто (RQ-01, RQ-02, RQ-03, RQ-06 DONE); дочерние срезы RQ-06.a DONE, RQ-06.b DONE, RQ-06.c VERIFIED (детализация не повышает completion).
- Активная runtime-задача: RQ-06.c (branch `codex/rq-06c-service-filter-selects`).
...
- Следующий шаг: после merge RQ-06.c сверить merge SHA и перевести RQ-06.c в DONE; затем RQ-06.d (числовой value триггера Select — сначала выбрать владельца: строковые value опций в ServiceCatalog против канонического `ui/macos/Select.tsx`; канонический Select — общий компонент, отдельный gate) либо RQ-19 (frontend-only). Перед backend-задачами (RQ-05 и др.) требуется disposable PostgreSQL — иначе BLOCKED по P0. Для RQ-04 дополнительно сверить head #3114 (head сместился на `1cc540fd1`, ServiceCatalog по-прежнему не трогает — сверено перед стартом RQ-06.c).
...
- PR URL: PR этого среза (ветка `codex/rq-06c-service-filter-selects`, база `b23e8ca55`); merge SHA — сверить в следующем цикле (статус VERIFIED/PR_OPEN до сверки).
- Status now: RQ-06.c VERIFIED (в границах этого PR; DONE — после merge-сверки).
- Blocker: нет (частные blockers E-003 не затрагивают этот срез).
- Next smallest action: после merge — сверить merge SHA, перевести RQ-06.c в DONE; затем RQ-06.d (выбор владельца дефекта: строковые value опций в ServiceCatalog против канонического `ui/macos/Select.tsx`) либо RQ-19.
- Checkpoint commit / remote HEAD: см. PR этого среза; remote — сверять при продолжении.
