...
- Обновлено: 2026-09-12, RQ-21.a DONE (merged #3188, merge SHA `ac27ac015eac1a95004bf241398f8ea28544aa40`: результат поиска/статус-фильтра, суженный до нуля строк, больше не показывается как «Очередь пуста» (F-17 закрыт на уровне UI) — новый pure helper `resolveRegistrarWorklistEmptyScopeKind` в SSOT-модуле `registrarWorklistRows.ts` возвращает факт области ('queue-empty' | 'filtered-empty') на ТЕХ же правилах разрешения тегов, что и строки (общие internals, дублирование предиката поиска исключено); WorklistView показывает отдельное состояние «Нет совпадений» (новый i18n-ключ `rp_worklist_no_matches` ×5 локалей + существующий `rp_empty_filter_desc`); подлинно пустая область и отсутствующий prop сохраняют QW-04-состояние пустой очереди (S-18 на rows-unit+MOCK уровне); до этого RQ-22/RQ-20/RQ-19/RQ-06.c/RQ-06.b/RQ-06.a/RQ-06/RQ-03/RQ-02/RQ-01 DONE). Родитель RQ-21 остается TODO: RQ-21.b (согласованные счетчики) ждет D-07. Следующие минимальные незаблокированные: RQ-11 (честный показ срока текущего QR; direct_execute для текста/отображения; deps RQ-01 закрыт) либо RQ-26.a (CSV round-trip текущего контракта; deps RQ-01), RQ-24.a/RQ-27.a (verification-срезы). RQ-06.d — нужен осознанный выбор владельца (канонический `ui/macos/Select.tsx` через отдельный gate). RQ-04/RQ-05 — валидация требует disposable PostgreSQL (P0): без него backend-часть BLOCKED.
...
- План: v1; исходный main аудита: `22febf376f710a3b3b6ce57ac85de589ebba3f21`; актуальный проверенный базис: `ac27ac015eac1a95004bf241398f8ea28544aa40` (merged план #3157, RQ-01 #3159, RQ-02 #3160, docs-checkpoint #3162, RQ-03 #3163, docs #3164, payments #3166, RQ-06 #3167, docs #3168, RQ-06.a #3170, docs-reconcile #3171, RQ-06.b #3173, docs-reconcile #3174, RQ-06.c #3175, docs-reconcile #3176, RQ-19 #3177, docs-reconcile #3178, QD-2C #3114, RQ-20.a #3179, docs-reconcile #3180, RQ-20.b #3181, docs-reconcile #3183, RQ-22 #3184, docs-reconcile #3186, RQ-21.a #3188).
- Реализация: 7/30 закрыто (RQ-01, RQ-02, RQ-03, RQ-06, RQ-19, RQ-20, RQ-22 DONE); дочерние срезы RQ-06.a DONE, RQ-06.b DONE, RQ-06.c DONE, RQ-20.a DONE, RQ-20.b DONE, RQ-21.a DONE (детализация не повышает completion; родитель RQ-21 ждет RQ-21.b — D-07).
...
- Следующий шаг: минимальные незаблокированные — RQ-11 (честный показ срока текущего QR; direct_execute для текста/отображения; first-touch `frontend/src/components/queue/ModernQueueManager.tsx` + `__tests__/QueueManager.contract.test.tsx` + i18n-ключи по точному locate; TTL/защиты не менять) либо RQ-26.a (CSV round-trip текущего контракта; first-touch `QueueProfilesManager.tsx` + соседний import/export-тест), либо RQ-24.a/RQ-27.a (verification-срезы, deps RQ-01). RQ-21.b ждет D-07 (не подменять согласованную семантику счетчиков самовольным решением). Перед backend-задачами (RQ-05 и др.) требуется disposable PostgreSQL — иначе BLOCKED по P0. Задачи с пометкой «проверить #3114» (RQ-04/08/09/13/14/15/16/18/24) сверяют якоря с merged QD-2C (`4e3fe78c`) — head-сверка устарела.
