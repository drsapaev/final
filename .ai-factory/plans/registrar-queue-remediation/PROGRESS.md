...
- Обновлено: 2026-09-11, RQ-22 DONE (merged #3184, merge SHA `d9337781d1ec7143a8b03f7243cea3b8169804df`: сбой refresh сохраняет ранее загруженные строки с явным флагом `stale` и баннером+повтором (F-18 закрыт), первичная ошибка отображается как ошибка, race-гвардия request-sequence игнорирует поздние ответы superseded-запросов (S-19 на Vitest-уровне); до этого RQ-20/RQ-19/RQ-06.c/RQ-06.b/RQ-06.a/RQ-06/RQ-03/RQ-02/RQ-01 DONE). Повторный клик по активной вкладке (toggle-off) сознательно НЕ менялся — план требует предварительного подтверждения требуемого поведения владельцем (характеризация в E-011). Следующие минимальные незаблокированные: RQ-21.a (отличить «Нет совпадений» от пустой очереди; RQ-01+RQ-02 закрыты; direct_execute-часть) либо RQ-06.d (нужен осознанный выбор владельца: строковые value опций в ServiceCatalog против канонического `ui/macos/Select.tsx` через отдельный gate). RQ-04/RQ-05 — валидация требует disposable PostgreSQL (P0): без него backend-часть BLOCKED.
...
- План: v1; исходный main аудита: `22febf376f710a3b3b6ce57ac85de589ebba3f21`; актуальный проверенный базис: `d9337781d1ec7143a8b03f7243cea3b8169804df` (merged план #3157, RQ-01 #3159, RQ-02 #3160, docs-checkpoint #3162, RQ-03 #3163, docs #3164, payments #3166, RQ-06 #3167, docs #3168, RQ-06.a #3170, docs-reconcile #3171, RQ-06.b #3173, docs-reconcile #3174, RQ-06.c #3175, docs-reconcile #3176, RQ-19 #3177, docs-reconcile #3178, QD-2C #3114, RQ-20.a #3179, docs-reconcile #3180, RQ-20.b #3181, docs-reconcile #3183, RQ-22 #3184).
- Реализация: 7/30 закрыто (RQ-01, RQ-02, RQ-03, RQ-06, RQ-19, RQ-20, RQ-22 DONE); дочерние срезы RQ-06.a DONE, RQ-06.b DONE, RQ-06.c DONE, RQ-20.a DONE, RQ-20.b DONE (детализация не повышает completion).
...
- Следующий шаг: минимальные незаблокированные — RQ-21.a (отличить «Нет совпадений» от пустой очереди; first-touch `registrarWorklistRows.ts` + `views/WorklistView.tsx` + их тесты; исправление ложного пустого состояния не ждет новой семантики счетчиков) либо RQ-06.d (выбор владельца дефекта: строковые value опций в ServiceCatalog против канонического `ui/macos/Select.tsx`; канонический Select — общий компонент, отдельный gate). Перед backend-задачами (RQ-05 и др.) требуется disposable PostgreSQL — иначе BLOCKED по P0. Задачи с пометкой «проверить #3114» (RQ-04/08/09/13/14/15/16/18/24) сверяют якоря с merged QD-2C (`4e3fe78c`) — head-сверка устарела.
