...
- Обновлено: 2026-09-11, срез RQ-20.a PR_OPEN (см. E-011; до этого RQ-19 DONE merged #3177, merge SHA `ab24bdba9de7600bab1e1a120af0a546f5b3039f`; RQ-06.c/RQ-06.b/RQ-06.a/RQ-06/RQ-03/RQ-02/RQ-01 DONE). RQ-20 разделен на обязательных детей: RQ-20.a (заголовок + URL-семантика вкладок — этот срез) и RQ-20.b (явный сброс активных фильтров на worklist — TODO; WorklistView reference-only в срезе RQ-20, требует собственного gate). Повторный клик по активной вкладке (toggle-off) сознательно НЕ менялся — план требует предварительного подтверждения требуемого поведения владельцем (характеризация в E-011). Следующие минимальные незаблокированные: подтверждение merge RQ-20.a → docs-reconcile, затем RQ-20.b либо RQ-06.d (нужен осознанный выбор владельца: строковые value опций в ServiceCatalog против канонического `ui/macos/Select.tsx` через отдельный gate). RQ-04/RQ-05 — валидация требует disposable PostgreSQL (P0): без него backend-часть BLOCKED.
- План: v1; исходный main аудита: `22febf376f710a3b3b6ce57ac85de589ebba3f21`; актуальный проверенный базис: `ab24bdba9de7600bab1e1a120af0a546f5b3039f` (merged план #3157, RQ-01 #3159, RQ-02 #3160, docs-checkpoint #3162, RQ-03 #3163, docs #3164, payments #3166, RQ-06 #3167, docs #3168, RQ-06.a #3170, docs-reconcile #3171, RQ-06.b #3173, docs-reconcile #3174, RQ-06.c #3175, docs-reconcile #3176, RQ-19 #3177).
- Реализация: 5/30 закрыто (RQ-01, RQ-02, RQ-03, RQ-06, RQ-19 DONE); дочерние срезы RQ-06.a DONE, RQ-06.b DONE, RQ-06.c DONE (детализация не повышает completion).
...
- Следующий шаг: минимальная незаблокированная — RQ-20 (название/URL/навигация; D-07 только при смене маршрутов; first-touch `useRegistrarNavigation.ts` и соседние — сверить с #3114 head и wizard-PR при пересечениях) либо RQ-06.d (выбор владельца дефекта: строковые value опций в ServiceCatalog против канонического `ui/macos/Select.tsx`; канонический Select — общий компонент, отдельный gate). Перед backend-задачами (RQ-05 и др.) требуется disposable PostgreSQL — иначе BLOCKED по P0. Для RQ-04 дополнительно сверить head #3114 (head `be2b28934` на момент E-010 — сверять заново перед стартом).
...
| RQ-20 | Название и навигация | RQ-19; D-07 при смене маршрутов | PR_OPEN (срез RQ-20.a VERIFIED в границах PR; parent ждет RQ-20.b) | codex/rq-20-title-url-navigation / срез RQ-20.a / E-011 | E-011 |
...
| RQ-20.a | Произвольный профиль показывает свое название (helper `resolveRegistrarTabLabel`: SSOT-лейбл Tabs → backend title → легаси-ключ → raw key; заголовок worklist + breadcrumb) + URL-семантика вкладок: setActiveTab строит query из router searchParams (не window.location.search — ?q/?status больше не стираются), push вместо replace (back/forward обходят вкладки), синхронизация activeTab при внешнем изменении ?dept= | RQ-19 | PR_OPEN | codex/rq-20-title-url-navigation / E-011 |
| RQ-20.b | Явный сброс активных фильтров на worklist: бейдж status-фильтра видим, но не сбрасываем на самом worklist (только через Welcome-карточку или правку URL) — добавить явный control сброса + соседний тест | RQ-20.a | TODO | — (первое касание кандидат: `frontend/src/pages/registrar/views/WorklistView.tsx` — reference-only в родительском срезе, граница фиксируется этим ребенком; собственный gate перед правкой) |
