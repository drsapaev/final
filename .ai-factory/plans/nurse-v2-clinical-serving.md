...
- 2026-09-21: **N2-5 owner review round — frontend: handover action + стейт-машина P1×2+P2×3**. useNurseServingBoard: (P1-2) boardResourceIdRef — переключение станции A→B очищает board НЕМЕДЛИНО (не под pending/упавшим B), тот же ключ в NurseTabletPage = render-гейт (board рендерится только при совпадении queue_resource_id с selectedWorkplaceId); (P1-3) clearAllPhi — единое синхронное revocation-действие (bump board+draining эпох, board/draining/selection/notice в один patch) ДО любого второго запроса; resetWorkplace чистит до await loadWorkplaces; foreground board 403/404 чистит и draining (+bump эпох); draining 401/403→clear, 404→удалить items, network/5xx→stale+видимый warning; PHI-диалог (reason form) закрывается при потере stationBoard; (P2-1) catch silent poll board — та же epoch+selection проверка, что success; (P2-2) loadWorkplaces возвращает различимый NurseWorkplacesResult ({ok,items} | {ok:false,status}): network/5xx не трогают список и НЕ удаляют workflow (видимая ошибка + Retry; «Нет назначенного рабочего места» не показывается под ошибкой), 401/403→access-loss через clearAllPhi; (P2-3) drainingEpochRef — терминальные мутации и новые чтения суперсидят поздние draining-ответы. NurseStationBoard: takeover-карточка (banner «Сотрудник, вызвавший пациента, недоступен», кнопка «Принять пациента» вместо «Начать приём» для called, services для in_progress) + в others-блоке actionable-строки получают «Принять пациента»/«Продолжить обслуживание» (не-actionable чужие остаются read-only). current = my_entry ?? первый actionable active (только серверный предикат). i18n +6 ключей ×5 локалей (handover_banner, action_takeover, action_takeover_continue, others_handover, draining_stale, workplaces_unavailable; uz-Latn апострофы по конвенции ’/‘). Тесты: компонентные +16 (handover called/in_progress/others-block/read-only-владелец; switch pending/network-error PHI-clear; sync revocation с ВЕЧНО висящим listWorkplaces — board+draining исчезают ДО резолва; draining 403/404/network-warning; stale silent 403 discarded; workplaces network — workflow жив/заявка «нет назначений» исчезает; late draining не воскрешает карточку) + e2e business handover-сценарий. Гейты: tsc 0, vitest 2405/2405 (269 файлов), lint 0 errors, stylelint OK, build ✓, check-theme OK, icon-controls 0 новых.
