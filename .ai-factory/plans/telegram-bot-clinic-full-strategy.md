...
- [ ] Status: partially implemented. Protected Mini App identity validation, scoped aggregate manifest, appointment preview/create backend slices, protected section entry buttons, and read-only manifest shell panels exist; richer forms, cabinet records, payment details, result viewing/download, and remaining full-flow validation are still pending. Legacy `WebAppInfo` links do not count as completion without protected Mini App identity validation and scoped runtime flows.
...
- [ ] Implement appointment booking inside the protected Mini App flow.
  ...
  - [x] Frontend create action slice: `frontend/src/App.jsx` shows the protected create action only after a successful Mini App preview, reuses `POST /api/v1/telegram/mini-app/appointments`, clears stale preview/create state when the draft changes, and keeps appointment ids, payment provider payloads, and unrelated Mini App section endpoints out of the UI flow.
  - [x] Frontend create guardrail test slice: `frontend/src/__tests__/telegramMiniAppAppointmentCreateGuardrails.test.js` asserts the create action stays behind successful preview plus `create_enabled`, uses the existing appointment request body/endpoint, clears stale create state on draft or preview changes, and avoids provider/payment/webhook payload expansion.
  - [x] Expired entry-token UX slice: `frontend/src/App.jsx` maps Mini App `entry_token_invalid`/`entry_token_expired` manifest failures to patient-safe expired-link copy, suppresses handled raw API toasts for Mini App manifest/preview failures, exposes error alerts as assertive live regions, and lets the session badge wrap on narrow viewports.
  - [x] Expired entry-token guardrail test slice: `frontend/src/__tests__/telegramMiniAppExpiredLinkGuardrails.test.js` keeps expired-link copy, handled Mini App API toast suppression, assertive error alerts, error badge state, and mobile badge wrapping anchored to the Mini App shell source.
...
- [ ] Implement patient forms inside the protected Mini App flow.
  ...
  - [x] Frontend manifest shell slice: `frontend/src/App.jsx` opens the protected forms section from `?section=forms`, fetches `/telegram/mini-app/forms/manifest` only after a trusted Mini App aggregate manifest is ready, and renders capture/submission status without patient-entered form fields or mutation inputs.
- [ ] Implement patient cabinet inside the protected Mini App flow.
  ...
  - [x] Frontend manifest shell slice: `frontend/src/App.jsx` opens the protected cabinet section from `?section=cabinet`, fetches `/telegram/mini-app/cabinet/manifest` only for that section, and renders read/mutation and sensitive-data safety badges without cabinet records, passport data, billing records, or edit controls.
- [ ] Implement payment details inside the protected Mini App flow.
  ...
  - [x] Frontend manifest shell slice: `frontend/src/App.jsx` opens the protected payments section from `?section=payments`, fetches `/telegram/mini-app/payments/manifest` only for that section, and renders read/capture/provider safety badges without payment records, billing totals, debt amounts, provider payloads, or payment controls.
- [ ] Implement protected result/report viewing inside the Mini App flow.
  ...
  - [x] Frontend manifest shell slice: `frontend/src/App.jsx` opens the protected results section from `?section=results`, fetches `/telegram/mini-app/results/manifest` only for that section, and renders view/download/report safety badges without medical results, lab values, report records, file URLs, PDFs, diagnoses, or download controls.
  - [x] Frontend read-only manifest guardrail test slice: `frontend/src/__tests__/telegramMiniAppReadOnlyManifestGuardrails.test.js` keeps forms/cabinet/payments/results manifest fetches gated by trusted aggregate readiness plus selected sections, verifies manifest-only capability gates, and blocks mutating controls or raw sensitive payload access in status panels.
...
- [x] Frontend deep-link guardrail test slice: `frontend/src/__tests__/telegramMiniAppDeepLinkGuardrails.test.js` keeps canonical Mini App section aliases, `doctors -> appointments` and `documents -> results` mappings, unknown-section fallback, selected-section panel gates, and legacy `/patient?tab=` drift protected.
