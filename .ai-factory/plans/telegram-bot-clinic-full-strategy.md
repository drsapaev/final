...
- [x] Status: complete for non-medical staff bot state-changing actions. Staff bot read-only operations, pre-mutation confirmation requests, hash-only idempotency binding for replay protection, confirmed/completed/failed audit writers, queue/visit/payment/schedule protected domain adapters, queue adapter readiness metadata, queue `/call` + `/skip` domain adapter methods, `/cancel_visit` + `/move_visit` visit-record + queue-link adapter coverage, and protected target-bound execution for queue/visit/payment/schedule actions are implemented. Telegram callback/message execution remains disabled; `/publish_document` stays deferred to a medical-document approval slice.
...
- [x] Add domain service adapters for confirmed queue actions: call patient, skip patient, cancel/move visit.
  ...
  - [x] Full protected visit-record mutation adapter exists: `backend/app/services/telegram_staff_action_adapter_service.py::TelegramStaffActionAdapterService.staff_cancel_visit(...)` and `staff_move_visit(...)` mutate the `Visit` record and the queue-link status together while preserving `queue_time`; focused coverage is in `backend/tests/unit/test_telegram_staff_action_adapter_service.py`.
- [x] Add domain service adapters for confirmed payment actions: status change, refund where policy allows it. `TelegramStaffActionAdapterService.staff_change_payment_status(...)` delegates status transitions to `BillingService.update_payment_status(...)`; `staff_refund_payment(...)` enforces refundable status and available balance before mutating refund fields, with targeted coverage in `backend/tests/unit/test_telegram_staff_action_adapter_service.py`.
- [x] Add domain service adapters for confirmed schedule actions. `TelegramStaffActionAdapterService.staff_change_doctor_schedule(...)` updates protected schedule fields with time-range validation and targeted coverage in `backend/tests/unit/test_telegram_staff_action_adapter_service.py`.
- [x] Add remaining audit events for confirmed, failed, and completed state-changing actions; `TelegramStaffActionAdapterService` now writes `staff_action_confirmed`, `staff_action_completed`, and `staff_action_failed`, `backend/app/api/v1/endpoints/admin_telegram.py` publishes those events as runtime-ready, and `backend/tests/unit/test_telegram_staff_action_adapter_service.py` verifies the audit writers while keeping Telegram execution disabled.
- [x] Enable state-changing actions one by one behind explicit configuration and tests.
  - [x] `/call` is enabled behind the explicit protected-app confirmation contract, not Telegram callback execution: `POST /api/v1/admin/telegram/staff-actions/{confirmation_id}/confirm` consumes the hash-only confirmation once, rechecks the staff role, and executes `TelegramStaffActionAdapterService.staff_call_next_patient(...)`; targeted coverage is in `backend/tests/unit/test_telegram_staff_read_only_menu_runtime.py::TestTelegramStaffReadOnlyMenuRuntime::test_staff_call_confirmation_executes_once_in_protected_app`.
  - [x] `/skip` is enabled only with protected `entry_id` target binding; missing target is rejected before token consumption in `backend/tests/unit/test_telegram_staff_read_only_menu_runtime.py::TestTelegramStaffReadOnlyMenuRuntime::test_staff_skip_confirmation_requires_target_binding`, and successful execution is covered in `backend/tests/unit/test_telegram_staff_action_confirm_endpoint.py::test_confirm_skip_executes_only_with_bound_queue_entry`.
  - [x] `/cancel_visit` and `/move_visit` are enabled only with protected visit target payloads; `backend/tests/unit/test_telegram_staff_action_confirm_endpoint.py::test_confirm_cancel_and_move_visit_use_bound_visit_payloads` covers visit id/new date binding.
  - [x] Payment and schedule actions are enabled only with protected target payloads; `backend/tests/unit/test_telegram_staff_action_confirm_endpoint.py::test_confirm_payment_status_and_refund_use_bound_payment_payloads` and `backend/tests/unit/test_telegram_staff_action_confirm_endpoint.py::test_confirm_schedule_change_uses_bound_schedule_payload` cover payment/refund/schedule fields.
- [x] Add negative tests for unauthorized staff, stale confirmations, repeated confirmations, and cross-role action attempts.
  ...
  - [x] Protected runtime stale/repeated confirmation execution is covered for the shared execution path: `test_staff_call_confirmation_executes_once_in_protected_app` verifies single-use mutation/audit and repeat rejection; `test_staff_call_confirmation_rejects_expired_token_without_mutation` verifies expired confirmations do not mutate queue state; `test_staff_skip_confirmation_requires_target_binding` verifies missing target payload is not consumed or executed; `backend/tests/unit/test_telegram_staff_action_confirm_endpoint.py` covers the enabled target-bound queue/visit/payment/schedule commands through the same consume-before-adapter endpoint.
...
- [x] Implement appointment booking inside the protected Mini App flow.
  ...
  - [x] Frontend protected booking UI is wired: `frontend/src/pages/PatientPanel.jsx` detects `/patient/bookings`, requires Telegram Mini App `initData`, captures date/time/department/services/comment, calls `/api/v1/telegram/mini-app/appointments/preview` for review, and calls `/api/v1/telegram/mini-app/appointments` to create the appointment request.
  - [x] Frontend Mini App shell handles expired `entryToken` failures with patient-safe copy, suppresses handled raw API toasts, exposes error alerts as assertive live regions, and keeps the session badge responsive in `frontend/src/App.jsx`.
...
- [x] Add an aggregate backend-only patient Mini App manifest slice: `POST /api/v1/telegram/mini-app/patient/manifest` in `backend/app/api/v1/endpoints/telegram_webhook.py` validates Mini App `initData`, resolves linked patient scope, and returns appointment/forms/cabinet/payments/results capability status without exposing forms, cabinet records, payment records, provider payloads, report IDs, file URLs, PDFs, diagnoses, or medical results, with forged/staff/unlinked/missing-token coverage in `backend/tests/unit/test_telegram_webhook_security.py`.
- [x] Surface aggregate Mini App manifest readiness to admins: `backend/app/api/v1/endpoints/admin_telegram.py` publishes a `patient_mini_app_manifest` integration-status feature contract for `POST /api/v1/telegram/mini-app/patient/manifest`, and `frontend/src/components/TelegramManager.jsx` shows the safe endpoint readiness without enabling runtime bot actions or exposing records.
...
- [x] Add a script or CI job that prints all unchecked Telegram product roadmap items for release review: `scripts/report_unchecked_telegram_plan_items.py` skips process-only checklist sections and prints each product unchecked checkbox with section path and line number via `python scripts/report_unchecked_telegram_plan_items.py`.
