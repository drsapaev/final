...
- [ ] Status: partially closed. Staff bot read-only operations, pre-mutation confirmation requests, hash-only idempotency binding for replay protection, confirmed/completed/failed audit writers, queue/visit/payment/schedule protected domain adapters, queue adapter readiness metadata, queue `/call` + `/skip` domain adapter methods, and `/cancel_visit` + `/move_visit` visit-record + queue-link adapter coverage are implemented. Protected execution is enabled only for `/call`; remaining confirmed state-changing actions stay disabled until explicit target binding, action enablement, and runtime stale/replay execution tests are complete.
...
- [ ] Enable state-changing actions one by one behind explicit configuration and tests.
  - [x] `/call` is enabled behind the explicit protected-app confirmation contract, not Telegram callback execution: `POST /api/v1/admin/telegram/staff-actions/{confirmation_id}/confirm` consumes the hash-only confirmation once, rechecks the staff role, executes `TelegramStaffActionAdapterService.staff_call_next_patient(...)`, and keeps `/skip`, `/cancel_visit`, `/move_visit`, `/payment_status`, `/refund`, `/publish_document`, and `/change_schedule` disabled until target binding is explicit; targeted coverage is in `backend/tests/unit/test_telegram_staff_read_only_menu_runtime.py::TestTelegramStaffReadOnlyMenuRuntime::test_staff_call_confirmation_executes_once_in_protected_app`.
  - [ ] Enable `/skip` only after protected target binding identifies the queue entry/current called patient without trusting raw Telegram text.
  - [ ] Enable `/cancel_visit` and `/move_visit` only after protected target binding captures visit id/new date in the confirmation payload.
  - [ ] Enable payment and schedule actions only after protected target binding captures payment/refund/schedule fields and keeps idempotency tied to that payload.
- [ ] Add negative tests for unauthorized staff, stale confirmations, repeated confirmations, and cross-role action attempts.
  ...
  - [x] Protected runtime stale/repeated confirmation execution is covered for enabled `/call`: `test_staff_call_confirmation_executes_once_in_protected_app` verifies single-use mutation/audit and repeat rejection; `test_staff_call_confirmation_rejects_expired_token_without_mutation` verifies expired confirmations do not mutate queue state; `test_staff_skip_confirmation_stays_disabled_until_target_binding` verifies a disabled state-changing command is not consumed or executed.
  - [ ] Add equivalent runtime stale/repeated confirmation execution tests for each newly enabled target-bound action.
