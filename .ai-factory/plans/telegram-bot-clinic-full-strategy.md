...
- [ ] Implement appointment booking inside the protected Mini App flow.
  ...
  - [ ] Connect `/book` entry to a dedicated protected booking UI route instead of reusing `/patient/payments`; `PATIENT_BOOKING_ENTRY_ROUTE` in `backend/app/api/v1/endpoints/admin_telegram.py` is still temporarily aligned with payment route until a dedicated booking screen is available.
