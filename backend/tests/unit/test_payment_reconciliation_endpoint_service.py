...
@pytest.mark.unit
class TestPaymentReconciliationEndpointService:
    def test_get_reconciliation_report_applies_default_dates_and_alerts(self):
        ...
        with patch(
            "app.services.payment_reconciliation_endpoint_service.PaymentReconciliationService",
            return_value=reconciliation_service,
        ):
            service = PaymentReconciliationEndpointService(db=Mock())
    ...
    def test_reconcile_provider_raises_domain_error_on_failure(self):
        ...
        with patch(
            ...
            service = PaymentReconciliationEndpointService(db=Mock())
            with pytest.raises(PaymentReconciliationDomainError) as exc_info:
