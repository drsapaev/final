...
@pytest.mark.unit
class TestForceMajeureEndpointService:
    def test_cancel_queue_with_refund_rejects_invalid_type(self, monkeypatch):
        monkeypatch.setattr(
            "app.services.force_majeure_endpoint_service.get_force_majeure_service",
            lambda db: SimpleNamespace(),
        )

        repository = SimpleNamespace(db=None)
        service = ForceMajeureEndpointService(db=None, repository=repository)

        with pytest.raises(ForceMajeureDomainError) as exc_info:
