...
class TestNestedStringValueScrubbing:
    ...
    def test_percent_diagnostics_without_credentials_not_mangled(self):
        # Round 14 (codex P2): the percent-decoding form path is gated on a
        # credential-shaped field — ordinary diagnostics must pass through
        # byte-for-byte, without trailing "=" or URL decoding side effects.
        raw = "Database migration is 90% complete"
        result = mask_pii({"data": raw})
        assert result["data"] == raw

    def test_python_repr_single_quoted_value_is_redacted(self):
        # Round 13 (codex P1): Sentry may capture a Pydantic frame local as
        # a Python repr — the quoted value contains SPACES and the opposite
        # quote style, so the form regex would stop at the first space and
        # leave the endpoint/key material in the event.
        raw = (
            "PushDeviceRegisterRequest(token='{\"endpoint\": "
            "\"https://push.example/send\", \"keys\": "
            "{\"p256dh\": \"secret-material\"}}', platform='web')"
        )
        result = mask_pii({"data": raw})
        assert "secret-material" not in result["data"]
        assert "push.example/send" not in result["data"]
        assert "token=" in result["data"]
        assert "[REDACTED]" in result["data"]

    def test_python_repr_double_quoted_value_is_redacted(self):
        raw = 'push_device(token="pr6-secret-credential")'
        result = mask_pii({"data": raw})
        assert "pr6-secret-credential" not in result["data"]
        assert "token=" in result["data"]
        assert "[REDACTED]" in result["data"]
