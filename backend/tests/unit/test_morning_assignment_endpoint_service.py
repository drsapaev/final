...
@pytest.mark.unit
class TestMorningAssignmentEndpointService:
    def test_parse_target_date_handles_none_and_iso(self):
        assert MorningAssignmentEndpointService.parse_target_date(None) == date.today()
        assert MorningAssignmentEndpointService.parse_target_date("2026-02-14") == date(
            2026,
            2,
            14,
        )
    ...
    def test_parse_target_date_raises_for_invalid_date(self):
        with pytest.raises(ValueError):
            MorningAssignmentEndpointService.parse_target_date("bad-date")
    ...
    def test_manual_assignment_for_visits_returns_not_found_result(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.services.morning_assignment_endpoint_service.MorningAssignmentService",
            FakeMorningService,
        )
        ...
        service = MorningAssignmentEndpointService(db=None, repository=repository)
