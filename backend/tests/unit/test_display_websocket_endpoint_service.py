...
@pytest.mark.unit
class TestDisplayWebSocketEndpointService:
    @pytest.mark.asyncio
    async def test_call_patient_raises_when_entry_missing(self):
        ...
        service = DisplayWebSocketEndpointService(db=None, repository=repository)

        with pytest.raises(DisplayWebSocketDomainError) as exc_info:
