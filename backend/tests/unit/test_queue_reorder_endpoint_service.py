...
@pytest.mark.unit
class TestQueueReorderEndpointService:
    def test_reorder_queue_raises_when_queue_missing(self):
        class Repository:
            def get_queue(self, queue_id):
                return None

        service = QueueReorderEndpointService(db=None, repository=Repository())

        with pytest.raises(QueueReorderDomainError) as exc_info:
