"""Review #3274/#3269: real profile/department routes on isolated fixtures."""
...
def _profile(db, *, key=None, tags=None, visible=True, department=None):
    ...
    row = QueueProfile(
        key=key,
        title="Synthetic review profile",
        queue_tags=tags or [key],
        department_key=department,
        is_active=True,
        show_on_qr_page=visible,
    )
    db.add(row)
    db.commit()
    db.refresh(row)
...
def test_last_department_deactivation_stays_empty(
    client, db_session, admin_user, action, public
):
...
def test_truly_unconfigured_profile_table_keeps_bootstrap_fallback(
    ...
    for suffix, key in [("", "profiles"), ("/public", "specialists")]:
        result = client.get(
            "/api/v1/queues/profiles" + suffix, headers=_headers(admin_user)
        )
        assert result.status_code == 200, result.text
...
def test_profile_key_queue_counts_as_usage_even_outside_explicit_tags(
    ...
    db_session.flush()
    queue = DailyQueue(
        day=date.today(), specialist_id=doctor.id, queue_tag=key, active=True
    )
    db_session.add(queue)
    db_session.flush()
    entry = OnlineQueueEntry(
        queue_id=queue.id, number=1, status="waiting", source="online"
    )
    ...
    if operation == "preview":
        response = client.get(
            endpoint + "/impact-preview", headers=_headers(admin_user)
        )
        assert response.status_code == 200, response.text
        data = response.json()
...
def test_unused_profile_with_distinct_key_and_tags_can_still_be_deleted(
    client, db_session, admin_user
):
    key = "review_" + uuid.uuid4().hex[:10]
    ...
    response = client.delete(
        f"/api/v1/queues/profiles/{key}", headers=_headers(admin_user)
    )
    assert response.status_code == 200, response.text
    db_session.expire_all()
