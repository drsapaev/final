...
- AFTER the (day, tag) prelocks and BEFORE the first visit INSERT, every
  selected Doctor row is re-read ``FOR SHARE`` (sorted ids) and every owner
  User row too, and eligibility is re-run on the locked snapshot;
- the row locks are held until the cart's single commit, so a concurrent
  eligibility-changing UPDATE either blocks (the snapshot the cart
  validated stays valid until commit) or is already committed and VISIBLE
  to the locked re-read (cart rejected before any write);
- the lock MODE is FOR SHARE (never FOR UPDATE): compatible with the visit
  INSERT's FK KEY SHARE and with GraphQL joinQueue's ``Doctor FOR SHARE``
  (advisory tag lock first, then the doctor row) — no new lock-order
  inversion.

Proofs (two real workers against one PostgreSQL):

1. ``..._blocks_concurrent_doctor_deactivation``: while the cart holds the
   locked revalidation snapshot, ``UPDATE doctors SET active=false`` times
   out on the row lock; after the cart commits, the UPDATE lands.
...
3. ``..._rejects_cart_when_deactivation_commits_first``: the deactivation
   ...
   Visit/Invoice/QueueEntry rows (this is the exact race the unlocked gate
   alone cannot close).
"""
...
class _CartWorkerProbe:
    ...
    - ``"locked_user_read"``: right AFTER the locked revalidation's
      ``FROM users ... FOR SHARE`` SELECT (advisory + doctor + user row
      locks all held, nothing written yet).
...
def test_cart_locked_eligibility_blocks_concurrent_doctor_deactivation(
    ...
    # lock protocol: tag prelock BEFORE the doctor row lock; FOR SHARE, not
    # FOR UPDATE, on both re-reads
    advisory_index = _statement_index(probe, "pg_advisory_xact_lock")
    doctor_share_index = _statement_index(probe, "FOR SHARE")
    assert advisory_index is not None, "no tag claim prelock captured"
    assert doctor_share_index is not None, "no FOR SHARE re-read captured"
    assert advisory_index < doctor_share_index, (
