...
(``?hostaddr=``, or a remote ``?host=``) are rejected, so a remote
admin DSN must be
...
def _candidate_admin_urls() -> list[str]:
    ...
    rejected even though the authority is empty; the address-altering
    ``?hostaddr=`` is rejected outright (round-2 review P1) — it dials a
    remote address even when the authority spells a loopback. A remote
