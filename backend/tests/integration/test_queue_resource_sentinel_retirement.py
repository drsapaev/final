...
# ========= F. Foreign-source FK protection pins (review of #3324) ========
#
# The E-pins prove what the inventory must IGNORE (foreign worlds the
# pairs never touch). This block proves the complementary half — the
# one the pre-review #3324 got wrong by scoping the SOURCE side to
# current_schema(): a foreign-schema table that REFERENCES the current
# schema's users/doctors is a LIVE FK surface. Silently skipping it
# would let the pair DELETE fire a cross-schema ON DELETE action the
# migration never inventoried (a reporting.audit_rows -> public.users
# CASCADE silently deleting audit rows; a foreign SET NULL silently
# anonymizing). The pins:
#
# - C1: the foreign-source surface is DISCOVERED (src_schema carries
#   the foreign namespace) and a referencing row fail-closed aborts
#   BEFORE the deletion — the foreign child row survives;
# - C2: the self-reference exclusion and the login_attempts SET NULL
#   allowlist are LOCAL to the current schema — a foreign "doctors" /
#   "login_attempts" does not inherit the exemption by name collision;
# - C3: a foreign-source surface whose rows reference a NON-pair user
#   does not false-abort — the catch-all targets the pair ids, not
#   "any FK anywhere".


def _create_foreign_child(engine, schema: str, table: str, rule: str | None) -> None:
    """A foreign-schema child with an FK landing on the CURRENT schema's
    users.id (the ON DELETE rule selectable per test)."""
    rule_sql = f" ON DELETE {rule}" if rule else ""
    with engine.begin() as conn:
        ...
        conn.execute(
            sa.text(
                ...
                f'"id" INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,'
                f'"user_id" INTEGER NOT NULL REFERENCES users(id){rule_sql})'
            )
        )
...
@pytest.mark.integration
@pytest.mark.migration
def test_pg_introspection_aborts_on_a_foreign_source_cascade(
    retirement_pg_engine,
) -> None:
    """The review's exact scenario: a foreign-schema child references
    the current users with ON DELETE CASCADE and holds a row pointing
    at a synthetic user. The inventory must SEE the surface (src_schema
    carries the foreign namespace) and the retirement must abort before
    the deletion — the foreign child row and the pairs all survive."""
    module = _module()
    engine = retirement_pg_engine
    foreign = _foreign_schema_name()
    _create_foreign_child(engine, foreign, "audit_rows", "CASCADE")
    try:
        with engine.connect() as conn:
            # discovery-level pin: the foreign surface IS inventoried,
            # with its namespace, the right ref side and delete rule
            rows = conn.execute(module._SELECT_FK_SURFACES).fetchall()
            conn.rollback()
        ...
        with engine.begin() as conn:
            conn.execute(
                sa.text(f'INSERT INTO "{foreign}".audit_rows (user_id) VALUES (:u)'),
                {"u": pairs["ecg_resource"][0]},
            )

        with engine.connect() as conn:
            with pytest.raises(RuntimeError, match="FK surface") as excinfo:
                module.upgrade_with_conn(conn)
            conn.rollback()
            assert foreign in str(excinfo.value)
            assert "CASCADE" in str(excinfo.value)

            # the abort happened BEFORE the deletion — the foreign
            # child row survived (no silent cross-schema cascade) and
            # the synthetic pairs are intact
            (child_rows,) = conn.execute(
                sa.text(f'SELECT COUNT(*) FROM "{foreign}".audit_rows')
            ).fetchone()
...
@pytest.mark.integration
@pytest.mark.migration
def test_pg_introspection_aborts_on_a_foreign_set_null_surface(
    retirement_pg_engine,
) -> None:
    """A foreign-schema child with ON DELETE SET NULL holding a row
    pointing at a synthetic user is an UNKNOWN anonymization surface —
    the migration contract allows ONLY login_attempts (current schema)
    to carry SET NULL rows; every other surface is a loud abort."""
    module = _module()
    engine = retirement_pg_engine
    foreign = _foreign_schema_name()
    _create_foreign_child(engine, foreign, "shadow_notes", "SET NULL")
    try:
        with engine.connect() as conn:
            pairs = _seed_all_pairs(conn)
            conn.commit()
        ...
        with engine.begin() as conn:
            conn.execute(
                sa.text(f'INSERT INTO "{foreign}".shadow_notes (user_id) VALUES (:u)'),
                {"u": pairs["lab_resource"][0]},
            )
        ...
        with engine.connect() as conn:
            ...
            # nothing was silently anonymized: the foreign row still
            # points at the (surviving) synthetic user id
            (linked,) = conn.execute(
                sa.text(
                    f'SELECT COUNT(*) FROM "{foreign}".shadow_notes'
...
@pytest.mark.integration
@pytest.mark.migration
def test_pg_allowlist_does_not_inherit_across_schemas(
    retirement_pg_engine,
) -> None:
    """The allowlist is LOCAL: a foreign-schema table NAMED
    login_attempts with ON DELETE SET NULL is NOT the migration's own
    allowlisted surface — a name collision must never turn a foreign
    anonymization into a silent pass."""
    module = _module()
    engine = retirement_pg_engine
    foreign = _foreign_schema_name()
    _create_foreign_child(engine, foreign, "login_attempts", "SET NULL")
    try:
        with engine.connect() as conn:
            pairs = _seed_all_pairs(conn)
            conn.commit()
...
@pytest.mark.integration
@pytest.mark.migration
def test_pg_self_reference_exclusion_does_not_inherit_across_schemas(
    retirement_pg_engine,
) -> None:
    """The self-reference exclusion is LOCAL: a foreign-schema table
    NAMED doctors whose user_id references the CURRENT users is not the
    pairs' own self-reference — the name collision must not buy it a
    silent skip."""
    module = _module()
    engine = retirement_pg_engine
    foreign = _foreign_schema_name()
    _create_foreign_child(engine, foreign, "doctors", "CASCADE")
    try:
        with engine.connect() as conn:
            pairs = _seed_all_pairs(conn)
            conn.commit()
        ...
        with engine.begin() as conn:
            conn.execute(
                sa.text(f'INSERT INTO "{foreign}".doctors (user_id) VALUES (:u)'),
                {"u": pairs["ecg_resource"][0]},
            )
...
@pytest.mark.integration
@pytest.mark.migration
def test_pg_foreign_surface_on_non_pair_rows_does_not_false_abort(
    retirement_pg_engine,
) -> None:
    """The catch-all targets the PAIR ids, not "any FK anywhere": a
    foreign-schema surface holding rows that reference a NON-synthetic
    user is inventoried, counted — and passes."""
    module = _module()
    engine = retirement_pg_engine
    foreign = _foreign_schema_name()
    _create_foreign_child(engine, foreign, "audit_rows", "CASCADE")
    try:
        with engine.connect() as conn:
            ...
            conn.execute(
                sa.text(
                    "INSERT INTO users (username, hashed_password, role,"
                    " is_active, is_superuser, must_change_password)"
                    " VALUES ('real_person', :p, 'Patient', true, false, false)"
                ),
                {"p": _DISABLED_HASH},
            )
            (real_user_id,) = conn.execute(
                sa.text("SELECT id FROM users WHERE username = 'real_person'")
            ).fetchone()
            conn.commit()
        ...
        with engine.connect() as conn:
            ...
            assert result == {"users_deleted": 3, "doctors_deleted": 3}
            assert _usernames_present(conn) == set()
            # the foreign child row was never the migration's business:
            # it survives untouched, still pointing at its own user
            (child_rows,) = conn.execute(
                sa.text(f'SELECT COUNT(*) FROM "{foreign}".audit_rows')
            ).fetchone()
            assert int(child_rows) == 1
...
# ===== G. Reference-target identity pins (owner review, unified #3324) =====
#
# The F-pins prove WHERE an FK comes from (any source schema is
# inventoried). This block proves WHAT the FK lands on. PostgreSQL
# allows an FK to reference ANY unique key — not only the primary key —
# and this schema has live alternates (doctors.user_id, users.username,
# users.email are UNIQUE). The inventory counts id values; against an
# alternate-key surface that count is meaningless: id values match
# nothing, the guard reports a false 0, and the pair DELETE still fires
# the ON DELETE action through the alternate-key FK (a doctors.user_id
# CASCADE silently wiping foreign rows — the exact contract breach).
# The contract: the retirement supports ONLY FKs into the pairs'
# PRIMARY KEY id — every other referenced column travels with the
# surface and is a loud unsupported-reference-surface abort BEFORE any
# counting and any deletion, never a silent pass and never the opaque
# pre-fix counting error.
#
# - G1: a foreign-schema FK -> doctors(user_id) ON DELETE CASCADE with
#   a row holding the synthetic DOCTOR's user_id value. Pre-fix this
#   surface was counted against doctor id values (a false 0) and the
#   pair DELETE cascaded into the foreign child silently.
# - G2: an FK -> users.username must abort DETERMINISTICALLY as an
#   unsupported reference surface — never the pre-fix opaque
#   "operator does not exist: text = integer" counting error against
#   the text column, and never a silent pass.


@pytest.mark.integration
@pytest.mark.migration
def test_pg_introspection_aborts_on_fk_to_doctors_alternate_key(
    retirement_pg_engine,
) -> None:
    """The review's exact counterexample: shadow.audit.doctor_user_id ->
    doctors(user_id) ON DELETE CASCADE, holding the synthetic doctor's
    user_id value. The referenced column must travel with the surface,
    and a non-id target must abort the retirement BEFORE the deletion —
    the foreign child row and the pairs all survive."""
    module = _module()
    engine = retirement_pg_engine
    foreign = _foreign_schema_name()
    with engine.begin() as conn:
        conn.execute(sa.text(f'CREATE SCHEMA IF NOT EXISTS "{foreign}"'))
        conn.execute(
            sa.text(
                f'CREATE TABLE "{foreign}".audit ('
                f'"id" INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,'
                f'"doctor_user_id" INTEGER NOT NULL'
                f" REFERENCES doctors(user_id) ON DELETE CASCADE)"
            )
        )
    try:
        with engine.connect() as conn:
            # discovery-level pin: the referenced COLUMN is part of the
            # inventory — this FK lands on doctors.user_id, NOT the
            # primary key id
            rows = conn.execute(module._SELECT_FK_SURFACES).fetchall()
            conn.rollback()
            foreign_rows = [row for row in rows if row.src_schema == foreign]
            ...
            # Four filler users seeded BEFORE the pairs force the User and
            # Doctor serial spaces apart (filler user ids 1-4, pair user
            # ids 5-7 vs doctor ids 1-3), so the alternate-key value
            # can NEVER collide with a doctor id: a count against doctor
            # id values is provably a false 0, exactly the review's
            # Doctor.id=12 / Doctor.user_id=23 shape.
            for filler in range(4):
                conn.execute(
                    sa.text(
                        "INSERT INTO users (username, hashed_password, role,"
                        " is_active, is_superuser, must_change_password)"
                        " VALUES (:u, :p, 'Patient', true, false, false)"
                    ),
                    {"u": f"filler_{filler}", "p": _DISABLED_HASH},
                )
            pairs = _seed_all_pairs(conn)
            (max_doctor_id,) = conn.execute(
                sa.text("SELECT MAX(id) FROM doctors")
            ).fetchone()
            assert int(pairs["ecg_resource"][0]) > int(max_doctor_id), (
                "test premise: the alternate-key value must be disjoint "
                "from every doctor id, otherwise a false abort could fire "
                "for the wrong reason"
            )
            conn.commit()

        with engine.begin() as conn:
            # the child row holds the synthetic DOCTOR's user_id value
        ...
        with engine.connect() as conn:
            with pytest.raises(
                RuntimeError, match="unsupported reference surface"
            ) as excinfo:
                module.upgrade_with_conn(conn)
            conn.rollback()
            assert f"{foreign}.audit.doctor_user_id" in str(excinfo.value)
            assert "doctors.user_id" in str(excinfo.value)

            # the abort happened BEFORE the deletion: the alternate-key
            # child row survived (no silent cascade) and the pairs are
            # intact
            (child_rows,) = conn.execute(
                sa.text(f'SELECT COUNT(*) FROM "{foreign}".audit')
            ).fetchone()
            assert int(child_rows) == 1
            assert _usernames_present(conn) == set(_PAIR_USERNAMES)
            assert _pair_doctor_count(conn) == 3
...
@pytest.mark.integration
@pytest.mark.migration
def test_pg_introspection_aborts_deterministically_on_fk_to_users_username(
    retirement_pg_engine,
) -> None:
    """An FK into users.username (an alternate UNIQUE key) must be a
    deterministic unsupported-reference-surface abort carrying the
    surface and the target column — never the pre-fix opaque
    "operator does not exist: text = integer" counting error against
    the text column, and never a silent pass."""
    module = _module()
    engine = retirement_pg_engine
    ...
    with engine.connect() as conn:
        # discovery-level pin: the referenced column is reported as
        # username, not silently normalized to the primary key
        rows = conn.execute(module._SELECT_FK_SURFACES).fetchall()
        conn.rollback()
        local_rows = [row for row in rows if row.table_name == "legacy_profiles"]
        assert len(local_rows) == 1
        assert local_rows[0].column_name == "legacy_username"
        assert local_rows[0].ref_table == "users"
        assert local_rows[0].ref_column == "username"
    ...
    with engine.begin() as conn:
        conn.execute(
            sa.text("INSERT INTO legacy_profiles (legacy_username) VALUES (:u)"),
            {"u": "ecg_resource"},
        )
    ...
    with engine.connect() as conn:
        with pytest.raises(
            ...
            module.upgrade_with_conn(conn)
        conn.rollback()
        assert "legacy_profiles.legacy_username" in str(excinfo.value)
        assert "users.username" in str(excinfo.value)

        # the abort happened BEFORE the deletion — the pairs survive
        # and the child row is untouched
        assert _usernames_present(conn) == set(_PAIR_USERNAMES)
        (child_rows,) = conn.execute(
            sa.text("SELECT COUNT(*) FROM legacy_profiles")
        ).fetchone()
        assert int(child_rows) == 1
