...
def _candidate_admin_urls() -> list[str]:
    ...
    if env_url:
        u = make_url(env_url)
        # Sibling PR (audit P1 of #3468, same defect class): a libpq DSN
        # can carry a comma-separated failover host list (netloc host or
        # ?host= / ?hostaddr= query params). EVERY possible endpoint — not
        # just the first failover target — must be local before scratch
...
def pg_engine():
    ...
    # Sibling PR (audit P2 of #3468, same defect class): from this point on
    # a scratch database EXISTS on the admin server. Run-unique names mean
    # the next run can no longer sweep a leaked database (the old fixed-name
    # pre-drop used to), so every provisioning step between CREATE DATABASE
    # and the yield is failure-safe: teardown runs on ALL exit paths — a
    # pre-yield setup failure included — instead of only after a successful
    # yield.
    engine = None
    try:
        ...
        if engine is not None:
            engine.dispose()
        with psycopg.connect(admin_url, autocommit=True) as c:
            # Cleanup touches ONLY the run-unique database this process
            # created; WITH (FORCE) clears lingering connections (PG 13+),
            # falling back to the plain form on older servers.
            try:
