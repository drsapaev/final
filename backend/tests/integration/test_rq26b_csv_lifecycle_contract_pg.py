...
def test_department_create_key_contract_accepts_a_single_letter():
    """Round-5 review P2 (serverless): the server key contract is
    ``min_length=1`` — a single lowercase letter is a VALID department
    key — pinned at the schema so the admin form's mirror (which must
    accept the same value before the submit) cannot drift apart again.
    """
    ...
    validated = DepartmentCreate.model_validate({"key": "a", "name_ru": "синт"})
    assert validated.key == "a"
    with pytest.raises(ValidationError, match="at least 1 character"):
        DepartmentCreate.model_validate({"key": "", "name_ru": "синт"})
