...
Возвращает информацию о визите по токену подтверждения. Карточка patient-safe: внутреннее поле `notes` (клинический/административный текст) не возвращается ни через этот маршрут, ни через POST /visits/info — обе формы публикуют одну и ту же безопасную проекцию (PR 3407 delta review).

Ответы обеих форм помечены `Cache-Control: private, no-store` (карточка содержит PHI по bearer-токену и не должна кэшироваться браузерами/посредниками); текст внутренних ошибок 5xx скрывается за общим сообщением (PR 3417 review).
