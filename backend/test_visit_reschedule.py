...
def test_create_visit(token, patient_id):
    ...
    visit_data = {
        ...
        "notes": "Тестовый визит для переноса"
    }
...
def test_reschedule_visit(token, visit_id):
    ...
    reschedule_url = f"{BASE_URL}/api/v1/visits/visits/{visit_id}/reschedule?new_date={tomorrow}"
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                print(f"    ❌ Ошибка переноса визита: {response.read().decode('utf-8')}")
                return False
    except Exception as e:
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                print(f"    ❌ Ошибка переноса на завтра: {response.read().decode('utf-8')}")
                return False
    except Exception as e:
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                print(f"    ❌ Ошибка получения статуса: {response.read().decode('utf-8')}")
                return False
    except Exception as e:
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                if 'visit' in result:
                    visit = result['visit']
                    print(f"    ✅ Детали визита получены: ID {visit['id']}, статус: {visit.get('status', 'N/A')}")
                    return True
                else:
                    print(f"    ⚠️ Неожиданная структура ответа: {result}")
                    return False
            else:
                print(f"    ❌ Ошибка получения деталей: {response.read().decode('utf-8')}")
                return False
    except Exception as e:
...
def main():
    ...
    print(f"  ✅ Перенос визита на другой день: {'УСПЕШНО' if reschedule_success else 'ОШИБКА'}")
    print(f"  ✅ Перенос визита на завтра: {'УСПЕШНО' if tomorrow_success else 'ОШИБКА'}")
    print(f"  ✅ Получение статуса визита: {'УСПЕШНО' if status_success else 'ОШИБКА'}")
    print(f"  ✅ Получение деталей визита: {'УСПЕШНО' if details_success else 'ОШИБКА'}")
    
    success_count = sum([bool(patient_id), bool(visit_id), reschedule_success, tomorrow_success, status_success, details_success])
    total_count = 6
