...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("visit_id", sa.Integer(), sa.ForeignKey("visits.id", ondelete="CASCADE"), nullable=False, unique=True),
        sa.Column("subtotal_services", sa.Numeric(12, 2), nullable=False, server_default="0"),
        ...
        sa.Column("status", sa.String(length=16), nullable=False, server_default="open"),  # open|partial|paid
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    op.create_index("ux_visit_billings_visit", "visit_billings", ["visit_id"], unique=True)
