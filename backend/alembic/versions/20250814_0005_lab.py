...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("status", sa.String(length=16), nullable=False, server_default="ordered"),  # ordered|in_progress|done|canceled
    ...
    op.create_table(
        "lab_results",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("order_id", sa.Integer(), sa.ForeignKey("lab_orders.id", ondelete="CASCADE"), nullable=False),
