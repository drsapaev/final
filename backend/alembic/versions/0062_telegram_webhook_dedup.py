...
update_id makes the claim INSERT atomic, so two concurrent deliveries of
the same update cannot both win.
...
def upgrade() -> None:
    ...
    op.create_index(
        "uq_telegram_webhook_dedup_update_id",
        "telegram_webhook_dedup",
        ["update_id"],
        unique=True,
    )
...
def downgrade() -> None:
    op.drop_index(
        "uq_telegram_webhook_dedup_update_id", table_name="telegram_webhook_dedup"
    )
