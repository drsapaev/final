...
def _create_fk_if_absent(op, conn, name, source, target, local_cols, remote_cols, ondelete=None):
    ...
    if _fk_exists(conn, source, name):
        return
    op.create_foreign_key(name, source, target, local_cols=local_cols, remote_cols=remote_cols, ondelete=ondelete)
