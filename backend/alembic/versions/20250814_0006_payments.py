...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("visit_id", sa.Integer(), sa.ForeignKey("visits.id", ondelete="SET NULL"), nullable=True),
        sa.Column("cashier_id", sa.Integer(), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True),
        sa.Column("amount", sa.Numeric(12, 2), nullable=False, server_default="0"),
        sa.Column("currency", sa.String(length=8), nullable=False, server_default="UZS"),
        sa.Column("method", sa.String(length=16), nullable=False, server_default="cash"),  # cash|card|transfer
        sa.Column("status", sa.String(length=16), nullable=False, server_default="paid"),  # paid|void|refunded
