        ...
        with httpx.Client() as client:
            ...
            if weekly_response.status_code == 200:
                ...
                for day in weekly_data[:2]:  # Показываем первые 2 дня
                    print(f"   - {day.get('weekday_name')} {day.get('date')}: {len(day.get('templates', []))} шаблонов")
            else:
                print(f"❌ Ошибка получения недельного расписания: {weekly_response.text}")
            ...
            if slots_response.status_code == 200:
                ...
                for slot in slots_data[:3]:  # Показываем первые 3 слота
                    print(f"   - {slot.get('time')}: {slot.get('available_capacity')}/{slot.get('total_capacity')} мест")
            else:
            ...
            if dept_response.status_code == 200:
                ...
                for dept in dept_data:
                    print(f"   - {dept.get('department')}: {dept.get('template_count')} шаблонов")
            else:
            ...
            if dept_queue_response.status_code == 200:
                ...
                for dept in dept_queue_data:
                    print(f"   - {dept.get('department')}: ожидают {dept.get('waiting')}")
            else:
                print(f"⚠️  Получение отделений с очередями: {dept_queue_response.status_code}")
