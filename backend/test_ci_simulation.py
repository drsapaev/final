...
def test_ci_simulation():
    ...
    if not run_command("pip install requests argon2-cffi", "Установка дополнительных зависимостей"):
        success = False
    
    # 2. Создание тестовой базы данных
    if not run_command("python -c \"from app.db.base import Base; from app.db.session import engine; Base.metadata.create_all(bind=engine); print('База данных создана')\"", "Создание тестовой БД"):
        success = False
    
    # 3. Создание тестовых пользователей
    if not run_command("python reset_all_passwords.py", "Создание тестовых пользователей"):
        success = False
    ...
    if success:
        ...
        if not run_command("python -c \"from app.core.role_validation import validate_critical_user_roles; print('OK: Role validation passed' if validate_critical_user_roles() else 'ERROR: Role validation failed')\"", "Валидация ролей"):
            success = False
