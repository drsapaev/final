...
class AIModelInfo(BaseModel):
    ...
    provider_name: str = Field(..., description="Название провайдера (OpenAI, Gemini, etc.)")
    model_name: str = Field(..., description="Название модели (gpt-4, gemini-pro, etc.)")
    ...
    capabilities: Optional[Dict[str, Any]] = Field(None, description="Возможности модели")
...
class AIRequestTracking(BaseModel):
    ...
    task_type: str = Field(..., description="Тип задачи (analyze_complaints, generate_prescription, etc.)")
    specialty: Optional[str] = Field(None, description="Специализация (cardio, derma, dental, etc.)")
    ...
    # Временные метки
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Время создания запроса")
...
class AIResponseWithTracking(BaseModel):
    ...
    model_confidence: Optional[float] = Field(None, description="Уверенность модели (0-1)")
