...
class DiscountBenefitsService:
    ...
    def get_active_discounts(
        ...
        query = self.db.query(Discount).filter(
            ...
            or_(Discount.end_date.is_(None), Discount.end_date >= datetime.now()),
        )
    ...
    def calculate_discount(
        self, discount: Discount, amount: float, quantity: int = 1
    ) -> float:
    ...
    def verify_patient_benefit(
        ...
        patient_benefit = (
            self.db.query(PatientBenefit)
            .filter(PatientBenefit.id == patient_benefit_id)
            .first()
        )
    ...
    def calculate_benefit_discount(
        self, patient_benefit: PatientBenefit, amount: float
    ) -> float:
        ...
        if (
            patient_benefit.document_expiry_date
            and patient_benefit.document_expiry_date < datetime.now()
        ):
            return 0
        ...
        if (
            ...
            and discount_amount > benefit.max_discount_amount
        ):
    ...
    def get_monthly_benefit_usage(self, patient_benefit_id: int) -> float:
        ...
        result = (
            self.db.query(func.sum(BenefitApplication.benefit_amount))
            .filter(
    ...
    def get_yearly_benefit_usage(self, patient_benefit_id: int) -> float:
        ...
        current_year = datetime.now().replace(
    ...
    def enroll_patient_in_loyalty(
        self, patient_id: int, program_id: int
    ) -> PatientLoyalty:
        ...
        patient_loyalty = PatientLoyalty(patient_id=patient_id, program_id=program_id)
    ...
    def earn_loyalty_points(
        ...
        program = (
            ...
            .filter(LoyaltyProgram.id == program_id, LoyaltyProgram.is_active == True)
            .first()
        )
    ...
    def redeem_loyalty_points(
        ...
        if points < program.min_points_to_redeem:
            raise ValueError(
                f"Минимум для списания: {program.min_points_to_redeem} баллов"
            )
    ...
    def calculate_total_discount(
        ...
        active_programs = (
            self.db.query(LoyaltyProgram).filter(LoyaltyProgram.is_active == True).all()
        )
        ...
        for patient_benefit in patient_benefits:
            benefit_amount = self.calculate_benefit_discount(
                patient_benefit, current_amount
            )
            if benefit_amount > 0:
    ...
    def get_benefit_analytics(
        ...
        return {
            ...
            'average_benefit_percentage': (
                (total_benefit_amount / total_original_amount * 100)
                if total_original_amount > 0
                else 0
            ),
    ...
    def get_loyalty_analytics(self, program_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        total_points_earned = sum(
            record.total_points_earned for record in loyalty_records
        )
        total_points_redeemed = sum(
            record.total_points_redeemed for record in loyalty_records
        )
        total_amount_spent = sum(
            record.total_amount_spent for record in loyalty_records
        )
        ...
        return {
            ...
            'average_points_per_patient': (
                total_points_earned / total_patients if total_patients > 0 else 0
            ),
            'redemption_rate': (
                (total_points_redeemed / total_points_earned * 100)
