...
class MockProvider(BaseAIProvider):
    ...
    async def analyze_complaint(self, complaint: str, patient_info: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if "головн" in complaints_lower or "голов" in complaints_lower:
            ...
            consultations.extend(["Невролог", "Офтальмолог (для исключения внутричерепной гипертензии)"])
        ...
        if "тошнот" in complaints_lower or "рвот" in complaints_lower:
            ...
            lab_tests.append("Биохимический анализ крови (АЛТ, АСТ, билирубин, амилаза)")
            
        if "температур" in complaints_lower or "жар" in complaints_lower or "лихорад" in complaints_lower:
            preliminary_diagnosis.extend([
            ...
            red_flags.append("⚠️ Лихорадка >38.5°C более 3 дней или >39°C требует обязательной консультации врача")
            red_flags.append("⚠️ Сопутствующие симптомы: ригидность затылочных мышц, спутанность сознания, петехиальная сыпь")
            consultations.extend(["Терапевт", "Инфекционист (при подозрении на инфекционное заболевание)"])
        ...
        if "боль" in complaints_lower and "груд" in complaints_lower:
            ...
            consultations.extend(["🚨 ЭКСТРЕННАЯ госпитализация", "Кардиолог", "Пульмонолог"])
        ...
        if "кож" in complaints_lower or "сып" in complaints_lower or "зуд" in complaints_lower:
            preliminary_diagnosis.extend([
        ...
        urgency = "экстренно" if red_flags and "🚨" in str(red_flags) else (
