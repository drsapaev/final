...
class PIIAnonymizer(IAnonymizer):
    ...
    def _sanitize_text(self, text: str) -> str:
        ...
        FA-006: добавлены паттерны для ФИО, дат рождения, адресов.
        """
        if not text:
            return text
        ...
        # FA-006: ФИО — "Иван Петров", "Мария Сидорова" (два слова с заглавной)
        text = re.sub(
            r'\b([А-ЯЁ][а-яё]+\s+[А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+)?)\b',
            '[NAME]',
            text
        )
        # Фамилия И.О. — "Сидоров И.П."
        text = re.sub(
            r'\b([А-ЯЁ][а-яё]+\s+[А-ЯЁ]\.[А-ЯЁ]\.)\b',
            '[NAME]',
            text
        )
        # "Пациент Иванов", "Больной Петров"
        text = re.sub(
            r'\b(?:пациент|больной|пациентка)\s+[А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+)?\b',
            '[PATIENT_NAME]',
            text,
            flags=re.IGNORECASE
        )

        # FA-006: даты рождения
        text = re.sub(
        ...
        # FA-006: адреса (упрощённо)
        text = re.sub(
            r'\b(ул\.|улица|пр\.|проспект|пер\.)\s+[А-ЯЁа-яё]+,?\s*\d+',
            '[ADDRESS]',
            text
        )
