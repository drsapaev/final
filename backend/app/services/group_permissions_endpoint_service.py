...
@dataclass
class GroupPermissionsDomainError(Exception):
    status_code: int
    detail: str
...
class GroupPermissionsService:
    ...
    def get_user_permissions_payload(self, *, user_id: int, use_cache: bool) -> dict:
        ...
        if not user:
            raise GroupPermissionsDomainError(404, "Пользователь не найден")
    ...
    def create_permission_override(
        ...
        *,
        ...
        if not permission:
            raise GroupPermissionsDomainError(404, "Разрешение не найдено")
        ...
        if existing:
            raise GroupPermissionsDomainError(
                400,
