...
class GroupPermissionsService:
    ...
    def get_user_permissions(
        ...
        try:
            ...
            user_roles = (
                ...
                .filter(user_roles_table.c.user_id == user_id, Role.is_active == True)
                .all()
            )
            ...
            user_groups = (
                ...
                .filter(
                    user_groups_table.c.user_id == user_id, UserGroup.is_active == True
                )
                .all()
            )
            ...
            self.logger.error(
                f"Ошибка получения разрешений пользователя {user_id}: {e}"
            )
            return set()
    ...
    def _get_group_permissions(self, db: Session, group_id: int) -> Set[str]:
        ...
        group_roles = (
            ...
            .filter(group_roles_table.c.group_id == group_id, Role.is_active == True)
            .all()
        )
    ...
    def has_permission(
        self, db: Session, user_id: int, permission_codename: str
    ) -> bool:
        """
    ...
    def get_group_permissions_summary(
        ...
        try:
            ...
            for perm_codename in permissions:
                permission = (
                    db.query(Permission)
                    .filter(Permission.codename == perm_codename)
                    .first()
                )
                if permission:
            ...
            group_roles = (
                ...
                .join(group_roles_table, Role.id == group_roles_table.c.role_id)
                .filter(
                    group_roles_table.c.group_id == group_id, Role.is_active == True
                )
                .all()
            )
            ...
            users_count = (
                ...
                .filter(
                    user_groups_table.c.group_id == group_id, User.is_active == True
                )
                .count()
            )
