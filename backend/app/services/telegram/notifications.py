...
class TelegramNotificationService:
    ...
    async def send_appointment_reminder(
        ...
        try:
            ...
            # Логируем отправку
            logger.info(f"Appointment reminder sent to user {user_id} for visit {visit_id}")
            return True
            ...
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="📄 Скачать рецепт",
                        web_app=WebAppInfo(url=f"{settings.FRONTEND_URL}/prescription/{prescription_id}")
                    )
                ]
            ])
        ...
        for user_id in user_ids:
            try:
                ...
                if user and user.telegram_id:
                    success = await self.bot.send_notification(user.telegram_id, message)
                    if success:
                        results["sent"] += 1
                    else:
                        results["failed"] += 1
