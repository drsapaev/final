...
class ClinicTelegramBot:
    ...
    async def handle_start(self, message: types.Message):
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="🏥 Утренняя очередь", callback_data="queue"),
                InlineKeyboardButton(text="📅 Записаться на прием", callback_data="appointment")
            ],
            [
...
Выберите нужную функцию из меню ниже:
    ...
    async def handle_queue(self, message: types.Message):
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="👨‍⚕️ Терапевт", callback_data="queue_therapist"),
                InlineKeyboardButton(text="❤️ Кардиолог", callback_data="queue_cardiologist")
            ],
            [
                InlineKeyboardButton(text="👩‍⚕️ Дерматолог", callback_data="queue_dermatologist"),
                InlineKeyboardButton(text="🦷 Стоматолог", callback_data="queue_dentist")
            ],
            [
                InlineKeyboardButton(text="🔬 Лаборатория", callback_data="queue_lab")
            ]
        ])
    ...
    async def _handle_queue_callback(self, callback: types.CallbackQuery, data: str):
        ...
        try:
            ...
            if result.get("success"):
                ...
                await callback.message.answer(f"❌ Не удалось встать в очередь:\n{error_msg}")
...
Не забудьте прийти за 15 минут до назначенного времени.
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"confirm_{appointment_data.get('id')}"),
                InlineKeyboardButton(text="❌ Отменить", callback_data=f"cancel_{appointment_data.get('id')}")
            ]
        ])
