...
class AnalyticsVisualizationService:
    ...
    def create_doctor_performance_charts(
        ...
        if doctors:
            ...
            top_doctors = sorted(doctors, key=lambda x: x.get("performance_score", 0), reverse=True)[:5]
            
            charts["top_doctors"] = ChartData(
                labels=[doc.get("doctor_name", "") for doc in top_doctors],
                datasets=[{
                    "label": "Оценка эффективности",
                    "data": [doc.get("performance_score", 0) for doc in top_doctors],
            ...
            charts["completion_rates"] = ChartData(
                ...
                datasets=[{
                    ...
                    "data": [doc.get("completion_rate_percent", 0) for doc in top_doctors],
    ...
    def create_comprehensive_visualization(
        ...
        if "doctor_performance" in comprehensive_data:
            doctor_charts = self.create_doctor_performance_charts(comprehensive_data["doctor_performance"])
            visualization["charts"]["doctors"] = doctor_charts
        
        # Графики пациентов
        if "patient_analytics" in comprehensive_data:
            patient_charts = self.create_patient_analytics_charts(comprehensive_data["patient_analytics"])
            visualization["charts"]["patients"] = patient_charts
        
        # Графики доходов
        if "revenue_analytics" in comprehensive_data:
            revenue_charts = self.create_revenue_charts(comprehensive_data["revenue_analytics"])
