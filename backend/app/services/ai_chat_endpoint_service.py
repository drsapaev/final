...
@dataclass
class AIChatDomainError(Exception):
    status_code: int
    detail: str


class AIChatEndpointService:
    ...
    def get_session_payload(self, *, session_id: int, user_id: int) -> dict:
        ...
        if not session:
            raise AIChatDomainError(status_code=404, detail="Session not found")
    ...
    async def add_feedback_payload(
        ...
        *,
        ...
        if not message:
            raise AIChatDomainError(status_code=404, detail="Message not found")
        ...
        try:
            ...
            raise AIChatDomainError(status_code=400, detail=str(exc)) from exc
