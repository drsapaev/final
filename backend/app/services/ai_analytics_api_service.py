...
async def get_ai_usage_summary(
    ...
    try:
        ...
        summary = {
            ...
            "most_used_function": _get_most_used_function(
                analytics.get("function_breakdown", {})
            ),
...
async def get_function_performance(
    ...
    try:
        ...
        service = get_ai_analytics_service(db)
        analytics = service.get_ai_usage_analytics(
            start_date, end_date, ai_function=function_name
        )
    ...
    try:
        ...
        for function, stats in function_breakdown.items():
            function_costs[function] = {
                ...
                "cost_percentage": (
                    stats.get("total_cost", 0) / cost_analysis.get("total_cost_usd", 1)
                )
                * 100,
            }
...
async def compare_ai_models(
    ...
    try:
        ...
        _service = get_ai_analytics_service(db)
...
def _generate_cost_recommendations(function_costs: dict) -> list[str]:
    ...
    if function_costs:
        ...
        recommendations.append(
            f"Оптимизировать функцию '{most_expensive[0]}' - наибольшие затраты"
        )
