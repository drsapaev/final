...
@dataclass
class QueueReorderDomainError(Exception):
    status_code: int
    detail: str


class QueueReorderEndpointService:
    ...
    def reorder_queue(
        ...
        *,
        ...
        if not queue:
            raise QueueReorderDomainError(404, "Очередь не найдена")

        if current_user.role == "Doctor" and queue.specialist_id != current_user.id:
            raise QueueReorderDomainError(403, "Нет прав для изменения этой очереди")

        entries = self.repository.list_active_entries(queue_id=queue_id)
        if not entries:
            raise QueueReorderDomainError(404, "В очереди нет активных записей")
    ...
    def move_queue_entry(
        ...
        *,
        ...
        if not entry:
            raise QueueReorderDomainError(404, "Запись в очереди не найдена")
