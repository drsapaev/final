    ...
    async def export_emr_to_zip(
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                ...
                json_data = await self.export_emr_to_json(
                    emr_data, include_versions=True
                )
                zip_file.writestr(
                    'emr_data.json', json.dumps(json_data, ensure_ascii=False, indent=2)
                )
        ...
        try:
            ...
            if format_type == 'json':
                ...
                if 'metadata' in json_data:
                    validation_result["format_version"] = json_data['metadata'].get(
                        'format_version'
                    )
                ...
                try:
                    ...
                    if root.tag != 'emr_export':
                        validation_result["errors"].append(
                            "Неправильный корневой элемент XML"
                        )
                        validation_result["is_valid"] = False

                    if root.find('emr_data') is None:
                        validation_result["errors"].append(
                            "Отсутствуют данные EMR в XML"
                        )
                        validation_result["is_valid"] = False
    ...
    async def estimate_export_size(
        ...
        try:
            if format_type == 'json':
                ...
                zip_data = await self.export_emr_to_zip(
                    emr_data, include_attachments=True
                )
                size = len(zip_data)
            else:
                size = 0
