    ...
    try:
            ...
            if entry.patient_id:
                patient_entries = (
                    ...
                    .filter(
                        ...
                        OnlineQueueEntry.visit_id.is_(None),  # ⭐ Только записи без визита (потенциально текущая сессия)
                        OnlineQueueEntry.status.in_(["waiting", "called", "in_service"]),
                    )
                    .all()
                )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "[full_update_online_entry] Failed to update display for entry %d: %s",
                    entry.id,
                    e,
                )
