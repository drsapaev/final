...
def _create_queue_entries(
    ...
    for visit in visits:
        ...
        visit_services = (
            _repo(db)._db.query(VisitService).filter(VisitService.visit_id == visit.id).all()
        )
        service_ids = [vs.service_id for vs in visit_services]
        services = _repo(db)._db.query(Service).filter(Service.id.in_(service_ids)).all()
...
def init_invoice_payment(
    ...
    try:
        # Получаем invoice
        invoice = (
            _repo(db)._db.query(PaymentInvoice)
...
def check_invoice_status(
    ...
    try:
        invoice = (
            _repo(db)._db.query(PaymentInvoice).filter(PaymentInvoice.id == invoice_id).first()
        )
        if not invoice:
            raise HTTPException(status_code=404, detail="Invoice не найден")
            ...
            if provider:
                ...
                if result.success:
                    ...
                    if result.status == "completed":
                        ...
                        invoice_visits = (
                            _repo(db)._db.query(PaymentInvoiceVisit)
                        ...
                        for iv in invoice_visits:
                            visit = (
                                _repo(db)._db.query(Visit).filter(Visit.id == iv.visit_id).first()
                            )
                            if visit:
...
def create_cart_appointments(
    ...
    try:
        ...
        _repo(db)._db.add(invoice)
        _repo(db)._db.flush()  # Получаем ID invoice
        ...
        for visit in created_visits:
            ...
            _repo(db)._db.add(invoice_visit)
        ...
        try:
            for visit in created_visits:
                ...
                patient = (
                    _repo(db)._db.query(Patient).filter(Patient.id == visit.patient_id).first()
                )
                patient_name = (
                ...
                doctor = (
                    _repo(db)._db.query(Doctor).filter(Doctor.id == visit.doctor_id).first()
                    if visit.doctor_id
                    else None
                )
                # [OK] ИСПРАВЛЕНО: User имеет full_name, а не first_name/last_name
                if doctor and doctor.user_id:
                    user = _repo(db)._db.query(User).filter(User.id == doctor.user_id).first()
                    doctor_name = (
                ...
                visit_services = (
                    _repo(db)._db.query(VisitService)
...
def get_pending_price_overrides(
    ...
    """
    try:
        query = _repo(db)._db.query(DoctorPriceOverride).join(Service).join(Doctor)
        ...
        for override in overrides:
            ...
            visit = _repo(db)._db.query(Visit).filter(Visit.id == override.visit_id).first()
            patient_name = None
            if visit:
...
def approve_price_override(
    ...
    try:
        ...
        override = (
            _repo(db)._db.query(DoctorPriceOverride)
...
def get_all_free_requests(
    ...
    """
    try:
        query = _repo(db)._db.query(Visit).filter(Visit.discount_mode == "all_free")
        ...
        for visit in visits:
            ...
            for vs in visit_services:
                service = _repo(db)._db.query(Service).filter(Service.id == vs.service_id).first()
                if service:
            ...
            if visit.doctor_id:
                try:
                    ...
                    if doctor:
                        ...
                        if doctor.user_id:
                            user = (
                                _repo(db)._db.query(User).filter(User.id == doctor.user_id).first()
                            )
                            if user:
                                # [OK] ИСПРАВЛЕНО: User имеет full_name, а не first_name/last_name
                                doctor_name = (
...
def approve_all_free_request(
    ...
    try:
        # Получаем визит
        visit = _repo(db)._db.query(Visit).filter(Visit.id == approval_data.visit_id).first()
...
def get_benefit_settings(
    ...
    try:
        ...
        repeat_days_setting = (
            _repo(db)._db.query(ClinicSettings)
...
def update_wizard_settings(
    ...
    try:
        ...
        if not use_new_wizard_setting:
            ...
            _repo(db)._db.add(use_new_wizard_setting)
        ...
        _repo(db)._db.commit()
        _repo(db)._db.refresh(use_new_wizard_setting)
...
def get_visits(
    ...
    try:
        ...
        try:
            appointments_query = _repo(db)._db.query(Appointment)
        ...
        visits_query = _repo(db)._db.query(Visit)
        ...
        for visit in visits:
            ...
            for vs in visit_services:
                if vs.name:  # Используем сохраненное имя
                    ...
                    service = (
                        _repo(db)._db.query(Service).filter(Service.id == vs.service_id).first()
                    )
                    if service:
                        service_names.append(service.name)
            ...
            doctor_specialty = None
            if visit.doctor_id:
                doctor = _repo(db)._db.query(Doctor).filter(Doctor.id == visit.doctor_id).first()
                if doctor:
                    # [OK] ИСПРАВЛЕНО: User имеет full_name, а не first_name/last_name
                    if doctor.user_id:
                        user = _repo(db)._db.query(User).filter(User.id == doctor.user_id).first()
...
def get_all_appointments(
    ...
    try:
        ...
        for apt in appointments:
            ...
            patient_fio = None
            if apt.patient_id:
                patient = _repo(db)._db.query(Patient).filter(Patient.id == apt.patient_id).first()
                if patient:
                    patient_fio = patient.short_name()
        ...
        for visit in visits:
            ...
            if visit.visit_date == date.today():
                ...
                queue_entries = (
                    _repo(db)._db.query(OnlineQueueEntry)
...
def mark_visit_as_paid(
    ...
    try:
        ...
        # Находим запись
        visit = _repo(db)._db.query(Visit).filter(Visit.id == visit_id).first()
        if not visit:
            raise HTTPException(
...
def mark_queue_entry_as_paid(
    ...
    try:
        ...
        entry = _repo(db)._db.query(OnlineQueueEntry).filter(OnlineQueueEntry.id == entry_id).first()
        if not entry:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        # 1. Через visit_id
        if entry.visit_id:
            visit = _repo(db)._db.query(Visit).filter(Visit.id == entry.visit_id).first()
    ...
    for queue_tag in unique_queue_tags:
        ...
        if queue_tag == "ecg" and not doctor_id:
            ...
            if lab_resource:
                # ✅ ИСПРАВЛЕНО: Находим Doctor по user_id для правильного specialist_id
                lab_doctor = (
                    _repo(db)._db.query(Doctor).filter(Doctor.user_id == lab_resource.id).first()
                )
                if lab_doctor:
        ...
        patient = _repo(db)._db.query(Patient).filter(Patient.id == visit.patient_id).first()
