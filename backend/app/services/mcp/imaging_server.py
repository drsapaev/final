...
class MedicalImagingMCPServer(BaseMCPServer):
    ...
    def _load_imaging_types(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "xray": {
                ...
                "typical_findings": ["переломы", "пневмония", "ателектаз", "кардиомегалия"]
            },
            "ultrasound": {
                "name": "УЗИ",
                "modalities": ["abdomen", "thyroid", "cardiac", "vascular", "obstetric"],
    ...
    def _load_analysis_templates(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "chest_xray": {
                "sections": ["легочные поля", "сердце", "средостение", "костные структуры"],
                "checklist": [
            ...
            "abdominal_ultrasound": {
                "sections": ["печень", "желчный пузырь", "поджелудочная", "селезенка", "почки"],
                "checklist": [
                    "Размеры органов",
    ...
    @MCPTool(name="analyze_medical_image", description="Анализ медицинского изображения")
    ...
    @MCPTool(name="analyze_skin_lesion", description="Специализированный анализ кожных образований")
    async def analyze_skin_lesion(
        ...
        try:
            ...
            # Рекомендации
            recommendations = self._generate_skin_recommendations(risk_assessment, patient_history)
    ...
    async def get_imaging_types(
        ...
        if category:
            if category in self.imaging_types:
                return {
                    "category": category,
                    "details": self.imaging_types[category]
                }
            else:
                return {
    ...
    @MCPResource(name="quality_criteria", description="Критерии качества медицинских изображений")
    ...
    def _structure_analysis(
        ...
        if not raw_analysis.get("error"):
            ...
            structured["recommendations"].append("Рекомендуется консультация специалиста")
    ...
    def _assess_skin_lesion_risk(
        ...
        lesion_info: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _generate_skin_recommendations(
        ...
        if patient_history:
            if patient_history.get("family_melanoma"):
                recommendations.append("⚠️ Повышенная настороженность - семейный анамнез меланомы")
