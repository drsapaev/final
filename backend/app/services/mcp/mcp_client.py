...
class MedicalMCPClient:
    ...
    async def _call_server(
        ...
        request = MCPRequest(
            method=method, params=params, id=self._generate_request_id()
        )
    ...
    async def suggest_complaint_questions(
        ...
        return await self._call_server(
            ...
            {"complaint": complaint, "specialty": specialty},
        )
    ...
    async def validate_icd10(
        ...
        return await self._call_server(
            "icd10",
            "tool/validate_icd10",
            {"code": code, "symptoms": symptoms, "diagnosis": diagnosis},
        )
    ...
    async def get_normal_ranges(
        ...
        return await self._call_server(
            ...
            {"test_name": test_name, "patient_gender": patient_gender},
        )
    ...
    async def get_test_panels(
        ...
        return await self._call_server(
            ...
            {"panel_name": panel_name, "indication": indication},
        )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed_results.append(
                    {"status": "error", "error": str(result), "request_index": i}
                )
            else:
