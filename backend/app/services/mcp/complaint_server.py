...
class MedicalComplaintMCPServer(BaseMCPServer):
    ...
    @MCPTool(
        ...
        description="Анализ жалоб пациента с созданием плана обследования",
    )
                ...
                for alt_provider in alternative_providers:
                    try:
                        ...
                        if not result.get("error"):
                            ...
                            logger.info(
                                f"✅ Fallback to {alt_provider.value} successful"
                            )
                            break
                    except Exception as e:
                        logger.error(
                            f"Fallback to {alt_provider.value} failed: {str(e)}"
                        )
                        continue
    ...
    async def validate_complaint(self, complaint: str) -> Dict[str, Any]:
        ...
        for term in rules.get("warning_terms", []):
            if term in complaint_lower:
                confidence *= 0.7
                suggestions.append(
                    f"Обнаружен тревожный симптом: {term}. Рекомендуется срочная консультация"
                )

        # Проверка на наличие контекста
        has_time_context = any(
            word in complaint_lower
            for word in ["дня", "недели", "месяца", "вчера", "сегодня"]
        )
        has_location_context = any(
            word in complaint_lower
            for word in ["области", "слева", "справа", "голова", "живот", "спина"]
        )
        ...
        return {
            ...
            "has_context": {"time": has_time_context, "location": has_location_context},
        }
