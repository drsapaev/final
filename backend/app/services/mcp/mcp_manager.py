...
class MCPManager:
            ...
            try:
                ...
                if self.client:
                    ...
                    if health_status.get("overall") != "healthy":
                        logger.warning(
                            f"MCP health check: {health_status.get('overall')}"
                        )
    ...
    async def execute_request(
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self.client._call_server(server, method, params), timeout=timeout
            )
    ...
    async def batch_execute(
        ...
        if not self.config["enabled"] or not self.client:
            return [
                {"status": "error", "error": "MCP is not available"} for _ in requests
            ]
        ...
        if parallel:
            ...
            tasks = [
                self.execute_request(
                    req.get("server"), req.get("method"), req.get("params", {})
                )
                for req in requests
            ]
    ...
    def get_server_metrics(self, server_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if server_name:
            return self.metrics["server_stats"].get(
                server_name, {"error": f"No metrics for server {server_name}"}
            )
