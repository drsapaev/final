...
class AdvancedAnalyticsService:
    ...
    def get_kpi_metrics(
        ...
        try:
            ...
            completed_appointments = db.query(Appointment).filter(
                and_(*filters, Appointment.status == "completed")
            ).count()
            
            # Отмененные записи
            cancelled_appointments = db.query(Appointment).filter(
                and_(*filters, Appointment.status == "cancelled")
            ).count()
            ...
            # Общий доход
            revenue_query = db.query(func.sum(Payment.amount)).join(Appointment).filter(
                and_(*filters, Payment.status == "completed")
            )
            ...
            if queue_wait_times:
                ...
                for ticket in queue_wait_times:
                    if ticket.served_at:
                        wait_time = (ticket.served_at - ticket.created_at).total_seconds() / 60  # минуты
            ...
            schedule_utilization = (occupied_hours / total_working_hours * 100) if total_working_hours > 0 else 0
            
            # Процент повторных визитов
            repeat_visits = db.query(Appointment).filter(
                and_(*filters, Appointment.status == "completed")
            ).join(Patient).group_by(Patient.id).having(func.count(Appointment.id) > 1).count()
            
            repeat_visit_rate = (repeat_visits / completed_appointments * 100) if completed_appointments > 0 else 0
            ...
            doctor_stats = db.query(
                ...
                func.avg(func.case(
                    (Appointment.status == "completed", 
                     func.extract('epoch', Appointment.updated_at - Appointment.created_at) / 3600),
                    else_=None
                )).label('avg_visit_duration_hours')
            ).join(Appointment, User.id == Appointment.doctor_id).filter(
                and_(*filters)
            ).group_by(User.id, User.fio, User.specialty).all()
            
            doctor_performance = []
            for stat in doctor_stats:
                completion_rate = (stat.completed_appointments / stat.total_appointments * 100) if stat.total_appointments > 0 else 0
                cancellation_rate = (stat.cancelled_appointments / stat.total_appointments * 100) if stat.total_appointments > 0 else 0
                ...
                doctor_performance.append({
                    ...
                    "completion_rate_percent": round(completion_rate, 2),
                    "cancellation_rate_percent": round(cancellation_rate, 2),
                    "avg_visit_duration_hours": round(stat.avg_visit_duration_hours or 0, 2),
                    "performance_score": completion_rate - cancellation_rate  # Простая оценка эффективности
                })
            ...
            return {
                ...
                "summary": {
                    ...
                    "avg_completion_rate": round(
                        sum(d["completion_rate_percent"] for d in doctor_performance) / len(doctor_performance), 2
                    ) if doctor_performance else 0,
                    "top_performer": doctor_performance[0] if doctor_performance else None
                }
            }
    ...
    def get_patient_analytics(
        ...
        try:
            ...
            age_groups = db.query(
                func.case(
                    (func.extract('year', func.age(Patient.birth_date)) < 18, 'under_18'),
                    (func.extract('year', func.age(Patient.birth_date)) < 30, '18_29'),
                    (func.extract('year', func.age(Patient.birth_date)) < 50, '30_49'),
                    (func.extract('year', func.age(Patient.birth_date)) < 65, '50_64'),
                    else_='65_plus'
                ).label('age_group'),
            ...
            ).group_by(EMR.icd10).order_by(desc('count')).limit(10).all()
            ...
            return {
                ...
                "patient_metrics": {
                    ...
                    "patient_retention_rate": (active_patients / new_patients * 100) if new_patients > 0 else 0
                },
                "age_distribution": {
    ...
    def get_revenue_analytics(
        ...
        try:
            ...
            # Общий доход
            total_revenue = db.query(func.sum(Payment.amount)).join(Appointment).filter(
                and_(*filters)
            ).scalar() or 0
                ...
                "payment_methods": [
                    {
                        ...
                        "avg_transaction": round(method.revenue / method.transactions, 2) if method.transactions > 0 else 0
                    }
    ...
    def get_predictive_analytics(
        ...
        try:
            ...
            return {
                ...
                "trends": {
                    "appointments": [{"date": trend.date.isoformat(), "count": trend.count} for trend in appointment_trends],
                    "revenue": [{"date": trend.date.isoformat(), "revenue": float(trend.revenue)} for trend in revenue_trends]
                },
