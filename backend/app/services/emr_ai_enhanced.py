...
class EMREnhancedAIService:
    ...
    async def generate_smart_template(
        ...
        try:
            ...
            base_template = self.specialty_templates.get(
                specialty, self.specialty_templates["general"]
            )
            ...
            return self.specialty_templates.get(
                specialty, self.specialty_templates["general"]
            )
    ...
    async def get_smart_suggestions(
        ...
        try:
            ...
            if field_name == "complaints":
                suggestions = await self._get_complaint_suggestions(
                    current_data, specialty
                )
            elif field_name == "diagnosis":
                suggestions = await self._get_diagnosis_suggestions(
                    current_data, specialty
                )
            elif field_name == "treatment":
                suggestions = await self._get_treatment_suggestions(
                    current_data, specialty
                )
            elif field_name == "medications":
                suggestions = await self._get_medication_suggestions(
                    current_data, specialty
                )
            elif field_name == "procedures":
                suggestions = await self._get_procedure_suggestions(
                    current_data, specialty
                )
            ...
            for field in required_fields:
                if not emr_data.get(field):
                    validation_result["errors"].append(
                        f"Обязательное поле '{field}' не заполнено"
                    )
    ...
    async def generate_icd10_suggestions(
        ...
        try:
            ...
            if (
                "сердце" in diagnosis_text.lower()
                or "cardiac" in diagnosis_text.lower()
            ):
                suggestions.extend(
                    [
                        {
                            "code": "I25.9",
                            "description": "Хроническая ишемическая болезнь сердца неуточненная",
                            "confidence": 0.9,
                        },
                        {
                            "code": "I21.9",
                            "description": "Острый инфаркт миокарда неуточненный",
                            "confidence": 0.8,
                        },
                    ]
                )
                ...
                suggestions.extend(
                    [
                        {
                            "code": "L30.9",
                            "description": "Дерматит неуточненный",
                            "confidence": 0.85,
                        },
                        {
                            "code": "L70.9",
                            "description": "Угри неуточненные",
                            "confidence": 0.8,
                        },
                    ]
                )
                ...
                suggestions.extend(
                    [
                        {
                            "code": "K02.9",
                            "description": "Кариес зубов неуточненный",
                            "confidence": 0.9,
                        },
                        {
                            "code": "K05.0",
                            "description": "Острый гингивит",
                            "confidence": 0.8,
                        },
                    ]
                )
    ...
    def _get_dermatology_template(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "treatment": {"type": "textarea", "label": "Лечение", "required": True},
            "follow_up": {
                "type": "date",
    ...
    def _get_dentistry_template(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "diagnosis": {"type": "textarea", "label": "Диагноз", "required": True},
            "treatment_plan": {
                "type": "textarea",
                "label": "План лечения",
                "required": True,
    ...
    def _get_general_template(self) -> Dict[str, Any]:
        ...
        return {
            "complaints": {"type": "textarea", "label": "Жалобы", "required": True},
            "anamnesis": {"type": "textarea", "label": "Анамнез", "required": True},
            "examination": {
                "type": "textarea",
                "label": "Объективное обследование",
                "required": True,
    ...
    async def _check_logic_consistency(
        ...
        if "стенокардия" in diagnosis and "боль" not in complaints:
            warnings.append(
                "Диагноз 'стенокардия' обычно сопровождается болевыми ощущениями"
            )
    ...
    async def _generate_improvement_suggestions(
        ...
        if not emr_data.get("icd10") and specialty in ["cardiology", "dermatology"]:
            suggestions.append(
                "Рекомендуется указать код МКБ-10 для лучшей классификации"
            )
