...
class ConfirmationSecurityService:
    ...
    RATE_LIMITS = {
        'confirmation_attempt': RateLimitConfig(max_attempts=5, window_minutes=15, cooldown_minutes=30),
        'token_generation': RateLimitConfig(max_attempts=3, window_minutes=60, cooldown_minutes=60),
        'failed_confirmation': RateLimitConfig(max_attempts=10, window_minutes=60, cooldown_minutes=120),
    }
    ...
    def validate_confirmation_request(
        ...
        try:
            ...
            if visit.status not in ["pending_confirmation"]:
                return SecurityCheckResult(
                    allowed=False,
                    reason=f"Визит уже имеет статус: {visit.status}"
                )
            
            # 4. Проверяем rate limiting по IP
            if source_ip:
                ip_check = self._check_rate_limit_by_ip(source_ip, "confirmation_attempt")
                if not ip_check.allowed:
                    self._log_security_event("rate_limit_ip", {
                        "source_ip": source_ip,
                        "visit_id": visit.id,
                        "channel": channel,
            ...
            patient_check = self._check_rate_limit_by_patient(visit.patient_id, "confirmation_attempt")
            if not patient_check.allowed:
                self._log_security_event("rate_limit_patient", {
            ...
            suspicious_check = self._check_suspicious_activity(visit, source_ip, user_agent, channel)
            if not suspicious_check.allowed:
                return suspicious_check
            
            # Все проверки пройдены
            self._log_security_event("validation_passed", {
                "visit_id": visit.id,
                "patient_id": visit.patient_id,
                "source_ip": source_ip,
                "channel": channel
            ...
            return SecurityCheckResult(
                allowed=True,
                reason="Запрос прошел все проверки безопасности"
            )
            ...
            return SecurityCheckResult(
                allowed=False,
                reason="Внутренняя ошибка системы безопасности"
            )
    ...
    def validate_token_generation_request(
        ...
        try:
            ...
            patient_check = self._check_rate_limit_by_patient(patient_id, "token_generation")
            if not patient_check.allowed:
                self._log_security_event("rate_limit_token_generation", {
            ...
            if source_ip:
                ...
                if not ip_check.allowed:
                    self._log_security_event("rate_limit_token_generation_ip", {
                        "patient_id": patient_id,
                        "source_ip": source_ip,
                        "user_id": user_id,
                        "retry_after": ip_check.retry_after
            ...
            return SecurityCheckResult(
                allowed=True,
                reason="Генерация токена разрешена"
            )
    ...
    def record_confirmation_attempt(
        ...
        try:
            ...
            if source_ip:
                self._update_rate_limit_counter(f"ip:{source_ip}", "confirmation_attempt")
            
            visit = self.db.query(Visit).filter(Visit.id == visit_id).first()
            if visit:
                self._update_rate_limit_counter(f"patient:{visit.patient_id}", "confirmation_attempt")
                
                if not success:
                    self._update_rate_limit_counter(f"patient:{visit.patient_id}", "failed_confirmation")
                    if source_ip:
                        self._update_rate_limit_counter(f"ip:{source_ip}", "failed_confirmation")
    ...
    def cleanup_expired_tokens(self) -> int:
        ...
        try:
            ...
            expired_visits = self.db.query(Visit).filter(
                and_(
            ...
            for visit in expired_visits:
                ...
                self._log_security_event("token_expired_cleanup", {
                    "visit_id": visit.id,
                    "patient_id": visit.patient_id,
                    "original_expires_at": visit.confirmation_expires_at.isoformat() if visit.confirmation_expires_at else None
                })
    ...
    def _check_rate_limit_by_patient(self, patient_id: int, action: str) -> SecurityCheckResult:
        ...
        try:
            ...
            if user_agent:
                ...
                if any(agent in user_agent.lower() for agent in suspicious_agents):
                    ...
                    return SecurityCheckResult(
                        allowed=False,
                        reason="Подозрительный User-Agent"
                    )
            
            # 3. Проверяем временные аномалии
            if visit.created_at:
                time_since_creation = datetime.utcnow() - visit.created_at
                if time_since_creation.total_seconds() < 60:  # Слишком быстро после создания
                    self._log_security_event("too_fast_confirmation", {
                        "visit_id": visit.id,
                    ...
                    return SecurityCheckResult(
                        allowed=False,
                        reason="Слишком быстрая попытка подтверждения"
                    )
            
            return SecurityCheckResult(allowed=True, reason="Подозрительная активность не обнаружена")
            
        except Exception as e:
            logger.error(f"Ошибка проверки подозрительной активности: {e}")
            return SecurityCheckResult(allowed=True, reason="Ошибка проверки, разрешаем")
