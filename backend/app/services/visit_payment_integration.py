...
class VisitPaymentIntegrationService:
    ...
    def create_visit_from_payment(
        ...
        try:
            ...
            visit_data = {
                ...
                "notes": notes or f"Визит создан автоматически после оплаты через {webhook.provider}",
                "payment_status": "paid",
                "payment_amount": float(webhook.amount) / 100,  # Конвертируем из тийинов
            ...
            # Выполняем вставку
            result = db.execute(
                visits_table.insert().values(**visit_data)
            )
            db.commit()
    ...
    def update_visit_payment_status(
        ...
        try:
            ...
            return True, f"Статус платежа визита {visit_id} обновлён на '{payment_status}'"
    ...
    @staticmethod
    def get_visit_payment_info(db: Session, visit_id: int) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        """
    ...
    def get_visits_by_payment_status(
        ...
        try:
            ...
            query = select(visits_table).where(
                visits_table.c.payment_status == payment_status
            ).order_by(visits_table.c.id.desc()).limit(limit).offset(offset)
            ...
            return True, f"Найдено {len(visits)} визитов со статусом '{payment_status}'", visits
