...
class FileSystemService:
    ...
    def upload_file(
        ...
        try:
            ...
            file_quota.update_usage(
                db, user_id=user_id, size_delta=file_size, files_delta=1
            )

            # Логируем загрузку
            file_access_log.create(
                db, file_id=db_file.id, user_id=user_id, action="upload"
            )
    ...
    def download_file(
        ...
        if user_id:
            file_access_log.create(
                db, file_id=file_id, user_id=user_id, action="download"
            )
    ...
    def delete_file(self, db: Session, file_id: int, user_id: int) -> bool:
        ...
        if result:
            ...
            # Логируем удаление
            file_access_log.create(
                db, file_id=file_id, user_id=user_id, action="delete"
            )

        return result is not None

    def create_file_share(
        self, db: Session, file_id: int, share_data: dict, user_id: int
    ) -> FileShare:
        ...
        return file_share.create(
            db, file_id=file_id, share_data=share_create, created_by=user_id
        )

    def export_files(
        self, db: Session, export_request: FileExportRequest, user_id: int
    ) -> str:
        ...
        archive_path = os.path.join(
            temp_dir,
            f"export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{export_request.format}",
        )
