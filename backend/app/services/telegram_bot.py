...
class TelegramBotService:
    ...
    async def _handle_text_message(self, text: str, chat_id: int, telegram_user, db: Session):
        ...
        try:
            ...
            if any(word in text.lower() for word in ["время", "работа", "часы", "открыт"]):
                await self._send_working_hours(chat_id)
            elif any(word in text.lower() for word in ["адрес", "где", "найти", "расположение"]):
                await self._send_clinic_address(chat_id)
            elif any(word in text.lower() for word in ["телефон", "звонок", "связаться"]):
    ...
    async def _send_appointments_list(self, chat_id: int, telegram_user, db: Session):
        ...
        try:
            if not telegram_user.patient_id:
                await self._send_message(chat_id, "❌ Ваш профиль не привязан к пациенту. Обратитесь к администратору.")
                return
            ...
            await self._send_message(chat_id, "❌ Ошибка получения записей. Попробуйте позже.")
