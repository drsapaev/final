...
@dataclass
class DermaDomainError(Exception):
    status_code: int
    detail: str
...
class DermaService:
    ...
    def create_price_override(self, *, override_data, user_id: int):
        ...
        if not visit:
            raise DermaDomainError(404, "Визит не найден")

        service = self.repository.get_service(override_data.service_id)
        if not service:
            raise DermaDomainError(404, "Услуга не найдена")
        if not service.allow_doctor_price_override:
            raise DermaDomainError(
                400,
        ...
        if not doctor:
            raise DermaDomainError(404, "Врач не найден")
