...
class FeatureFlagService:
    ...
    def delete_flag(
        self, flag_key: str, user_id: str = None, reason: str = None
    ) -> bool:
        """
        Удаляет фича-флаг
    ...
    def _get_flag_from_cache_or_db(self, flag_key: str) -> Optional[FeatureFlag]:
        ...
        if (
            ...
            and (now - self._cache_timestamp).total_seconds() < self.cache_ttl
            and flag_key in self._cache
        ):
        ...
        # Обновляем кэш
        if (
            not self._cache_timestamp
            or (now - self._cache_timestamp).total_seconds() >= self.cache_ttl
        ):
            self._refresh_cache()
        else:
...
PREDEFINED_FLAGS = [
    ...
    },
    {
        ...
        "config": {"require_phone_verification": True, "session_timeout_minutes": 30},
    },
    {
        "key": "rate_limiting",
...
def initialize_feature_flags(db: Session):
    ...
    for flag_data in PREDEFINED_FLAGS:
        existing = (
            db.query(FeatureFlag).filter(FeatureFlag.key == flag_data["key"]).first()
        )
