...
class WebhookService:
    ...
    def get_webhooks(
        ...
        return (
            query.order_by(desc(Webhook.created_at)).offset(offset).limit(limit).all()
        )
    ...
    async def trigger_event(
        ...
        try:
            ...
            logger.info(
                f"Обработано событие {event_type.value} для {len(tasks)} webhook'ов"
            )
    ...
    async def _schedule_retry(self, call: WebhookCall):
        ...
        try:
            ...
            logger.info(
                f"Запланирован повтор webhook {webhook.id} (попытка {retry_call.attempt_number})"
            )
    ...
    def cleanup_old_calls(self, days: int = 30):
        ...
        try:
            ...
            deleted_count = (
                self.db.query(WebhookCall)
                .filter(WebhookCall.created_at < cutoff_date)
                .delete()
            )
    ...
    def get_system_webhook_stats(self) -> Dict[str, Any]:
        ...
        active_webhooks = (
            self.db.query(Webhook)
            .filter(
                and_(Webhook.is_active == True, Webhook.status == WebhookStatus.ACTIVE)
            )
            .count()
        )
        ...
        unprocessed_events = (
            self.db.query(WebhookEvent).filter(WebhookEvent.processed == False).count()
        )
        ...
        return {
            ...
            "recent_24h": {
                ...
                "success_rate": (
                    (recent_success / recent_calls * 100) if recent_calls > 0 else 0
                ),
            },
