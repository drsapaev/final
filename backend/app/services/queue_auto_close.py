...
class QueueAutoCloseService:
    ...
    def check_and_close_expired_queues(self) -> Dict[str, Any]:
        ...
        queues_to_close = self.db.query(DailyQueue).filter(
            and_(
        ...
        for queue in queues_to_close:
            try:
                ...
                # Логируем закрытие
                logger.info(f"Auto-closed queue {queue.id} for specialist {queue.specialist_id} at {current_time}")
    ...
    def get_queues_pending_close(self) -> List[Dict[str, Any]]:
        ...
        pending_queues = self.db.query(DailyQueue).filter(
            and_(
                DailyQueue.day == today,
                DailyQueue.active == True,
        ...
        for queue in pending_queues:
            result.append({
                ...
                "entries_count": self._count_queue_entries(queue.id),
                "minutes_remaining": self._calculate_minutes_remaining(current_time, queue.online_end_time)
            })
...
def run_auto_close_check():
    ...
    try:
        ...
        if result["closed_count"] > 0:
            print(f"✅ Auto-closed {result['closed_count']} queues at {result['check_time']}")
        else:
