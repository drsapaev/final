...
def _service_display_code(service) -> str | None:
    if not service:
        return None
    return service.service_code or f"service:{service.id}"


@dataclass
class DentalDomainError(Exception):
    status_code: int
    detail: str
...
class DentalService:
    ...
    async def create_dental_price_override(self, *, override_data, user: User) -> DoctorPriceOverride:
        ...
        if not visit:
            raise DentalDomainError(404, "Визит не найден")

        service = self.repository.get_service(override_data.service_id)
        if not service:
            raise DentalDomainError(404, "Услуга не найдена")
        if not service.allow_doctor_price_override:
            raise DentalDomainError(
                400,
        ...
        if not doctor:
            raise DentalDomainError(404, "Врач не найден")
    ...
    async def send_price_override_notification(
        ...
        *,
        ...
        message = (
            ...
            f"Врач: {doctor_name}\n"
            f"{patient_info}\n"
            f"Услуга: {service.name} ({_service_display_code(service)})\n"
    ...
    async def approve_price_override(
        ...
        *,
        ...
        if not price_override:
            raise DentalDomainError(404, "Изменение цены не найдено")
