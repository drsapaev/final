...
from app.models.dynamic_pricing import (
    PricingRule, PricingRuleService, ServicePackage, PackageService,
    PackagePurchase, DynamicPrice, PriceHistory,
    PricingRuleType, DiscountType
)
...
class DynamicPricingService:
    ...
    def apply_pricing_rules(
        ...
        return {
            ...
            'services_with_prices': self._update_service_prices(services, applied_discounts)
        }
    ...
    def _update_service_prices(
        ...
        for service in services:
            ...
            if original_total > 0:
                ...
                discounted_price = service.get('price', 0) - (service_discount / service.get('quantity', 1))
            else:
                discounted_price = service.get('price', 0)
            
            updated_service = service.copy()
            updated_service['discounted_price'] = max(0, discounted_price)
            updated_service['discount_amount'] = service.get('price', 0) - discounted_price
    ...
    def get_available_packages(
        ...
        query = self.db.query(ServicePackage).filter(
        ...
        query = query.filter(
            or_(
                ServicePackage.valid_from.is_(None),
                ServicePackage.valid_from <= now
            ),
            or_(
                ServicePackage.valid_to.is_(None),
                ServicePackage.valid_to >= now
            )
        )
        ...
        if patient_id:
            ...
            for package in packages:
                if package.per_patient_limit:
                    patient_purchases = self.db.query(PackagePurchase).filter(
    ...
    def purchase_package(
        ...
        """Купить пакет услуг"""
        package = self.db.query(ServicePackage).filter(
    ...
    def update_dynamic_prices(self) -> Dict[str, Any]:
        ...
        for dp in dynamic_prices:
            ...
            new_price = dp.base_price * demand_factor * time_factor * capacity_factor * seasonal_factor
    ...
    def _calculate_demand_factor(self, service_id: int) -> float:
        ...
        recent_bookings = self.db.query(func.count(Visit.id)).join(
    ...
    def _calculate_capacity_factor(self, service_id: int) -> float:
        ...
        booked_slots = self.db.query(func.count(Visit.id)).filter(
            Visit.visit_date >= today,
            Visit.visit_date <= next_week
        ).scalar() or 0
    ...
    def get_pricing_analytics(
        ...
        rules_stats = self.db.query(
            PricingRule.name,
            PricingRule.current_uses,
            func.sum(PriceHistory.old_price - PriceHistory.new_price).label('total_savings')
        ).outerjoin(
            PriceHistory,
            and_(
            ...
            PricingRule.is_active == True
        ).group_by(PricingRule.id, PricingRule.name).all()
        
        # Статистика по пакетам
        packages_stats = self.db.query(
            ServicePackage.name,
            ...
            ServicePackage.is_active == True
        ).group_by(ServicePackage.id, ServicePackage.name).all()
