...
class DoctorInfoService:
    ...
    def get_doctor_full_info(self, doctor_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "department": department_info["name"] if department_info else "Общее отделение",
                "specialty": doctor.specialty,
                "cabinet": {
                    "number": doctor.cabinet,
                    "floor": None,
                    "building": None
                },
    ...
    def get_doctors_by_specialization(self, specialization: str) -> List[Dict[str, Any]]:
        ...
        try:
            doctors = self.db.query(Doctor).filter(
                and_(Doctor.specialty.ilike(f"%{specialization}%"), Doctor.active == True)
            ).all()
            
            return [self.get_doctor_full_info(doctor.id) for doctor in doctors]
        except Exception as e:
            logger.error(f"Ошибка получения врачей по специализации {specialization}: {e}")
            return []
    ...
    def get_all_departments(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            specialties = self.db.query(Doctor.specialty).filter(Doctor.active == True).distinct().all()
            departments = []
    ...
    def get_appointment_doctor_info(self, appointment_id: int, appointment_type: str = "appointment") -> Dict[str, Any]:
        """Получает информацию о враче для записи"""
        try:
            if appointment_type == "visit":
                appointment = self.db.query(Visit).filter(Visit.id == appointment_id).first()
            else:
            ...
            department_info = self.get_department_info(doctor_info["specialty"]) if doctor_info["specialty"] else None
    ...
    def format_department_info_for_notification(self, department_info: Dict[str, Any]) -> str:
