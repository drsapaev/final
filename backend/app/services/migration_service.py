...
class MigrationService:
        ...
        try:
            ...
            for ticket in old_tickets:
                try:
                    ...
                    specialist_id = self._get_specialist_for_department(
                        ticket.department
                    )
    ...
    def _get_or_create_daily_queue(
        self, day: date, specialist_id: int, queue_tag: str
    ) -> DailyQueue:
        ...
        if not daily_queue:
            daily_queue = DailyQueue(
                day=day, specialist_id=specialist_id, queue_tag=queue_tag, active=True
            )
    ...
    def _check_data_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            duplicate_numbers = self.db.execute(
                text(
                    """
            ...
            orphaned_visits = self.db.execute(
                text(
                    """
            ...
            orphaned_patients = self.db.execute(
                text(
                    """
                SELECT COUNT(*) as count
                FROM queue_entries qe
            ...
            orphaned_queues = self.db.execute(
                text(
                    """
                SELECT COUNT(*) as count
                FROM queue_entries qe
            ...
            for queue in daily_queues:
                ...
                for entry in queue.entries:
                    entry_data = {
                        ...
                        "print_data": (
                            entry.print_data if hasattr(entry, 'print_data') else None
                        ),
                        "printed_at": (
                            entry.printed_at.isoformat()
                            if hasattr(entry, 'printed_at') and entry.printed_at
                            else None
                        ),
                    }
    ...
    def restore_queue_data(self, backup_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for queue_data in backup_data["queues"]:
                ...
                existing_queue = (
                    self.db.query(DailyQueue)
                    .filter(DailyQueue.id == queue_data["id"])
                    .first()
                )

                if existing_queue:
                    logger.warning(
                        f"Очередь {queue_data['id']} уже существует, пропускаем"
                    )
                    continue
                ...
                queue = DailyQueue(
                    ...
                    opened_at=(
                        datetime.fromisoformat(queue_data["opened_at"])
                        if queue_data["opened_at"]
                        else None
                    ),
                )
                self.db.add(queue)
                ...
                for entry_data in queue_data["entries"]:
                    entry = OnlineQueueEntry(
                        ...
                        called_at=(
                            datetime.fromisoformat(entry_data["called_at"])
                            if entry_data["called_at"]
                            else None
                        ),
                    )
                    ...
                    if (
                        "printed_at" in entry_data
                        and entry_data["printed_at"]
                        and hasattr(entry, 'printed_at')
                    ):
                        entry.printed_at = datetime.fromisoformat(
                            entry_data["printed_at"]
                        )
