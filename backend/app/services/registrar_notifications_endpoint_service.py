...
@dataclass
class RegistrarNotificationsDomainError(Exception):
    status_code: int
    detail: str


class RegistrarNotificationsEndpointService:
    ...
    def get_appointment_context(
        ...
        *,
        ...
        if not appointment:
            raise RegistrarNotificationsDomainError(404, "Запись не найдена")

        patient = self.repository.get_patient(appointment.patient_id)
        if not patient:
            raise RegistrarNotificationsDomainError(404, "Пациент не найден")
    ...
    def get_price_change_context(self, *, price_override_id: int):
        ...
        if not price_override:
            raise RegistrarNotificationsDomainError(404, "Изменение цены не найдено")

        doctor = self.repository.get_doctor(price_override.doctor_id)
        if not doctor:
            raise RegistrarNotificationsDomainError(404, "Врач не найден")

        service_obj = self.repository.get_service(price_override.service_id)
        if not service_obj:
            raise RegistrarNotificationsDomainError(404, "Услуга не найдена")
    ...
    def get_queue_entry(self, *, queue_entry_id: int):
        ...
        if not queue_entry:
            raise RegistrarNotificationsDomainError(
                404,
