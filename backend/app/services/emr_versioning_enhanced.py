...
class EMRVersioningEnhancedService:
    ...
    async def get_version_comparison(
        ...
        try:
            ...
            # Сравниваем данные
            comparison = await self._compare_versions(version1.data, version2.data)
    ...
    async def restore_version_with_backup(
        ...
        try:
            ...
            restored_version = emr_version.create_version(
                ...
                or f"Восстановление версии {target_version.version_number}",
                changed_by=restored_by,
            )
        ...
        try:
            ...
            for version in versions:
                ...
                if version.changed_by:
                    changes_by_user[version.changed_by] = (
                        changes_by_user.get(version.changed_by, 0) + 1
                    )
            ...
            return {
                ...
                "most_active_user": (
                    max(changes_by_user.items(), key=lambda x: x[1])[0]
                    if changes_by_user
                    else None
                ),
            }
    ...
    async def _analyze_changes(
        ...
        for key in all_keys:
            ...
            if key not in old_data:
                changes["added_fields"].append({"field": key, "new_value": new_value})
            elif key not in new_data:
                changes["removed_fields"].append({"field": key, "old_value": old_value})
        ...
        for key in all_keys:
            ...
            if key not in data1:
                ...
                comparison["changes"].append(
                    {"field": key, "type": "added", "value": value2}
                )
            elif key not in data2:
                comparison["fields_removed"] += 1
                comparison["changes"].append(
                    {"field": key, "type": "removed", "value": value1}
                )
            elif value1 != value2:
