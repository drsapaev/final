...
class QRQueueService:
    ...
    def generate_qr_token(
        ...
        if target_date:
            ...
            logger.debug(f"[QRQueueService] Используется явно указанная дата: {target_date}")
        else:
            # Если после 09:00 - создаем на завтра, иначе на сегодня
            if current_time > "09:00":
                target_date = today + timedelta(days=1)
                logger.debug(f"[QRQueueService] Текущее время {current_time} > 09:00, создаем QR на завтра: {target_date}")
            else:
                target_date = today
                logger.debug(f"[QRQueueService] Текущее время {current_time} <= 09:00, создаем QR на сегодня: {target_date}")
        ...
        return {
            ...
            "specialist_id": specialist_id,
            "department": department,
            "expires_at": token_meta["expires_at"].isoformat() if token_meta.get("expires_at") else None,
            "active": True
        }
    ...
    def get_qr_token_info(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"[QRQueueService.get_qr_token_info] Запрос информации о токене: {token[:20]}...")
            ...
            logger.debug(f"[QRQueueService.get_qr_token_info] Текущее время (timezone-aware): {now}")

            # Сначала проверим, существует ли токен вообще
            qr_token_any = self.db.query(QueueToken).filter(
            ...
            if qr_token_any:
                ...
                if qr_token_any.expires_at:
                    ...
                    expires_aware = qr_token_any.expires_at if qr_token_any.expires_at.tzinfo else qr_token_any.expires_at.replace(tzinfo=timezone)
                    logger.debug(f"  - истек: {expires_aware <= now}")
            else:
                logger.debug(f"[QRQueueService.get_qr_token_info] Токен вообще не найден в БД!")
            ...
            if not qr_token:
                logger.debug(f"[QRQueueService.get_qr_token_info] Токен не найден или неактивен")
                return None
            ...
            if qr_token.expires_at:
                ...
                if expires_aware <= now:
                    logger.debug(f"[QRQueueService.get_qr_token_info] Токен истек: {expires_aware} <= {now}")
                    return None
                else:
                    logger.debug(f"[QRQueueService.get_qr_token_info] Токен действителен: {expires_aware} > {now}")

            logger.debug(f"[QRQueueService.get_qr_token_info] Токен найден: specialist_id={qr_token.specialist_id}, is_clinic_wide={qr_token.is_clinic_wide}")
            ...
            if qr_token.specialist_id is not None:
                ...
                logger.debug(f"[QRQueueService.get_qr_token_info] Целевая дата: {target_date}")
            ...
            logger.debug(f"[QRQueueService.get_qr_token_info] DailyQueue найдена: {daily_queue is not None}")
        except Exception as e:
            logger.error(f"[QRQueueService.get_qr_token_info] КРИТИЧЕСКАЯ ОШИБКА в начальной части: {e}")
            import traceback
            traceback.print_exc()
            return None
        ...
        try:
            if qr_token.is_clinic_wide or qr_token.specialist_id is None:
                ...
                try:
                    if hasattr(specialist, 'user') and specialist.user:
                        specialist_name = specialist.user.full_name or f"Врач ID {qr_token.specialist_id}"
                    elif hasattr(specialist, 'user_id') and specialist.user_id:
                        # Пытаемся загрузить user явно
                        user = self.db.query(User).filter(User.id == specialist.user_id).first()
                    ...
                    if not specialist_name or (isinstance(specialist_name, str) and specialist_name.strip() == ""):
                        specialist_name = f"Врач ID {qr_token.specialist_id}"

                    logger.debug(f"[QRQueueService] Имя врача получено: {specialist_name}")
                except Exception as e:
                ...
                try:
                    if hasattr(specialist, 'specialty') and specialist.specialty:
                        specialty_label = specialty_mapping.get(specialist.specialty, specialist.specialty)
                    else:
                        # Если нет специальности у врача, используем department из токена
                        specialty_label = specialty_mapping.get(qr_token.department, self._get_department_name(qr_token.department))
                except Exception as e:
                    logger.debug(f"[QRQueueService] Ошибка получения специальности: {e}")
                    specialty_label = specialty_mapping.get(qr_token.department, self._get_department_name(qr_token.department))
                ...
                try:
                    ...
                    visit_count = self.db.query(Visit).filter(
                    ...
                    if appointment_count is None:
                        appointment_count = 0
                    logger.debug(f"[QRQueueService] appointment_count: {appointment_count}")
            ...
            logger.error(f"[QRQueueService] Критическая ошибка обработки специалиста: {e}")
            import traceback
            traceback.print_exc()
            specialty_label = specialty_mapping.get(qr_token.department, self._get_department_name(qr_token.department))
        ...
        try:
            ...
            if not qr_token.is_clinic_wide and qr_token.specialist_id is not None:
                queue_active = daily_queue is not None and daily_queue.active if daily_queue else False

            # Определяем department_name
            final_department_name = department_name if qr_token.is_clinic_wide or qr_token.specialist_id is None else specialty_mapping.get(specialty_label, specialty_label)
            ...
            logger.debug(f"[QRQueueService.get_qr_token_info] Ответ сформирован успешно: {result}")
            return result
        except Exception as e:
            logger.error(f"[QRQueueService.get_qr_token_info] ОШИБКА при формировании ответа: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def start_join_session(
        ...
        Returns:
            Данные сессии
        """
        logger.debug(f"[QRQueueService.start_join_session] Начало сессии для токена: {token[:20]}...")
        ...
        try:
            ...
            logger.debug(f"[QRQueueService.start_join_session] Токен найден: queue_active={token_info.get('queue_active')}")

            # Для общего QR токена разрешаем создание сессии даже если queue_active=False
            # (так как очереди могут быть еще не созданы)
            if not token_info.get("is_clinic_wide") and not token_info.get("queue_active", False):
                error_msg = "Очередь в данный момент не активна"
                logger.debug(f"[QRQueueService.start_join_session] ❌ {error_msg}")
                raise ValueError(error_msg)

            # Проверяем временные ограничения
            logger.debug(f"[QRQueueService.start_join_session] Проверка временных ограничений...")
            time_check = self._check_online_time_restrictions(token)
            logger.debug(f"[QRQueueService.start_join_session] Результат проверки времени: allowed={time_check.get('allowed')}, message={time_check.get('message')}")

            if not time_check.get("allowed", False):
                error_msg = time_check.get("message", "Временные ограничения не пройдены")
                logger.debug(f"[QRQueueService.start_join_session] ❌ {error_msg}")
                raise ValueError(error_msg)
    ...
    def complete_join_session_multiple(
        ...
        qr_token = (
            self.db.query(QueueToken).filter(QueueToken.token == session.qr_token).first()
        )
        if not qr_token:
            raise ValueError("QR токен не найден")
    ...
    def call_next_patient(self, specialist_id: int, called_by_user_id: int, target_date: Optional[date] = None) -> Dict[str, Any]:
        """
    ...
    def _get_frontend_url(self) -> str:
        ...
        if configured_url and configured_url not in [
            ...
            logger.debug(f"[QRQueueService] Используется FRONTEND_URL из настроек: {configured_url}")
        ...
        try:
            ...
            logger.debug(f"[QRQueueService] Автоматически определен локальный IP: {frontend_url}")
            return frontend_url
        except Exception as e:
            # Если не удалось определить IP, используем fallback из настроек
            logger.debug(f"[QRQueueService] Ошибка определения IP: {e}, используем FRONTEND_URL из настроек: {configured_url}")
    ...
    def _get_queue_length(self, queue_id: int) -> int:
        ...
        try:
            ...
            online_count = self.db.query(OnlineQueueEntry).filter(
            ...
            logger.debug(f"[_get_queue_length] queue_id={queue_id}, date={target_date}, specialist={specialist_id}")
            logger.debug(f"[_get_queue_length] visit={visit_count}, appointment={appointment_count}, online={online_count}, total={total}")
    ...
    def _estimate_wait_time(self, queue_id: int, queue_number: int) -> int:
        ...
        try:
            ...
            logger.debug(f"[_estimate_wait_time] queue_number={queue_number}, total_queue={total_queue_length}, waiting_before={waiting_before}")
    ...
    def _update_queue_statistics(self, queue_id: int, stat_field: str):
        ...
        if current_value is None:
            current_value = 0
        logger.debug(f"[_update_queue_statistics] field={stat_field}, current={current_value}, new={current_value + 1}")
    ...
    def _check_online_time_restrictions(self, token: str) -> Dict[str, Any]:
        ...
        if qr_token.is_clinic_wide or qr_token.specialist_id is None:
            ...
            if not daily_queue:
                ...
                if target_date < today:
                    logger.debug(f"[_check_online_time_restrictions] ❌ Дата {target_date} в прошлом")
                    return {"allowed": False, "message": f"Нельзя записаться на прошедшую дату ({target_date.strftime('%d.%m.%Y')})"}
                ...
                if target_date == today:
                    ...
                    if current_time < start_time:
                        logger.debug(f"[_check_online_time_restrictions] ❌ Время еще не наступило: {current_time.strftime('%H:%M')} < {start_time.strftime('%H:%M')}")
                        return {
                            "allowed": False,
                ...
                logger.debug(f"[_check_online_time_restrictions] ⚠️ Нет активных очередей на {target_date}, но дата валидна - разрешаем запись")
                return {
                    "allowed": True,
            ...
            any_opened = self.db.query(DailyQueue).filter(
                DailyQueue.day == target_date,
                DailyQueue.active == True,
            ...
            if not daily_queue:
                logger.debug(f"[_check_online_time_restrictions] ❌ DailyQueue НЕ НАЙДЕНА!")
                # Дополнительная диагностика
                all_queues = self.db.query(DailyQueue).filter(
                    DailyQueue.specialist_id == qr_token.specialist_id
                ).all()
                logger.debug(f"[_check_online_time_restrictions] Все очереди для specialist_id={qr_token.specialist_id}:")
                for q in all_queues:
        ...
        if target_date > today:
            ...
            return {
                ...
                "message": f"Запись на {target_date.strftime('%d.%m.%Y')} доступна",
                "status": "available",
                "start_time": getattr(daily_queue, 'online_start_time', '07:00') if daily_queue else '07:00',
                "end_time": getattr(daily_queue, 'online_end_time', '09:00') if daily_queue else '09:00',
        ...
        if current_time_obj < start_time_obj:
            ...
            start_datetime = now.replace(hour=start_time_obj.hour, minute=start_time_obj.minute, second=0, microsecond=0)
        ...
        end_time_str = getattr(daily_queue, 'online_end_time', '09:00') if daily_queue else '09:00'
        if end_time_str:
            # Парсим строку времени в объект time
            end_hour, end_minute = map(int, end_time_str.split(':'))
            end_time_obj = datetime.now().replace(hour=end_hour, minute=end_minute, second=0, microsecond=0).time()
        ...
        if qr_token.is_clinic_wide or qr_token.specialist_id is None:
            ...
            max_entries = getattr(daily_queue, 'max_online_entries', 15) if daily_queue else 15
            # Подсчитываем общее количество онлайн записей на эту дату
            all_queues_ids = [q.id for q in self.db.query(DailyQueue).filter(
                DailyQueue.day == target_date,
                DailyQueue.active == True
            ).all()]
            current_entries = self.db.query(OnlineQueueEntry).filter(
                ...
                OnlineQueueEntry.source == "online",
                OnlineQueueEntry.status.in_(["waiting", "called"])
            ).count() if all_queues_ids else 0
        else:
        ...
        start_time_str = getattr(daily_queue, 'online_start_time', '07:00') if daily_queue else '07:00'
        end_time_str = getattr(daily_queue, 'online_end_time', '09:00') if daily_queue else '09:00'
