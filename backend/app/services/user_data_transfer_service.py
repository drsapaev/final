...
class UserDataTransferService:
                    ...
                    summary["transferable_data"].append(
                        {
                            ...
                            "services_count": (
                                len(visit.services) if visit.services else 0
                            ),
                            "total_amount": (
                    ...
                    summary["transferable_data"].append(
                        {
                            ...
                            "status": entry.status,
                            "queue_date": (
                                entry.queue.day.isoformat() if entry.queue else None
                            ),
                            "specialist_name": (
    ...
    def transfer_user_data(
        ...
        try:
            ...
            # Получаем пациентов
            source_patient = crud_patient.get_patient_by_user_id(
                db, user_id=source_user_id
            )
            target_patient = crud_patient.get_patient_by_user_id(
            ...
            if not source_patient:
                return {
                    "success": False,
                    "error": "Профиль пациента-источника не найден",
                }

            if not target_patient:
                # Создаем профиль пациента для получателя
                target_patient = crud_patient.create_patient_from_user(
                    db, user=target_user
                )
            ...
            if 'appointments' in data_types:
                result = self._transfer_appointments(
                    db, source_patient.id, target_patient.id
                )
            ...
            if 'queue_entries' in data_types:
                result = self._transfer_queue_entries(
                    db, source_patient.id, target_patient.id
                )
    ...
    def _transfer_appointments(
        ...
        try:
            appointments = (
                db.query(Appointment)
                .filter(Appointment.patient_id == source_patient_id)
                .all()
            )
            ...
            return {"count": transferred_count, "success": True}
    ...
    def _transfer_queue_entries(
        ...
        try:
            queue_entries = (
                db.query(OnlineQueueEntry)
                .filter(OnlineQueueEntry.patient_id == source_patient_id)
                .all()
            )
        ...
        try:
            ...
            if requester_user_id not in [source_user_id, target_user_id]:
                ...
                if not any(
                    ...
                    for role in requester_user.roles
                ):
