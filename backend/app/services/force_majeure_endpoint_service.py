...
@dataclass
class ForceMajeureDomainError(Exception):
    status_code: int
    detail: str


class ForceMajeureEndpointService:
    ...
    def __init__(
        ...
        force_majeure_service: Any | None = None,
    ):
        self.repository = repository or ForceMajeureApiRepository(db)
        self.force_majeure_service = force_majeure_service or get_force_majeure_service(db)

    def transfer_queue_to_tomorrow(self, *, request, current_user_id: int) -> dict[str, Any]:
        service = self.force_majeure_service
    ...
    def get_refund_request(self, *, request_id: int) -> dict[str, Any]:
        ...
        if not req:
            raise ForceMajeureDomainError(404, "Заявка на возврат не найдена")
        ...
        *,
        ...
        if action == "approve":
            ...
            if not process_request.rejection_reason:
                raise ForceMajeureDomainError(400, "Укажите причину отклонения")
            ...
            raise ForceMajeureDomainError(400, f"Неизвестное действие: {action}")
    ...
    def get_patient_deposit(self, *, patient_id: int) -> dict[str, Any]:
        ...
        if not deposit:
            raise ForceMajeureDomainError(404, "Депозит пациента не найден")
    ...
    def use_deposit_for_payment(self, *, request, current_user_id: int) -> dict[str, Any]:
        ...
        if not deposit.is_active:
            raise ForceMajeureDomainError(400, "Депозит деактивирован")
