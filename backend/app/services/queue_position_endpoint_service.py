...
@dataclass
class QueuePositionDomainError(Exception):
    status_code: int
    detail: str


class QueuePositionEndpointService:
    ...
    def _get_entry_or_error(self, entry_id: int):
        ...
        if not entry:
            raise QueuePositionDomainError(404, "Запись в очереди не найдена")
        return entry
    ...
    def get_position_entry_by_number(
        ...
        *,
        ...
        if not queue:
            raise QueuePositionDomainError(404, "Очередь не найдена")
        entry = self.repository.get_queue_entry_by_number(
            queue_id=queue.id,
            queue_number=queue_number,
        )
        if not entry:
            raise QueuePositionDomainError(
                404,
