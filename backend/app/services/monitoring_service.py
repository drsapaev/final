...
class MonitoringService:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                for table in main_tables:
                    ...
                    if table not in main_tables or not all(
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                "database" in app_metrics
                and app_metrics["database"].get("status") == "healthy"
            ):
                health_status["components"]["database"] = "healthy"

                # Проверка времени отклика БД
                if "performance" in app_metrics:
                    db_response = app_metrics["performance"].get(
            ...
            for metric in history:
                if "system" in metric and isinstance(metric["system"], dict):
                    if "cpu" in metric["system"]:
                        cpu_values.append(
                            metric["system"]["cpu"].get("usage_percent", 0)
                        )
                    if "memory" in metric["system"]:
                        memory_values.append(
                            metric["system"]["memory"].get("usage_percent", 0)
                        )
                    if "disk" in metric["system"]:
                        disk_values.append(
                            metric["system"]["disk"].get("usage_percent", 0)
                        )

                if "application" in metric and isinstance(metric["application"], dict):
                    if "performance" in metric["application"]:
                        db_time = metric["application"]["performance"].get(
                            "database_response_time", 0
                        )
                        if db_time > 0:
        ...
        while True:
            try:
                ...
                if health.get("overall_status") == "critical":
                    logger.critical(
                        f"Критическое состояние системы: {health.get('alerts', [])}"
                    )
    ...
    def update_thresholds(self, new_thresholds: Dict[str, float]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "updated_thresholds": self.thresholds}
