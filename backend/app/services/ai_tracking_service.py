...
class AITrackingService:
    ...
    def complete_request_tracking(
        ...
        if tracking.created_at:
            response_time = (tracking.completed_at - tracking.created_at).total_seconds() * 1000
        ...
        response = AIResponseWithTracking(
            data=response_data,
            tracking=tracking
        )
    ...
    def get_model_stats(
        ...
        query = self.db.query(
            ...
            func.sum(func.case([(AIUsageLog.success == True, 1)], else_=0)).label('successful_requests'),
            func.sum(func.case([(AIUsageLog.success == False, 1)], else_=0)).label('failed_requests'),
            ...
            func.avg(func.case([(AIUsageLog.cached_response == True, 1)], else_=0)).label('cache_hit_rate'),
