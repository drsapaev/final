...
@dataclass
class DynamicPricingDomainError(Exception):
    status_code: int
    detail: str


class DynamicPricingEndpointService:
    ...
    def get_pricing_rule(self, *, rule_id: int):
        ...
        if not rule:
            raise DynamicPricingDomainError(404, "Правило не найдено")
        return rule
    ...
    def get_service_package(self, *, package_id: int):
        ...
        if not package:
            raise DynamicPricingDomainError(404, "Пакет не найден")
        return package
