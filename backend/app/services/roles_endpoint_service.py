...
@dataclass
class RolesDomainError(Exception):
    status_code: int
    detail: str
...
class RolesService:
    ...
    def delete_role(self, *, role_id: int) -> None:
        role = self.get_role_or_error(role_id)
        if role.is_system:
            raise RolesDomainError(400, "Нельзя удалить системную роль")
