...
class PDFService:
    ...
    def generate_pdf_from_html(
        ...
        if not WEASYPRINT_AVAILABLE:
            raise Exception("WeasyPrint не установлен. Используйте: pip install weasyprint")
    ...
    def generate_pdf_with_reportlab(
        ...
        if not REPORTLAB_AVAILABLE:
            raise Exception("ReportLab не установлен. Используйте: pip install reportlab")
        ...
        if medications:
            ...
            for i, med in enumerate(medications, 1):
                ...
                if med.get('instructions'):
                    story.append(Paragraph(f"Способ применения: {med.get('instructions')}", styles['Normal']))
    ...
    def _build_certificate_reportlab(self, data: Dict[str, Any]) -> list:
        ...
        if visit.get("diagnosis"):
            story.append(Paragraph(f"<b>Диагноз:</b> {visit.get('diagnosis')}", styles['Normal']))
            if visit.get("icd10"):
                story.append(Paragraph(f"<b>МКБ-10:</b> {visit.get('icd10')}", styles['Normal']))
            story.append(Spacer(1, 20))
        
        # Подпись
        story.append(Spacer(1, 50))
        doctor = data.get("doctor", {})
        story.append(Paragraph(f"Врач: {doctor.get('full_name', '')}", styles['Normal']))
    ...
    def _build_lab_results_reportlab(self, data: Dict[str, Any]) -> list:
        ...
        if lab_results:
            ...
            results_table = Table(results_data, colWidths=[6*cm, 3*cm, 2*cm, 4*cm])
