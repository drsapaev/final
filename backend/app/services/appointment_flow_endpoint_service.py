...
@dataclass
class AppointmentFlowDomainError(Exception):
    status_code: int
    detail: str


class AppointmentFlowEndpointService:
    ...
    def resolve_appointment_from_visit(self, *, appointment_id: int, emr_data):
        ...
        if not visit:
            raise AppointmentFlowDomainError(404, "Запись не найдена")
    ...
    def resolve_canonical_visit(self, *, appointment_id: int) -> int:
        if not self.canonical_visit_service:
            raise AppointmentFlowDomainError(
                500,
