...
class ScheduleNextVisitRequest(BaseModel):
    ...
    # Backward compatibility for clients/tests that still send service_ids
    ...
    def normalize_services(self):
        ...
        if not self.services:
            raise ValueError("Either services or service_ids must be provided")
...
async def schedule_next_visit(
    ...
    try:
        ...
        visit = Visit(
            ...
            # Backward compatibility: for Admin/legacy flows keep a non-null doctor_id
            # even if Doctor profile is missing.
