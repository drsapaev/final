...
@dataclass
class PaymentWebhookDomainError(Exception):
    status_code: int
    detail: str


class PaymentWebhookEndpointService:
    """Orchestrates payment webhook endpoint logic."""

    def __init__(
        self,
        db,
        repository: PaymentWebhookApiRepository | None = None,
        notification_platform_service: NotificationPlatformService | None = None,
    ):  # type: ignore[no-untyped-def]
        self.repository = repository or PaymentWebhookApiRepository(db)
        self.notification_platform_service = (
            notification_platform_service or NotificationPlatformService(db)
        )
    ...
    def create_provider(self, provider_in: PaymentProviderCreate):
        ...
        if existing:
            raise PaymentWebhookDomainError(
                status_code=400,
    ...
    def get_provider(self, provider_id: int):
        ...
        if not provider:
            raise PaymentWebhookDomainError(status_code=404, detail="Provider not found")
        return provider
    ...
    def get_webhook(self, webhook_id: int):
        ...
        if not webhook:
            raise PaymentWebhookDomainError(status_code=404, detail="Webhook not found")
        return webhook
    ...
    def _emit_legacy_payment_notification(
        ...
        *,
        ...
        try:
            ...
            platform_service = self.notification_platform_service
    ...
    def _resolve_legacy_webhook_recipient(
        self, webhook: Any
        ...
        transaction = self.repository.get_latest_transaction_for_webhook(
        ...
        recipient = self.repository.get_active_user(patient.user_id)
    ...
    def _extract_patient_id_from_raw_data(raw_data: Any) -> int | None:
        ...
        for key in ("patient_id", "patientId", "client_id", "clientId"):
            candidate = PaymentWebhookEndpointService._safe_int(raw_data.get(key))
        ...
        if isinstance(params, dict):
            ...
            if isinstance(account, dict):
                for key in ("patient_id", "patientId", "client_id", "clientId"):
                    candidate = PaymentWebhookEndpointService._safe_int(account.get(key))
                    if candidate is not None:
                        return candidate
        return None
