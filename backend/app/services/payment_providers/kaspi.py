...
class KaspiProvider(BasePaymentProvider):
    ...
    def process_webhook(self, webhook_data: Dict[str, Any]) -> PaymentResult:
        ...
        try:
            ...
            if not self.validate_webhook_signature(webhook_data, signature):
                ...
                return PaymentResult(
                    success=False, error_message="Неверная подпись webhook"
                )
    ...
    def refund_payment(self, payment_id: str, amount: Decimal = None) -> PaymentResult:
        ...
        try:
            ...
            signature = self._generate_api_signature(
                "POST", url, json.dumps(refund_data), timestamp
            )
    ...
    def _generate_api_signature(
        self, method: str, url: str, body: str, timestamp: int
    ) -> str:
        ...
        return hmac.new(
            self.secret_key.encode(), sign_string.encode(), hashlib.sha256
        ).hexdigest()
