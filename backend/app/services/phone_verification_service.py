    ...
    async def send_verification_code(
        ...
        try:
            ...
            sms_result = await self.sms_manager.send_sms(
                phone=phone, message=message, provider_type=provider_type
            )
            ...
            if sms_result.success:
                ...
                logger.error(
                    f"Failed to send verification code to {phone}: {sms_result.error}"
                )
                return {
                    "success": False,
        ...
        try:
            ...
            return {
                ...
                "time_left_minutes": max(
                    ...
                    int(
                        (
                            verification_data['expires_at'] - datetime.now()
                        ).total_seconds()
                        / 60
                    ),
                ),
    ...
    async def verify_and_update_user_phone(
        ...
        try:
            ...
            updated_user = crud_user.update_user(
                db, user_id=user_id, user_data=user_data
            )
