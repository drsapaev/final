...
class AIAnalyticsService:
        ...
        try:
            ...
            usage_record = {
                ...
                "cost_estimate": self._estimate_cost(
                    ai_function, input_data, output_data
                ),
            }
    ...
    def get_ai_usage_analytics(
        ...
        try:
            ...
            usage_data = self._get_usage_data(
                start_date, end_date, user_id, ai_function
            )
    ...
    def get_ai_learning_insights(
        ...
        try:
            insights = {
                ...
                "medical_patterns": self._analyze_medical_patterns(
                    start_date, end_date
                ),
                "diagnostic_accuracy": self._analyze_diagnostic_accuracy(
                    start_date, end_date
                ),
                "treatment_effectiveness": self._analyze_treatment_effectiveness(
                    start_date, end_date
                ),
                "patient_outcomes": self._analyze_patient_outcomes(
                    start_date, end_date
                ),
        ...
        try:
            ...
            for function in ai_functions:
                ...
                optimization_results["models_analyzed"].append(
                    {"function": function, "analysis": function_analysis}
                )
    ...
    def generate_ai_training_dataset(
        ...
        try:
            ...
            if data_type == "diagnostic_patterns":
                dataset = self._generate_diagnostic_dataset(
                    start_date, end_date, anonymize
                )
            elif data_type == "treatment_outcomes":
                dataset = self._generate_treatment_dataset(
                    start_date, end_date, anonymize
                )
            elif data_type == "patient_symptoms":
                dataset = self._generate_symptoms_dataset(
                    start_date, end_date, anonymize
                )
            elif data_type == "scheduling_patterns":
                dataset = self._generate_scheduling_dataset(
                    start_date, end_date, anonymize
                )
            else:
    ...
    def _save_usage_record(self, record: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(
                f"AI Usage: {record['ai_function']} by user {record['user_id']} - {record['execution_time']:.3f}s"
            )
        except Exception as e:
    ...
    def _calculate_performance_metrics(
        ...
        return {
            "peak_hour": (
                max(hourly_load.items(), key=lambda x: x[1])[0] if hourly_load else None
            ),
    ...
    def _analyze_usage_trends(self, usage_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "peak_day": (
                max(daily_usage.items(), key=lambda x: x[1])[0] if daily_usage else None
            ),
            "lowest_day": (
                min(daily_usage.items(), key=lambda x: x[1])[0] if daily_usage else None
            ),
        }
    ...
    def _generate_usage_recommendations(
        ...
        if stats.get("average_execution_time", 0) > 5:
            recommendations.append(
                "Среднее время выполнения AI запросов превышает 5 секунд. Рекомендуется оптимизация промптов."
            )

        # Рекомендации по успешности
        if stats.get("success_rate", 100) < 95:
            recommendations.append(
                "Низкий процент успешных AI запросов. Проверьте качество входных данных и настройки API."
            )

        # Рекомендации по затратам
        if stats.get("total_cost_usd", 0) > 100:
            recommendations.append(
                "Высокие затраты на AI. Рассмотрите возможность оптимизации запросов или использования более экономичных моделей."
            )

        # Рекомендации по использованию
        function_stats = self._analyze_by_function(usage_data)
        underused_functions = [
            func for func, data in function_stats.items() if data["requests"] < 10
        ]
        if underused_functions:
            recommendations.append(
                f"Малоиспользуемые AI функции: {', '.join(underused_functions)}. Рассмотрите обучение пользователей."
            )
    ...
    def _analyze_medical_patterns(
        ...
        try:
            ...
            patterns = {
                ...
                "seasonal_diseases": self._identify_seasonal_patterns(
        ...
        return {
            "effective_treatments": [
                ...
                },
                {
                    ...
                    "treatment": "АПФ ингибиторы",
                    "success_rate": 87,
                },
                {
                    "condition": "Остеохондроз",
                    "treatment": "НПВС + физиотерапия",
                    "success_rate": 78,
                },
            ],
            "drug_combinations": [
    ...
    def _analyze_patient_outcomes(
        ...
        return {
            ...
            "readmission_rates": {"within_30_days": 5.2, "within_90_days": 12.7},
        }
