...
class BillingRule(Base):
    ...
    trigger_event = Column(String(50), nullable=False)  # visit_completed, appointment_created, etc.
    service_types = Column(String(500))  # Типы услуг (JSON)
    patient_categories = Column(String(500))  # Категории пациентов (JSON)
    amount_threshold_min = Column(Float)  # Минимальная сумма
    amount_threshold_max = Column(Float)  # Максимальная сумма
...
class BillingSettings(Base):
    ...
    overdue_threshold_days = Column(Integer, default=7)  # Через сколько дней считать просроченным
    ...
    # Напоминания
    reminder_days_before = Column(String(50), default="7,3,1")  # За сколько дней напоминать
    reminder_days_after = Column(String(50), default="1,7,14,30")  # Через сколько дней после просрочки
