...
class PrinterConfig(Base):
    ...
    name = Column(String(100), unique=True, nullable=False)  # ticket_printer, prescription_printer
    display_name = Column(String(150), nullable=False)  # "Принтер талонов"
    printer_type = Column(String(50), nullable=False)  # ESC/POS, A5, A4
    
    # Подключение
    connection_type = Column(String(20), nullable=False, default="network")  # network, usb, serial
    ...
    margins = Column(JSON, nullable=True)  # {"top": 5, "bottom": 5, "left": 5, "right": 5}
...
class PrintTemplate(Base):
    ...
    template_type = Column(String(50), nullable=False)  # ticket, prescription, memo, receipt
...
class PrintJob(Base):
    ...
    # Тип документа
    document_type = Column(String(50), nullable=False)  # ticket, prescription, memo, receipt
    document_id = Column(String(100), nullable=True)  # ID визита/платежа и т.д.
    
    # Статус
    status = Column(String(20), default="pending", nullable=False)  # pending, printing, completed, failed
