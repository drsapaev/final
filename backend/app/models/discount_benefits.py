...
class BenefitApplication(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    patient_benefit_id = Column(Integer, ForeignKey("patient_benefits.id"), nullable=False)
    benefit_id = Column(Integer, ForeignKey("benefits.id"), nullable=False)
...
class LoyaltyProgram(Base):
    ...
    min_purchase_for_points = Column(Float, default=0)  # Минимальная покупка для начисления
...
class PatientLoyalty(Base):
    ...
    point_transactions = relationship("LoyaltyPointTransaction", back_populates="patient_loyalty")
...
class LoyaltyPointTransaction(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    patient_loyalty_id = Column(Integer, ForeignKey("patient_loyalty.id"), nullable=False)
    
    # Тип транзакции
    transaction_type = Column(String(50), nullable=False)  # earned, redeemed, expired, bonus
    points = Column(Integer, nullable=False)  # Положительное для начисления, отрицательное для списания
    ...
    # Связи
    patient_loyalty = relationship("PatientLoyalty", back_populates="point_transactions")
    # appointment = relationship("Appointment")  # убираем пока
    # visit = relationship("Visit")  # убираем пока
    creator = relationship("User", foreign_keys=[created_by])
