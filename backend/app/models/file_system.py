...
class File(Base):
    ...
    file_hash: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)  # SHA-256 хеш
    ...
    tags: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON массив тегов
    file_metadata: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON с дополнительными данными
    ...
    appointment: Mapped[Appointment | None] = relationship("Appointment", foreign_keys=[appointment_id])
    emr: Mapped[EMR | None] = relationship("EMR", foreign_keys=[emr_id])
    folder: Mapped[FileFolder | None] = relationship("FileFolder", foreign_keys=[folder_id])
...
class FileShare(Base):
    ...
    shared_with_email: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
...
class FileFolder(Base):
    ...
    parent: Mapped[FileFolder | None] = relationship("FileFolder", remote_side=[id])
...
class FileStorage(Base):
    ...
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    max_file_size: Mapped[int | None] = mapped_column(Integer, nullable=True)  # Максимальный размер файла в байтах
    allowed_types: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON массив разрешенных типов
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
    updated_at: Mapped[datetime] = mapped_column(
...
class FileQuota(Base):
    ...
    max_files: Mapped[int | None] = mapped_column(Integer, nullable=True)  # Максимальное количество файлов
