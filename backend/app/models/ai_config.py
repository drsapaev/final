...
class AIProvider(Base):
    ...
    api_url: Mapped[str | None] = mapped_column(String(200), nullable=True)  # Базовый URL API
    model: Mapped[str | None] = mapped_column(String(100), nullable=True)  # gpt-4, gemini-pro
    temperature: Mapped[float] = mapped_column(Float, default=0.2, nullable=False)
    max_tokens: Mapped[int] = mapped_column(Integer, default=1000, nullable=False)
    active: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    is_default: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    capabilities: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)  # ["text", "vision", "ocr"]
...
class AIPromptTemplate(Base):
    ...
    examples: Mapped[list[dict[str, Any]] | None] = mapped_column(JSON, nullable=True)  # Примеры для few-shot learning

    # Настройки
    temperature: Mapped[float | None] = mapped_column(Float, nullable=True)  # Переопределяет настройки провайдера
    max_tokens: Mapped[int | None] = mapped_column(Integer, nullable=True)
    response_schema: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # JSON Schema для валидации ответа
