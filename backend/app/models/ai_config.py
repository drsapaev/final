...
class AIProvider(Base):
    ...
    limits = Column(JSON, nullable=True)  # {"requests_per_minute": 60, "tokens_per_day": 10000}
...
class AIPromptTemplate(Base):
    ...
    task_type = Column(String(50), nullable=False, index=True)  # complaints2plan, icd10, lab_interpret
    specialty = Column(String(50), nullable=True, index=True)  # cardiology, dermatology, stomatology
...
class AIUsageLog(Base):
    ...
    # Контекст
    request_hash = Column(String(64), nullable=True, index=True)  # Хеш запроса для кэширования
