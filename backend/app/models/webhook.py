...
class Webhook(Base):
    ...
    calls = relationship("WebhookCall", back_populates="webhook", cascade="all, delete-orphan")
...
class WebhookCall(Base):
    ...
    # Результат
    status = Column(SQLEnum(WebhookCallStatus), default=WebhookCallStatus.PENDING, index=True)
...
class WebhookEvent(Base):
    ...
    failed_webhooks = Column(JSON, default=[])  # Список webhook'ов, которые не смогли обработать событие
