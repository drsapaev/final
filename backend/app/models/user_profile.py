...
class UserProfile(Base):
    ...
    alternative_email: Mapped[str | None] = mapped_column(String(120), nullable=True)
    ...
    salary: Mapped[int | None] = mapped_column(Integer, nullable=True)  # В копейках
    ...
    notifications: Mapped[UserNotificationSettings | None] = relationship(
...
class UserPreferences(Base):
    ...
    emr_recent_icd10: Mapped[list[str] | None] = mapped_column(
        JSON, default=lambda: []
    )  # ["I10", "I25.9", "E11.9"]
    # Недавно использованные шаблоны назначений
    emr_recent_templates: Mapped[list[str] | None] = mapped_column(
        JSON, default=lambda: []
    )  # ["med-1", "exam-2"]
    # Избранные шаблоны по специальностям
    emr_favorite_templates: Mapped[dict[str, Any] | None] = mapped_column(
        JSON, default=lambda: {}
    )  # {"cardiology": ["med-1"], "general": ["med-g1"]}
    # Кастомные шаблоны пользователя
    emr_custom_templates: Mapped[list[dict[str, Any]] | None] = mapped_column(
        JSON, default=lambda: []
...
class UserAuditLog(Base):
    ...
    resource_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)  # row_id

    # Детали изменений
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    old_values: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # Старые значения (для UPDATE/DELETE)
    new_values: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # Новые значения (для CREATE/UPDATE)
    diff_hash: Mapped[str | None] = mapped_column(String(32), nullable=True)  # Хеш различий для быстрого сравнения

    # Контекст запроса
    ip_address: Mapped[str | None] = mapped_column(String(45), nullable=True, index=True)
    user_agent: Mapped[str | None] = mapped_column(Text, nullable=True)
    session_id: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)  # Используется для request_id
    request_id: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)  # UUID запроса для трассировки
