...
class Role(Base):
    ...
    level: Mapped[int] = mapped_column(Integer, default=0, nullable=False)  # Уровень в иерархии
    
    # Метаданные
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    is_system: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)  # Системная роль
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class UserGroup(Base):
    ...
    # Тип группы
    group_type: Mapped[str] = mapped_column(String(50), default="custom", nullable=False, index=True)  # department, custom, system
...
class UserPermissionOverride(Base):
    ...
    # Тип переопределения
    override_type: Mapped[str] = mapped_column(String(20), nullable=False)  # grant, deny
...
class RoleHierarchy(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    parent_role_id: Mapped[int] = mapped_column(Integer, ForeignKey("roles.id", ondelete="CASCADE"), nullable=False, index=True)
    child_role_id: Mapped[int] = mapped_column(Integer, ForeignKey("roles.id", ondelete="CASCADE"), nullable=False, index=True)
...
class PermissionAuditLog(Base):
    ...
    action: Mapped[str] = mapped_column(String(50), nullable=False, index=True)  # grant, revoke, override
    target_type: Mapped[str] = mapped_column(String(20), nullable=False)  # user, role, group
    ...
    # Связи
    user: Mapped["User"] = relationship("User", foreign_keys=[user_id])
    permission: Mapped[Optional["Permission"]] = relationship("Permission", foreign_keys=[permission_id])
