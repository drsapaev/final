...
class DailyQueue(Base):
    ...
    )  # ИСПРАВЛЕНО: relationship к Doctor
...
class OnlineQueueEntry(Base):
    ...
    discount_mode = Column(
        String(20), default="none", nullable=False
    )  # none, repeat, benefit, all_free
    services = Column(
        JSON, nullable=True
    )  # Список услуг с полными данными: [{"service_id": 123, "name": "...", "code": "K01", "quantity": 1, "price": 50000, "queue_time": "2025-11-11T10:30:00", "cancelled": false, "cancel_reason": null, "cancelled_by": null, "was_paid_before_cancel": false}]
    service_codes = Column(
        JSON, nullable=True
    )  # DEPRECATED: Коды услуг ["K01", "K02", ...] - сохранено для обратной совместимости
