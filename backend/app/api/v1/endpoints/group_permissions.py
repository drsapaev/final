...
from app.models.role_permission import Role, Permission, UserGroup, UserPermissionOverride
...
def check_user_permission(
    ...
    try:
        ...
        logger.error(f"Ошибка проверки разрешения {permission} для пользователя {user_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get("/groups/{group_id}/permissions", response_model=GroupPermissionsSummaryResponse)
...
def revoke_role_from_group(
    ...
    try:
        service = get_group_permissions_service()
        result = service.revoke_role_from_group(
            db, group_id, role_id, current_user.id
        )
...
def add_user_to_group(
    ...
    try:
        ...
        logger.error(f"Ошибка добавления пользователя {request.user_id} в группу {group_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def remove_user_from_group(
    ...
    try:
        ...
        result = service.remove_user_from_group(
            db, user_id, group_id, current_user.id
        )
