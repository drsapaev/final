...
async def get_version_timeline(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка получения временной линии: {str(e)}"
        )
...
async def compare_versions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка сравнения версий: {str(e)}"
        )
...
async def create_version_with_analysis(
    ...
    try:
        ...
        version = await emr_versioning_enhanced.create_version_with_analysis(
            ...
            previous_version=previous_version.version_data if previous_version else None
        )
...
async def get_version_details(
    ...
    try:
        ...
        if version.emr_id != emr_id:
            raise HTTPException(status_code=400, detail="Версия не принадлежит указанному EMR")
        
        # Получаем краткое описание данных
        data_summary = await emr_versioning_enhanced._get_data_summary(version.version_data)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка получения деталей версии: {str(e)}"
        )
...
async def export_versions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка экспорта версий: {str(e)}"
        )
