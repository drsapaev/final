...
async def get_queue_status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения статуса очереди: {str(e)}")
...
async def open_queue(
    ...
    try:
        ...
        online_queue._set_int(db, online_queue._k(department, date_str, "start_number"), start_number)
        online_queue._set_int(db, online_queue._k(department, date_str, "last_ticket"), start_number - 1)
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Ошибка открытия очереди: {str(e)}")
...
async def close_queue(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Ошибка закрытия очереди: {str(e)}")
...
async def call_next_patient(
    ...
    try:
        ...
        online_queue._set_int(db, online_queue._k(department, date_str, "waiting"), waiting)
        
        # Увеличиваем количество обслуживаемых
        serving = stats.serving + 1
        online_queue._set_int(db, online_queue._k(department, date_str, "serving"), serving)
        
        db.commit()
        
        # Отправляем broadcast через WebSocket
        try:
            updated_stats = online_queue.load_stats(db, department=department, date_str=date_str)
...
async def complete_patient(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Ошибка завершения приема: {str(e)}")
...
async def add_to_queue(
    ...
    try:
        ...
        online_queue._set_int(db, online_queue._k(department, date_str, "last_ticket"), next_ticket)
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Ошибка добавления в очередь: {str(e)}")
...
async def get_queue_departments(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения списка отделений: {str(e)}")
...
async def get_queue_history(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения истории очереди: {str(e)}")
