...
async def get_cardiology_analytics(
    ...
    total_revenue = (
        base_query.with_entities(func.sum(Visit.payment_amount)).scalar() or 0
    )
...
async def get_specialized_statistics(
    ...
    cardiology_revenue = (
        cardiology_query.with_entities(func.sum(Visit.payment_amount)).scalar() or 0
    )
    ...
    dentistry_revenue = (
        dentistry_query.with_entities(func.sum(Visit.payment_amount)).scalar() or 0
    )
