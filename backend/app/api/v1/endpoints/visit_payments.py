...
def get_visit_payment_info(
    ...
    try:
        success, message, payment_info = VisitPaymentIntegrationService.get_visit_payment_info(
            db, visit_id
        )
...
@router.get("/visit-payments/by-status/{payment_status}", summary="Визиты по статусу платежа")
def get_visits_by_payment_status(
    ...
    try:
        success, message, visits = VisitPaymentIntegrationService.get_visits_by_payment_status(
...
@router.post("/visit-payments/{visit_id}/update-status", summary="Обновление статуса платежа")
    ...
    try:
        ...
        for status in statuses:
            ...
            if success:
                summary[status] = {
                    "count": len(visits),
                    "total_amount": sum(float(v.get("payment_amount", 0)) for v in visits if v.get("payment_amount"))
                }
            else:
        ...
        return {
            ...
            "payment_success_rate": (summary.get("paid", {}).get("count", 0) / max(total_visits, 1)) * 100
        }
...
@router.post("/visit-payments/{visit_id}/create-from-payment", summary="Создание визита из платежа")
