...
async def integrate_lab_results_with_emr(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка интеграции лабораторных данных: {str(e)}"
        )
...
async def get_abnormal_lab_results(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка получения аномальных результатов: {str(e)}"
        )
...
async def notify_doctor_about_lab_result(
    ...
    try:
        ...
        return {
            "message": "Уведомление врача отправлено",
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка уведомления врача: {str(e)}"
        )
    ...
    try:
        ...
        if values:
            trend_stats = {
                ...
                "abnormal_percentage": (len([r for r in results if r["is_abnormal"]]) / len(results)) * 100
            }
        else:
            trend_stats = {}
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка получения трендов: {str(e)}"
        )
