...
SECURITY_MANAGEMENT_PUBLIC_ERROR = "Internal server error"


def _security_management_http_error(exc: Exception) -> HTTPException:
    logger.warning(
        "Security management endpoint failed error_type=%s",
        type(exc).__name__,
    )
    return HTTPException(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        detail=SECURITY_MANAGEMENT_PUBLIC_ERROR,
    )
