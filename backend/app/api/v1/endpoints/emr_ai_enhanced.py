...
async def generate_smart_template(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка генерации шаблона: {str(e)}"
        )
...
async def get_smart_suggestions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка получения подсказок: {str(e)}"
        )
...
async def auto_fill_emr_fields(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка автозаполнения: {str(e)}"
        )
...
async def get_icd10_suggestions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка генерации ICD-10 предложений: {str(e)}"
        )
...
async def analyze_patient_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка анализа пациента: {str(e)}"
        )
...
async def enhance_emr_with_ai(
    emr_id: int,
    enhancement_type: str = Query(..., description="Тип улучшения: suggestions, validation, auto_fill"),
    db: Session = Depends(deps.get_db),
    current_user: User = Depends(deps.require_roles("Admin", "Doctor"))
) -> Any:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка улучшения EMR: {str(e)}"
        )
