...
def list_patients(
    ...
    """
    patients = patient_crud.get_patients(db, skip=skip, limit=limit, search_query=q, phone=phone)
    return patients
...
def create_patient(
    *,
    ...
    has_individual_names = (
        (patient_in.last_name and patient_in.last_name.strip()) or
    ...
    name_parts = normalize_patient_name(
        ...
        last_name=patient_in.last_name.strip() if (patient_in.last_name and patient_in.last_name.strip()) else None,
        first_name=patient_in.first_name.strip() if (patient_in.first_name and patient_in.first_name.strip()) else None,
        middle_name=patient_in.middle_name.strip() if (patient_in.middle_name and patient_in.middle_name.strip()) else None
    )
    ...
    print(f"[create_patient] Нормализация: full_name={patient_in.full_name}, last_name={patient_in.last_name}, first_name={patient_in.first_name}")
    print(f"[create_patient] Результат нормализации: last_name='{normalized_last_name}', first_name='{normalized_first_name}'")
    ...
    normalized_first_name = normalized_first_name.strip() if normalized_first_name else ""
    ...
    if patient_in.birth_date and not validate_birthdate(patient_in.birth_date):
        raise HTTPException(
            status_code=400, detail="Некорректная дата рождения"
        )
