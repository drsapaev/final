...
@router.post("/close", name="online_queue_close", dependencies=[Depends(require_roles("Admin"))])
def close_alias(
    department: str = Query(...),
    date_str: Optional[str] = Query(None),
    date: Optional[str] = Query(None),
...
def join_online_queue(
    ...
    if not is_within_morning_window(db=db, department=dep, date_str=d):
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="online window is closed")
    ...
    if payload.phone or payload.tg_id:
        if (old := get_existing_ticket_for_identity(db=db, department=dep, date_str=d, phone=payload.phone, tg_id=payload.tg_id)) is not None:
    ...
    remember_identity_ticket(db=db, department=dep, date_str=d, phone=payload.phone, tg_id=payload.tg_id, ticket=ticket)
    return OnlineJoinResponse(
        queue_entry_id=0,
