...
def update_payment_provider_settings(
    ...
    try:
        ...
        logger.info(
            f"Настройки платежных провайдеров обновлены пользователем {current_user.username}"
        )
    ...
    try:
        ...
        if provider_name == "click":
            ...
            try:
                ...
                return TestProviderResponse(
                    success=False, message=f"Ошибка инициализации провайдера: {str(e)}"
                )
            ...
            return TestProviderResponse(
                success=False, message=f"Неподдерживаемый провайдер: {provider_name}"
            )
