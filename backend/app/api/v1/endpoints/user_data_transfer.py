...
class DataTransferRequest(BaseModel):
    ...
    confirmation_required: bool = Field(
        default=True, description="Требуется ли подтверждение от пользователей"
    )
...
def search_users_for_transfer(
    ...
    try:
        ...
        return {"users": result, "total": len(result), "query": query}
...
def transfer_user_data(
    ...
    try:
        ...
        is_valid, error_message = service.validate_transfer_request(
            db, request.source_user_id, request.target_user_id, current_user.id
        )
...
def confirm_data_transfer(
    ...
    try:
        ...
        result = service.confirm_transfer_by_token(
            db, token=request.token, confirmed_by_user_id=current_user.id
        )
...
def get_transfer_history(
    ...
    try:
        ...
        history = service.get_transfer_history(db, user_id=user_id, limit=limit)
...
def validate_transfer_request(
    ...
    try:
        ...
        is_valid, error_message = service.validate_transfer_request(
            db, source_user_id, target_user_id, current_user.id
        )
