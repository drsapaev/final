...
async def search_doctors(
    ...
    try:
        ...
        for doctor in doctors:
            ...
            total_patients = crud_appointment.count_doctor_patients(db, doctor_id=doctor.id)
            ...
            result.append({
                ...
                "available_today": crud_doctor.is_doctor_available_today(db, doctor.id),
                "next_available_slot": crud_doctor.get_next_available_slot(db, doctor.id),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка поиска врачей: {str(e)}"
        )
    ...
    try:
        ...
        for service in services:
            result.append({
                ...
                "available_doctors": crud_service.get_service_doctors_count(db, service.id)
            })
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка поиска услуг: {str(e)}"
        )
    ...
    try:
        ...
        for category in categories:
            ...
            result.append({
                ...
                "average_price": crud_service.get_category_avg_price(db, category.name)
            })
...
async def get_my_queue_position(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка получения позиции в очереди: {str(e)}"
        )
...
async def cancel_appointment(
    ...
    try:
        ...
        appointment = crud_appointment.get_appointment(db, appointment_id=request.appointment_id)
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500,
                detail="Не удалось отменить запись"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка отмены записи: {str(e)}"
        )
...
async def reschedule_appointment(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500,
                detail="Не удалось перенести запись"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка переноса записи: {str(e)}"
        )
...
ID: #{emergency.id}"""
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка экстренного обращения: {str(e)}"
        )
...
async def update_profile(
    ...
    try:
        ...
        if request.birth_date:
            update_data["birth_date"] = datetime.fromisoformat(request.birth_date).date()
        if request.address:
        ...
        updated_patient = crud_patient.update_patient(db, patient_id=patient.id, update_data=update_data)
...
async def upload_avatar(
    ...
    try:
        ...
        crud_patient.update_patient(db, patient_id=patient.id, update_data={"avatar_url": file_path})
...
async def update_notification_settings(
    ...
    try:
        ...
        crud_user.update_notification_settings(db, user_id=current_user.id, settings=settings_data)
...
async def get_clinic_info(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка получения информации о клинике: {str(e)}"
        )
