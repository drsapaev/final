...
def _user_throttle_key(request: Request) -> str:
    # PR-6 rounds 12-13 (codex P2→P1): key the registration throttle by the
    # STABLE authenticated identity. The bearer credential itself is NOT
    # stable (refresh mints a new token with a fresh jti), so the key is
    # derived from the VERIFIED JWT subject; the raw credential never
    # enters limiter keys. Requests without a bearer header (they fail
    # auth anyway) fall back to the client IP, preserving an abuse bound.
    ...
    if token:
        try:
            ...
            sub = payload.get("sub")
            # Round 14 (codex P2): the two mint sites disagree on `sub` —
            # mobile initial login uses user.username, auth_svc refresh
            # uses str(user.id) + a `username` claim. Normalize BOTH to the
            # username so one user holds ONE bucket across token forms.
            identity = payload.get("username") or sub
            if identity:
                return f"push-user:{identity}"
        except Exception:
            # Expired/garbled token: it cannot survive get_current_user, but
            # the fallback key keeps a bounded per-credential bucket.
            pass
        return "push-user:tok:" + hashlib.sha256(token.encode()).hexdigest()[:32]
