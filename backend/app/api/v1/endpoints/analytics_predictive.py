...
async def get_scenario_analysis(
    ...
    end_date: str = Query(..., description="Конечная дата (YYYY-MM-DD)"),
    department: Optional[str] = Query(None, description="Отделение"),
    scenario: str = Query("optimistic", description="Сценарий: optimistic, realistic, pessimistic"),
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
...
async def get_predictive_insights(
    ...
    end_date: str = Query(..., description="Конечная дата (YYYY-MM-DD)"),
    department: Optional[str] = Query(None, description="Отделение"),
    insight_type: str = Query("all", description="Тип инсайтов: all, revenue, patients, efficiency"),
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
