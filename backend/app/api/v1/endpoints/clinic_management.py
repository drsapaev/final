...
from app.schemas.clinic import (
    BranchCreate, BranchUpdate, BranchOut, BranchStatsOut,
...
def get_branches(
    *,
    ...
    search: Optional[str] = Query(None, description="Поиск по названию, адресу или коду"),
    current_user: User = Depends(require_admin)
):
...
def update_branch(
    *,
    ...
    """Обновить филиал"""
    branch = branch_management.update_branch(db=db, branch_id=branch_id, branch_data=branch_data)
    if not branch:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Филиал не найден"
...
def create_equipment(
    *,
    ...
    try:
        return equipment_management.create_equipment(db=db, equipment_data=equipment_data)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def get_equipment(
    *,
    ...
    status: Optional[str] = Query(None, description="Фильтр по статусу"),
    search: Optional[str] = Query(None, description="Поиск по названию, модели или серийному номеру"),
    current_user: User = Depends(require_admin)
):
...
def update_equipment(
    *,
    ...
    equipment = equipment_management.update_equipment(db=db, equipment_id=equipment_id, equipment_data=equipment_data)
    if not equipment:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Оборудование не найдено"
...
def get_maintenance_due(
    *,
    db: Session = Depends(get_db),
    days_ahead: int = Query(30, ge=1, le=365, description="Количество дней вперед для проверки"),
    current_user: User = Depends(require_admin)
):
...
@router.post("/equipment/{equipment_id}/maintenance", response_model=EquipmentMaintenanceOut, status_code=status.HTTP_201_CREATED)
def schedule_maintenance(
    *,
    ...
    try:
        return equipment_management.schedule_maintenance(db=db, equipment_id=equipment_id, maintenance_data=maintenance_data)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
@router.get("/equipment/{equipment_id}/maintenance", response_model=List[EquipmentMaintenanceOut])
...
def get_licenses(
    *,
    ...
    status: Optional[str] = Query(None, description="Фильтр по статусу"),
    search: Optional[str] = Query(None, description="Поиск по названию, ключу или издателю"),
    current_user: User = Depends(require_admin)
):
...
def update_license(
    *,
    ...
    """Обновить лицензию"""
    license = license_management.update_license(db=db, license_id=license_id, license_data=license_data)
    if not license:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Лицензия не найдена"
...
@router.post("/licenses/{license_id}/activate", response_model=LicenseActivationOut, status_code=status.HTTP_201_CREATED)
def activate_license(
    *,
    ...
    try:
        return license_management.activate_license(db=db, license_id=license_id, activation_data=activation_data)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
@router.get("/licenses/{license_id}/activations", response_model=List[LicenseActivationOut])
...
def update_backup(
    *,
    ...
    backup = backup_management.update_backup(db=db, backup_id=backup_id, backup_data=backup_data)
    if not backup:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Резервная копия не найдена"
...
@router.post("/backups/full", response_model=BackupOut, status_code=status.HTTP_201_CREATED)
...
@router.post("/backups/incremental", response_model=BackupOut, status_code=status.HTTP_201_CREATED)
