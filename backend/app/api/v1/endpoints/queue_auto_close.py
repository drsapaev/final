...
async def check_and_close_expired_queues(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка при проверке очередей: {str(e)}")
...
async def get_queues_pending_close(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка при получении очередей: {str(e)}")
...
async def force_close_queue(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка при закрытии очереди: {str(e)}")
