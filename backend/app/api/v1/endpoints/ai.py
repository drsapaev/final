    ...
    try:
        ...
        if metadata:
            try:
                ...
                raise HTTPException(status_code=400, detail="Неверный формат метаданных")
...
async def suggest_drug_alternatives(
    request: DrugAlternativesRequest,
    current_user: User = Depends(get_current_user)
):
...
async def predict_readmission_risk(
    request: ReadmissionRiskRequest,
    current_user: User = Depends(get_current_user)
):
...
async def structure_medical_text(
    request: TextStructuringRequest,
    current_user: User = Depends(get_current_user)
):
...
async def predict_appointment_duration(
    request: AppointmentDurationRequest,
    current_user: User = Depends(get_current_user)
):
...
async def generate_shift_recommendations(
    request: ShiftRecommendationsRequest,
    current_user: User = Depends(get_current_user)
):
...
async def analyze_documentation_quality(
    request: DocumentationQualityRequest,
    current_user: User = Depends(get_current_user)
):
...
async def detect_documentation_gaps(
    request: DocumentationGapsRequest,
    current_user: User = Depends(get_current_user)
):
...
async def validate_clinical_consistency(
    request: ClinicalConsistencyRequest,
    current_user: User = Depends(get_current_user)
):
...
async def audit_prescription_safety(
    request: PrescriptionSafetyRequest,
    current_user: User = Depends(get_current_user)
):
