...
async def get_wait_time_analytics(
    ...
    try:
        ...
        analytics = service.calculate_accurate_wait_times(start, end, department, doctor_id)
...
async def get_service_wait_analytics(
    ...
    try:
        ...
        service = get_wait_time_analytics_service(db)
        analytics = service.get_wait_time_analytics_by_service(start, end, service_codes_list)
...
async def get_wait_time_summary(
    ...
    try:
        ...
        service = get_wait_time_analytics_service(db)
        analytics = service.calculate_accurate_wait_times(start_date, end_date, department)
        ...
        summary = {
            ...
            "performance_rating": _calculate_performance_rating(overall_stats.get("average_minutes", 0)),
...
async def get_wait_time_comparison(
    ...
    compare_with_previous: bool = Query(True, description="Сравнить с предыдущим периодом"),
    department: Optional[str] = Query(None, description="Фильтр по отделению"),
    db: Session = Depends(get_db),
    current_user: User = Depends(require_roles(["Admin", "Registrar", "Doctor"]))
):
    ...
    try:
        ...
        if compare_with_previous:
            ...
            current_avg = current_analytics.get("overall_stats", {}).get("average_minutes", 0)
            previous_avg = previous_analytics.get("overall_stats", {}).get("average_minutes", 0)
    ...
    try:
        ...
        for hour in range(8, 19):  # Рабочие часы с 8 до 18
            ...
            heatmap_data.append({
                ...
                "intensity": _calculate_intensity(hour_stats.get("average_minutes", 0))
            })
        ...
        return {
            ...
            "summary": {
                "peak_hour": max(heatmap_data, key=lambda x: x["average_wait_minutes"])["hour"] if heatmap_data else None,
                "best_hour": min(heatmap_data, key=lambda x: x["average_wait_minutes"] or float('inf'))["hour"] if heatmap_data else None,
                "busiest_hour": max(heatmap_data, key=lambda x: x["patient_count"])["hour"] if heatmap_data else None
            }
        }
