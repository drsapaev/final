...
from app.crud.payment_webhook import (
    get_all_webhooks, get_webhook_by_id, get_all_transactions, 
    get_transaction_by_id, get_all_providers, get_provider_by_id,
    create_provider, update_provider, delete_provider
)
from app.schemas.payment_webhook import (
    PaymentWebhookOut, PaymentTransactionOut, PaymentProviderOut,
    PaymentProviderCreate, PaymentProviderUpdate
)
...
@router.get("/payment/providers", name="list_providers", response_model=List[PaymentProviderOut])
def list_providers(
    db: Session = Depends(get_db),
...
@router.post("/payment/providers", name="create_provider", response_model=PaymentProviderOut)
def create_provider(
...
@router.get("/payment/providers/{provider_id}", name="get_provider", response_model=PaymentProviderOut)
def get_provider(
    provider_id: int,
    db: Session = Depends(get_db),
    current_user=Depends(require_roles("Admin"))
...
@router.put("/payment/providers/{provider_id}", name="update_provider", response_model=PaymentProviderOut)
def update_provider(
    provider_id: int,
...
@router.get("/payment/transactions", name="list_transactions", response_model=List[PaymentTransactionOut])
...
@router.get("/payment/transactions/{transaction_id}", name="get_transaction", response_model=PaymentTransactionOut)
def get_transaction(
    transaction_id: int,
    db: Session = Depends(get_db),
    current_user=Depends(require_roles("Admin", "Registrar"))
...
@router.get("/payment/{webhook_id}", name="get_webhook", response_model=PaymentWebhookOut)
def get_webhook(
    webhook_id: int,
    db: Session = Depends(get_db),
    current_user=Depends(require_roles("Admin", "Registrar"))
