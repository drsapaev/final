...
async def get_dashboard_data(
    ...
    if today.month == 12:
        month_end = today_start.replace(
            year=today.year + 1, month=1, day=1
        ) - timedelta(days=1)
    else:
        month_end = today_start.replace(month=today.month + 1, day=1) - timedelta(
            days=1
        )

    dashboard_data = {
        "today": {
            "visits": analytics_service.get_visit_statistics(
                db, today_start, today_end
            ),
            "revenue": analytics_service.get_revenue_statistics(
                db, today_start, today_end
            ),
            "queues": analytics_service.get_queue_statistics(
                db, today_start, today_end
            ),
        },
        "week": {
        ...
        "month": {
            ...
            "patients": analytics_service.get_patient_statistics(
                db, month_start, month_end
            ),
            "services": analytics_service.get_service_statistics(
                db, month_start, month_end
            ),
        },
...
async def get_payment_provider_analytics(
    ...
    return analytics_service.get_payment_provider_analytics(db, start, end)
...
async def get_appointment_flow_analytics(
    ...
    return analytics_service.get_appointment_flow_analytics(db, start, end, department)
...
async def get_revenue_breakdown_analytics(
    ...
    return analytics_service.get_revenue_breakdown_analytics(db, start, end, department)
...
async def get_quick_stats(
    ...
    today_patients = (
        db.query(Patient)
        .filter(Patient.created_at >= today_start, Patient.created_at <= today_end)
        .count()
    )
