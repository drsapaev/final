...
async def get_doctors_list(
    ...
    try:
        ...
        return DoctorListResponse(
            doctors=doctors_response, total_count=len(doctors_response)
        )
...
async def get_department_info(
    ...
    try:
        ...
        if not department_info:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Отделение не найдено"
            )
...
async def get_departments_list(
    ...
    try:
        ...
        return DepartmentListResponse(
            departments=departments_response, total_count=len(departments_response)
        )
...
@router.get(
    "/appointments/{appointment_id}/doctor-info",
    response_model=AppointmentDoctorInfoResponse,
)
async def get_appointment_doctor_info(
    appointment_id: int,
    appointment_type: str = Query(
        "appointment", description="Тип записи: appointment или visit"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(require_roles(["Admin", "Registrar", "Doctor"])),
):
    ...
    try:
        ...
        return AppointmentDoctorInfoResponse(
            doctor=doctor_response, department=department_response
        )
...
async def get_formatted_department_info(
    ...
    try:
        ...
        formatted_info = service.format_department_info_for_notification(
            department_info
        )
        ...
        logger.error(
            f"Ошибка форматирования информации об отделении {department_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка форматирования информации: {str(e)}",
