    ...
    def _join_queue_impl(
        ...
        try:
            with get_db_session() as db:
                ...
                # QD-2C (Codex round-22 P2, теперь пояс-надежности): проверяем
                # ФАКТ по возвращённой очереди после залоченного re-fetch.
                # QD-2E: гвард канонического врача задним числом выполняется
                # для ЛЮБОЙ очереди doctor-оси (queue_resource_id is None), а
                # не только для окна registry-misresolution — переход на
                # queue_service-сим убрал внутренний коммит crud-сема, и окно
                # «врач деактивирован между ранним гвардом и выдачей номера»
                # больше не прикрывалось ничем (пин round-15). Реальный врач,
                # прошедший ранний гвард, проходит и этот — семантика
                # байт-идентична; ресурсная ось (queue_resource_id set) гвард
                # врача не требует. Инстанс врача мог быть загружен ранним
                # гвардом ДО окна — expайрим его, чтобы канонический
                # предикат перечитал строку (plain query не перезаписывает
                # незакэкспайренный identity-map инстанс).
                if daily_queue.queue_resource_id is None:
                    if input.doctor_id is not None:
                        _doctor_inst = db.get(Doctor, input.doctor_id)
                        if _doctor_inst is not None:
                            db.expire(_doctor_inst)
                    try:
                        ensure_doctor_eligible_for_appointment(db, input.doctor_id)
                    except HTTPException:
                        return QueueMutationResponse(
