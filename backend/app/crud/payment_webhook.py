...
from sqlalchemy import and_, select, MetaData, Table
from datetime import datetime

from app.models.payment_webhook import PaymentWebhook, PaymentTransaction, PaymentProvider
from app.schemas.payment_webhook import PaymentWebhookCreate, PaymentWebhookUpdate, PaymentTransactionCreate, PaymentTransactionUpdate, PaymentProviderCreate, PaymentProviderUpdate
...
def get_webhook_by_transaction_id(db: Session, transaction_id: str) -> Optional[PaymentWebhook]:
    """Получение вебхука по transaction_id"""
    t = _payment_webhooks(db)
    row = db.execute(select(t).where(t.c.transaction_id == transaction_id)).mappings().first()
    return PaymentWebhook(**dict(row)) if row else None


def get_webhooks_by_provider(db: Session, provider: str, skip: int = 0, limit: int = 100) -> List[PaymentWebhook]:
...
def get_pending_webhooks(db: Session, skip: int = 0, limit: int = 100) -> List[PaymentWebhook]:
...
def get_failed_webhooks(db: Session, skip: int = 0, limit: int = 100) -> List[PaymentWebhook]:
...
def update_webhook(db: Session, webhook_id: int, webhook_in: dict) -> Optional[PaymentWebhook]:
    """Обновление вебхука"""
    t = _payment_webhooks(db)
...
def get_all_webhooks(db: Session, skip: int = 0, limit: int = 100) -> List[PaymentWebhook]:
    """Получение всех вебхуков"""
    t = _payment_webhooks(db)
    rows = (
        db.execute(select(t).offset(skip).limit(limit))
        .mappings()
        .all()
    )
    return [PaymentWebhook(**dict(row)) for row in rows]
...
def create_transaction(db: Session, transaction_in: PaymentTransactionCreate) -> PaymentTransaction:
    """Создание транзакции"""
...
def get_transaction_by_id(db: Session, transaction_id: int) -> Optional[PaymentTransaction]:
...
def get_transaction_by_transaction_id(db: Session, transaction_id: str) -> Optional[PaymentTransaction]:
...
def get_transactions_by_provider(db: Session, provider: str, skip: int = 0, limit: int = 100) -> List[PaymentTransaction]:
...
def get_transactions_by_status(db: Session, status: str, skip: int = 0, limit: int = 100) -> List[PaymentTransaction]:
...
def get_transactions_by_visit(db: Session, visit_id: int) -> List[PaymentTransaction]:
    ...
    t = _payment_transactions(db)
    rows = (
        db.execute(select(t).where(t.c.visit_id == visit_id))
        .mappings()
        .all()
    )
    return [PaymentTransaction(**dict(row)) for row in rows]
...
def get_successful_transactions_by_visit(db: Session, visit_id: int) -> List[PaymentTransaction]:
...
def update_transaction(db: Session, transaction_id: int, transaction_in: dict) -> Optional[PaymentTransaction]:
    ...
    row = (
        db.execute(t.update().where(t.c.id == transaction_id).values(**data).returning(t))
        .mappings()
        .first()
    )
    if row:
...
def get_all_transactions(db: Session, skip: int = 0, limit: int = 100) -> List[PaymentTransaction]:
...
def get_active_providers(db: Session) -> List[PaymentProvider]:
    ...
    t = _payment_providers(db)
    rows = (
        db.execute(select(t).where(t.c.is_active == True))
        .mappings()
        .all()
    )
...
def get_provider_by_provider_type(db: Session, provider_type: str) -> Optional[PaymentProvider]:
    """Получаем провайдера по типу (payme, click, etc.)"""
    return get_provider_by_code(db, provider_type)


def update_provider(db: Session, provider_id: int, provider_in: PaymentProviderUpdate) -> Optional[PaymentProvider]:
...
def get_all_providers(db: Session, skip: int = 0, limit: int = 100) -> List[PaymentProvider]:
...
class CRUDPaymentWebhook:
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100) -> List[PaymentWebhook]:
        return get_all_webhooks(db, skip, limit)
    
    def update(self, db: Session, db_obj: PaymentWebhook, obj_in: dict) -> Optional[PaymentWebhook]:
...
class CRUDPaymentTransaction:
    def create(self, db: Session, obj_in: PaymentTransactionCreate) -> PaymentTransaction:
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100) -> List[PaymentTransaction]:
        return get_all_transactions(db, skip, limit)
    
    def update(self, db: Session, db_obj: PaymentTransaction, obj_in: dict) -> Optional[PaymentTransaction]:
...
class CRUDPaymentProvider:
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100) -> List[PaymentProvider]:
        return get_all_providers(db, skip, limit)
    
    def update(self, db: Session, db_obj: PaymentProvider, obj_in: dict) -> Optional[PaymentProvider]:
