...
class CRUDBranch:
    ...
    def get_stats(self, db: Session, *, branch_id: int) -> Optional[BranchStatsOut]:
        ...
        # Подсчет врачей
        doctors_count = db.query(func.count()).select_from(Branch).join(
            Branch.doctors
        ).filter(Branch.id == branch_id).scalar() or 0
        
        # Подсчет оборудования
        equipment_count = db.query(func.count()).select_from(Branch).join(
            Branch.equipment
        ).filter(Branch.id == branch_id).scalar() or 0
        
        # Активное оборудование
        active_equipment = db.query(func.count()).select_from(Branch).join(
            Branch.equipment
        ).filter(
        ...
        maintenance_due = db.query(func.count()).select_from(Branch).join(
            Branch.equipment
        ).filter(
            and_(
                Branch.id == branch_id,
        ...
        last_backup = db.query(Backup.completed_at).filter(
...
class CRUDEquipment:
    ...
    def update(self, db: Session, *, db_obj: Equipment, obj_in: EquipmentUpdate) -> Equipment:
    ...
    def get_maintenance_due(self, db: Session, *, days_ahead: int = 30) -> List[Equipment]:
...
class CRUDEquipmentMaintenance:
    ...
    def create(self, db: Session, *, obj_in: EquipmentMaintenanceCreate) -> EquipmentMaintenance:
    ...
    def get(self, db: Session, *, id: int) -> Optional[EquipmentMaintenance]:
        ...
        return db.query(EquipmentMaintenance).filter(EquipmentMaintenance.id == id).first()
    ...
    def get_by_equipment(
        ...
        *, 
        ...
        ).order_by(desc(EquipmentMaintenance.maintenance_date)).offset(skip).limit(limit).all()
    
    def update(self, db: Session, *, db_obj: EquipmentMaintenance, obj_in: EquipmentMaintenanceUpdate) -> EquipmentMaintenance:
    ...
    def delete(self, db: Session, *, id: int) -> Optional[EquipmentMaintenance]:
        ...
        obj = db.query(EquipmentMaintenance).filter(EquipmentMaintenance.id == id).first()
        if obj:
            db.delete(obj)
            db.commit()
        return obj
...
class CRUDLicenseActivation:
    ...
    def create(self, db: Session, *, obj_in: LicenseActivationCreate) -> LicenseActivation:
    ...
    def get_by_license(
        ...
        *, 
        ...
        return db.query(LicenseActivation).filter(
            LicenseActivation.license_id == license_id
        ).order_by(desc(LicenseActivation.activation_date)).offset(skip).limit(limit).all()
    
    def update(self, db: Session, *, db_obj: LicenseActivation, obj_in: LicenseActivationUpdate) -> LicenseActivation:
        """Обновить активацию"""
        update_data = obj_in.dict(exclude_unset=True)
        for field, value in update_data.items():
            setattr(db_obj, field, value)
...
class CRUDClinicStats:
    ...
    def get_clinic_stats(self, db: Session) -> ClinicStatsOut:
        ...
        active_branches = db.query(func.count(Branch.id)).filter(
            Branch.status == BranchStatus.ACTIVE
        ).scalar() or 0
        
        # Врачи
        total_doctors = db.query(func.count()).select_from(Branch).join(
            Branch.doctors
        ).scalar() or 0
        active_doctors = db.query(func.count()).select_from(Branch).join(
            Branch.doctors
        ).filter(Branch.doctors.any()).scalar() or 0
        ...
        equipment_in_maintenance = db.query(func.count(Equipment.id)).filter(
        ...
        recent_backups = db.query(func.count(Backup.id)).filter(
        ...
        if equipment_in_maintenance > total_equipment * 0.3:  # Более 30% оборудования на обслуживании
