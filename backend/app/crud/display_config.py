...
from app.models.display_config import DisplayBoard, DisplayTheme, DisplayAnnouncement
...
def get_board_announcements(
    ...
    query = db.query(DisplayAnnouncement).filter(DisplayAnnouncement.board_id == board_id)
    
    if active_only:
        query = query.filter(DisplayAnnouncement.active == True)
    
    return query.order_by(DisplayAnnouncement.priority.desc(), DisplayAnnouncement.created_at.desc()).all()

def create_board_announcement(db: Session, announcement_data: Dict[str, Any]) -> DisplayAnnouncement:
