...
def get_photo(db: Session, photo_id: int) -> Optional[DermatologyPhoto]:
    ...
    return (
        ...
        .filter(
            and_(DermatologyPhoto.id == photo_id, DermatologyPhoto.is_active == True)
        )
        .first()
    )
...
def get_patient_photo_stats(db: Session, patient_id: int) -> Dict[str, Any]:
    ...
    # Общий размер файлов
    total_size = (
        db.query(func.sum(DermatologyPhoto.file_size))
        .filter(
            and_(
                DermatologyPhoto.patient_id == patient_id,
                DermatologyPhoto.is_active == True,
    ...
    # Последняя загрузка
    last_upload = (
        db.query(func.max(DermatologyPhoto.created_at))
        .filter(
            and_(
                DermatologyPhoto.patient_id == patient_id,
                DermatologyPhoto.is_active == True,
