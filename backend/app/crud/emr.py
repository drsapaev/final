...
class CRUDEMR(CRUDBase[EMR, EMRCreate, EMRUpdate]):
    ...
    def get_drafts(self, db: Session, *, skip: int = 0, limit: int = 100) -> List[EMR]:
        ...
        return db.query(EMR).filter(EMR.is_draft == True).offset(skip).limit(limit).all()


class CRUDPrescription(CRUDBase[Prescription, PrescriptionCreate, PrescriptionUpdate]):
    """CRUD операции для рецептов"""
    
    def get_by_appointment(self, db: Session, *, appointment_id: int) -> Optional[Prescription]:
    ...
    def save_prescription(self, db: Session, *, prescription_id: int) -> Optional[Prescription]:
    ...
    def mark_printed(self, db: Session, *, prescription_id: int) -> Optional[Prescription]:
    ...
    def get_drafts(self, db: Session, *, skip: int = 0, limit: int = 100) -> List[Prescription]:
        """Получить черновики рецептов"""
        return db.query(Prescription).filter(Prescription.is_draft == True).offset(skip).limit(limit).all()
