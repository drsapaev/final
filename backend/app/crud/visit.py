...
def create_visit(
    ...
    source: str = "desk",
    # Fix C (cart atomicity): владелец транзакции может отключить внутренний
    # commit и закоммитить композицию сам (см. /registrar/cart).
    commit: bool = True,
) -> Visit:
    """
    ...
    Args:
        ...
        commit: закоммитить ли транзакцию внутри функции. True (по умолчанию)
            сохраняет историческое поведение одиночного визита. Код, который
            строит многосущную транзакцию (визиты + invoice + очередь),
            передаёт commit=False и коммитит сам — иначе частичный сбой
            композиции оставляет уже закоммиченные визиты в БД (неатомарно).
    ...
    # Fix C (cart atomicity): commit только по явному запросу владельца
    # транзакции. refresh() работает и без commit — объект уже flush'нут.
    if commit:
        db.commit()
    db.refresh(visit)
    return visit
