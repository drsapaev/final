    ...
    if services:
        for service_data in services:
            visit_service = VisitService(
                ...
                code=normalize_service_code(service_data.get("code")) if service_data.get("code") else None,
...
def get_visit_history(
    ...
    return db.query(Visit).filter(
        Visit.patient_id == patient_id
    ).order_by(desc(Visit.visit_date), desc(Visit.created_at)).limit(limit).all()