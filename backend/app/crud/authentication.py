...
class CRUDLoginAttempt(CRUDBase[LoginAttempt, None, None]):
    ...
    def get_failed_attempts(
        self, db: Session, user_id: int, since: datetime = None
    ) -> int:
        """Получить количество неудачных попыток"""
        query = db.query(LoginAttempt).filter(
            and_(LoginAttempt.user_id == user_id, LoginAttempt.success == False)
        )
...
class CRUDUserActivity(CRUDBase[UserActivity, None, None]):
    ...
    def get_by_activity_type(
        ...
        return (
            db.query(UserActivity)
            .filter(UserActivity.activity_type == activity_type)
            .order_by(desc(UserActivity.created_at))
            .limit(limit)
            .all()
        )
...
class CRUDSecurityEvent(CRUDBase[SecurityEvent, None, None]):
    ...
    def resolve_event(
        self, db: Session, event_id: int, resolved_by: int, notes: str = None
    ) -> bool:
        """Разрешить событие"""
    ...
    def cleanup_old_events(self, db: Session, days: int = 365) -> int:
        ...
        count = (
            db.query(SecurityEvent)
            .filter(
                and_(SecurityEvent.created_at < since, SecurityEvent.resolved == True)
            )
            .delete()
        )
        db.commit()
