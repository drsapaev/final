...
def get_default_ai_provider(db: Session) -> Optional[AIProvider]:
    ...
    return db.query(AIProvider).filter(
        and_(AIProvider.is_default == True, AIProvider.active == True)
    ).first()
...
def create_ai_provider(db: Session, provider: AIProviderCreate) -> AIProvider:
    ...
    if provider.active and not db.query(AIProvider).filter(AIProvider.is_default == True).first():
...
def update_ai_provider(db: Session, provider_id: int, provider: AIProviderUpdate) -> Optional[AIProvider]:
...
def delete_ai_provider(db: Session, provider_id: int) -> bool:
    ...
    if db_provider.is_default:
        other_provider = db.query(AIProvider).filter(
            and_(AIProvider.id != provider_id, AIProvider.active == True)
        ).first()
        if other_provider:
...
def create_prompt_template(db: Session, template: AIPromptTemplateCreate) -> AIPromptTemplate:
...
def get_ai_usage_stats(
    ...
    avg_response_time = sum(l.response_time_ms for l in logs if l.response_time_ms) / len(logs) if logs else 0
    ...
    for provider_name in by_provider:
        provider_logs = [l for l in logs if l.provider and l.provider.name == provider_name]
        successful = len([l for l in provider_logs if l.success])
        by_provider[provider_name]["success_rate"] = (successful / len(provider_logs)) * 100 if provider_logs else 0
...
def update_ai_system_settings(db: Session, settings: Dict[str, Any], user_id: int) -> Dict[str, Any]:
