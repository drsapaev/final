...
class CRUDFile:
    ...
    def search(
        ...
        # Пагинация
        files = (
            query.order_by(desc(File.created_at))
    ...
    def _get_size_ranges(self, db: Session, query) -> List[Dict[str, Any]]:
        ...
        for range_def in ranges:
            ...
            if count > 0:
                result.append({"label": range_def["label"], "count": count})
...
class CRUDFileVersion:
    ...
    def create(
        self, db: Session, *, file_id: int, version_data: dict, created_by: int
    ) -> FileVersion:
...
class CRUDFileShare:
    ...
    def create(
        self, db: Session, *, file_id: int, share_data: FileShareCreate, created_by: int
    ) -> FileShare:
...
class CRUDFileFolder:
    ...
    def get_folder_tree(
        ...
        for folder in folders:
            folder_dict = {
                ...
                "children": self.get_folder_tree(
                    db, user_id=user_id, parent_id=folder.id
                ),
            }
    ...
    def delete(self, db: Session, *, id: int) -> bool:
        ...
        if obj and not obj.is_system:
            ...
            children_count = (
                db.query(FileFolder).filter(FileFolder.parent_id == id).count()
            )
            if children_count > 0:
...
class CRUDFileQuota:
    ...
    def update_usage(
        self, db: Session, *, user_id: int, size_delta: int, files_delta: int
    ) -> bool:
    ...
    def check_quota(
        ...
        *,
        ...
        if (
            ...
            and (quota.used_storage_bytes + additional_size) > quota.max_storage_bytes
        ):
            return (
                False,
                f"Превышена квота по размеру: {quota.used_storage_bytes + additional_size} / {quota.max_storage_bytes} байт",
            )
        ...
        if quota.max_files and (quota.used_files + additional_files) > quota.max_files:
            return (
                False,
                f"Превышена квота по количеству файлов: {quota.used_files + additional_files} / {quota.max_files}",
            )
...
class CRUDFileStorage:
    ...
    def get_default(self, db: Session) -> Optional[FileStorage]:
        ...
        return (
            ...
            .filter(FileStorage.is_default == True, FileStorage.is_active == True)
            .first()
        )
