        ...
        if phone:
            ...
            query = query.filter(
                or_(
                    ...
                    self.model.phone.like(
                        f"%{normalized_phone}"
                    ),  # Совпадение без форматирования
                )
            )
...
def create_patient_from_user(db: Session, user: User) -> Patient:
    ...
    if not patient.last_name or not patient.last_name.strip():
        raise ValueError(
            f"Ошибка сохранения: фамилия пациента не была сохранена (patient_id={patient.id})"
        )
    if not patient.first_name or not patient.first_name.strip():
        raise ValueError(
            f"Ошибка сохранения: имя пациента не было сохранено (patient_id={patient.id})"
        )
...
def normalize_patient_name(
    ...
    if full_name and full_name.strip():
        ...
        return {"last_name": "", "first_name": "", "middle_name": None}
...
def find_or_create_patient(db: Session, patient_data: Dict[str, Any]) -> Patient:
    ...
    has_individual_names = (last_name_input and last_name_input.strip()) or (
    ...
    normalized_last_name = (
        name_parts["last_name"].strip() if name_parts["last_name"] else ""
    )
    normalized_first_name = (
        name_parts["first_name"].strip() if name_parts["first_name"] else ""
    )
    ...
    if not new_patient.last_name or not new_patient.last_name.strip():
        raise ValueError(
            f"Ошибка сохранения: фамилия пациента не была сохранена (patient_id={new_patient.id})"
        )
    if not new_patient.first_name or not new_patient.first_name.strip():
        raise ValueError(
            f"Ошибка сохранения: имя пациента не было сохранено (patient_id={new_patient.id})"
        )
...
def search_patients(db: Session, query: str, limit: int = 10) -> List[Patient]:
    ...
    patients = (
        ...
        .limit(limit * 2)
        .all()
    )  # Берем больше, чтобы отсортировать
