...
from app.schemas.webhook import (
    WebhookCreate, WebhookUpdate, WebhookCallCreate
)
...
class CRUDWebhook:
    ...
    def create(self, db: Session, *, obj_in: WebhookCreate, created_by: int = None) -> Webhook:
    ...
    def get_active_for_event(self, db: Session, event_type: WebhookEventType) -> List[Webhook]:
    ...
    def get_stats(self, db: Session, *, id: int) -> Dict[str, Any]:
        ...
        recent_calls = db.query(WebhookCall).filter(
            and_(
                WebhookCall.webhook_id == id,
...
class CRUDWebhookCall:
    ...
    def get_multi_by_webhook(
        ...
        *,
        ...
        return query.order_by(desc(WebhookCall.created_at)).offset(skip).limit(limit).all()
    
    def get_pending_retries(self, db: Session, limit: int = 50) -> List[WebhookCall]:
        """Получает вызовы, готовые к повтору"""
        return db.query(WebhookCall).filter(
            and_(
    ...
    def cleanup_old(self, db: Session, days: int = 30) -> int:
        ...
        deleted_count = db.query(WebhookCall).filter(
