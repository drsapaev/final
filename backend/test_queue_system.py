...
def test_create_patient(token):
    ...
    try:
        req = urllib.request.Request(patient_url, data=json.dumps(patient_data).encode('utf-8'))
        ...
        with urllib.request.urlopen(req) as response:
            if response.status in [200, 201]:  # 200 OK или 201 Created
                ...
                print(f"✅ Пациент создан: {patient['first_name']} {patient['last_name']} (ID: {patient['id']})")
                return patient['id']
            else:
...
def test_create_visit(token, patient_id):
    ...
    visit_data = {
        ...
        "notes": "Тестовый визит для очереди"
    }
    
    visit_url = f"{BASE_URL}/api/v1/visits/visits"
    
    try:
        req = urllib.request.Request(visit_url, data=json.dumps(visit_data).encode('utf-8'))
        req.add_header('Content-Type', 'application/json')
        req.add_header('Authorization', f'Bearer {token}')
        ...
        with urllib.request.urlopen(req) as response:
            if response.status in [200, 201]:  # 200 OK или 201 Created
                ...
                print(f"✅ Визит создан: ID {visit['id']} для пациента {visit['patient_id']}")
                return visit['id']
            else:
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                print(f"    ❌ Ошибка получения очереди: {response.read().decode('utf-8')}")
                return False
    except Exception as e:
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                print(f"    ❌ Ошибка получения статистики: {response.read().decode('utf-8')}")
                return False
    except Exception as e:
...
def test_appointments(token):
    ...
    today = time.strftime("%Y-%m-%d")
    appointments_url = f"{BASE_URL}/api/v1/appointments/stats?department=THERAPY&date={today}"
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                print(f"    ❌ Ошибка получения статистики приёмов: {response.read().decode('utf-8')}")
                return False
    except Exception as e:
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                print(f"    ❌ Ошибка получения состояния дисплей-борда: {response.read().decode('utf-8')}")
                return False
    except Exception as e:
...
def test_printing(token, visit_id):
    ...
    today = time.strftime("%Y-%m-%d")
    ticket_url = f"{BASE_URL}/api/v1/print/ticket.pdf?department=THERAPY&ticket_number=1"
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                if 'application/pdf' in content_type:
                    print(f"    ✅ Билет PDF сгенерирован (размер: {len(response.read())} байт)")
                    return True
                else:
                    print(f"    ⚠️ Получен не PDF: {content_type}")
                    return False
            else:
                print(f"    ❌ Ошибка генерации билета: {response.read().decode('utf-8')}")
                return False
    except Exception as e:
...
def main():
    ...
    print(f"  ✅ Управление приёмами: {'УСПЕШНО' if appointments_success else 'ОШИБКА'}")
    print(f"  ✅ Дисплей-борд: {'УСПЕШНО' if board_success else 'ОШИБКА'}")
    print(f"  ✅ Система печати: {'УСПЕШНО' if print_success else 'ОШИБКА'}")
    
    success_count = sum([bool(patient_id), bool(visit_id), queue_success, online_success, appointments_success, board_success, print_success])
    total_count = 7
