...
class PaymentCycleTester:
    ...
    def create_payment(self):
        ...
        try:
            req = urllib.request.Request(payment_url, data=json.dumps(payment_data).encode('utf-8'))
            req.add_header('Content-Type', 'application/json')
            req.add_header('Authorization', f'Bearer {self.token}')
            ...
            with urllib.request.urlopen(req) as response:
                if response.status == 201:
                    ...
                    print(f"❌ Ошибка создания платежа: {response.read().decode('utf-8')}")
                    return False
        except Exception as e:
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response:
                if response.status == 200:
                    ...
                    print(f"❌ Ошибка получения транзакций: {response.read().decode('utf-8')}")
                    return False
        except Exception as e:
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response:
                if response.status == 200:
                    ...
                    print(f"❌ Ошибка получения визита: {response.read().decode('utf-8')}")
                    return False
        except Exception as e:
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response:
                if response.status == 200:
                    ...
                    print(f"  - Всего транзакций: {summary.get('total_transactions', 'N/A')}")
                    return True
                else:
                    print(f"❌ Ошибка получения сводки: {response.read().decode('utf-8')}")
                    return False
        except Exception as e:
