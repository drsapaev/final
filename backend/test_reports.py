...
def test_reports_summary_different_departments(token):
    ...
    periods = [
        ("сегодня", datetime.now().strftime("%Y-%m-%d"), datetime.now().strftime("%Y-%m-%d")),
        ("вчера", (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d"), (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")),
        ("неделя", (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d"), datetime.now().strftime("%Y-%m-%d")),
        ("месяц", (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), datetime.now().strftime("%Y-%m-%d"))
    ]
    ...
    for period_name, date_from, date_to in periods:
        try:
            reports_url = f"{BASE_URL}/api/v1/reports/summary?from={date_from}&to={date_to}"
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 422:
                ...
                print(f"    ❌ Неожиданный статус без параметра 'from': {response.status}")
                return False
...
def test_reports_summary_invalid_parameters(token):
    ...
    try:
        reports_url = f"{BASE_URL}/api/v1/reports/summary?from=invalid-date&to=2025-08-29"
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 422:
                ...
                print(f"    ❌ Неожиданный статус с неверным форматом даты: {response.status}")
                return False
    except urllib.error.HTTPError as e:
        if e.code == 422:
            print("    ✅ Правильно отклонён запрос с неверным форматом даты (422)")
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                ...
                print(f"    ❌ Не удалось получить отчёт для анализа: {response.status}")
                return False
    except Exception as e:
...
def main():
    ...
    tests = [
        ...
        ("Отчёты для разных периодов", lambda: test_reports_summary_different_departments(token)),
        ("Отчёты для разных дат", lambda: test_reports_summary_different_dates(token)),
        ("Отчёты с отсутствующими параметрами", lambda: test_reports_summary_missing_parameters(token)),
        ("Отчёты с неверными параметрами", lambda: test_reports_summary_invalid_parameters(token)),
