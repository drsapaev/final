...
**Live smoke note:** backend `18000` is up; authoritative AI/MCP smoke gate is `GET /api/v1/mcp/health` (returned `overall=healthy` on `2026-03-29`). `GET /api/v1/mcp/status` is informational and may still report `healthy=false` until the cached health state refreshes. Browser proof artifacts for `/admin/ai-settings` and `/admin/ai-analytics` are generated locally under ignored `output/playwright/`; they are not required as committed files.
