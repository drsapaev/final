...
|---|---|---|---|---|---|---|---|---|---|
| `backend/app/api/v1/endpoints/registrar_integration.py` | `create_queue_entries_batch()` | Mounted runtime batch allocator endpoint | `patient_id`, `source`, `services[]` | `success`, `entries[]`, `message` | Yes, via `queue_service.create_queue_entry(auto_number=True)` | Yes, router-level pre-check by `patient_id + specialist/day/service queue_tag + waiting/called` | Yes | No | No direct billing/payment side effects |
...
| `backend/app/repositories/queue_batch_repository.py` | `resolve_specialist_user_id()` | Batch-only specialist normalization helper | `specialist_id` | `(doctor_id, converted)` | No | No | No | No | No |
| `backend/app/repositories/queue_batch_repository.py` | `find_existing_active_entry()` | Batch-only duplicate lookup helper | `specialist_id`, `day`, `patient_id` | existing `OnlineQueueEntry` or `None` | No | Yes, canonical active statuses `waiting/called/in_service/diagnostics` | No | No | No |
...
### 2. `specialist_id` input is doctor-profile based

The current mounted runtime treats `specialist_id` as `doctors.id`.
...
### 5. Existing entry reuse is status-sensitive and queue-tag-sensitive
...
The mounted lookup uses the `DailyQueue` resolved from the first service `queue_tag`.
An existing row in a different queue_tag bucket is a different queue claim.
