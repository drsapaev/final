# Next Execution Unit After EMR AI API Service Cleanup
...
- continue the protected EMR / clinical audit/proof lane with
  `backend/app/services/emr_ai_enhanced_api_service.py`
...
Why:
- `emr_ai` is now resolved as a proof-backed detached duplicate pair
- the remaining cleanup-capable protected inventory is still in the EMR /
  clinical bucket
- `emr_ai_enhanced` is the next clean entry point in that remaining bucket
