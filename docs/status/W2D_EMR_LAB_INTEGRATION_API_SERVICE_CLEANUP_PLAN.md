# EMR Lab Integration API Service Cleanup Plan

Scope:
- review detached `backend/app/services/emr_lab_integration_api_service.py`
- review detached `backend/app/repositories/emr_lab_integration_api_repository.py`
- prove the live `/api/v1/emr/lab/*` contract before any deletion
- avoid broader EMR/lab workflow changes in this slice

Evidence:
- `backend/app/api/v1/api.py` mounts
  `backend/app/api/v1/endpoints/emr_lab_integration.py`
- `backend/openapi.json` publishes the live `/api/v1/emr/lab/*` routes
...
Out of scope:
- rewriting `EMRLabIntegrationService`
- changing EMR/lab integration semantics
- redesigning frontend lab-integration UX
