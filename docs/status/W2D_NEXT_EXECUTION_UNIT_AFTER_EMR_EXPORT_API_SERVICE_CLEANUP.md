# Next Execution Unit After EMR Export API Service Cleanup
...
Why:
- `emr_export` is now resolved as a proof-backed detached duplicate pair
- the remaining cleanup-capable protected inventory is still in the EMR /
  clinical bucket
- `emr_versioning_enhanced` is the next clean entry point in that remaining
  bucket
