# Next Execution Unit After EMR Lab Integration API Service Cleanup
...
- continue the protected EMR / clinical audit/proof lane with
  `backend/app/services/emr_ai_api_service.py`
...
Why:
- `emr_lab_integration` is now resolved as a proof-backed detached duplicate
  pair
- the remaining cleanup-capable protected inventory is still in the EMR /
  clinical bucket
- `emr_ai` is the next clean entry point in that remaining bucket
