...
| `backend/app/api/v1/endpoints/registrar_wizard.py` | non-confirmation queue creation branches | Mounted confirmation bridge and same-day wizard create-branch handoff are migrated; shadow/unmounted wizard branches still need compatibility review | Mixed logic | High-risk allocator migration |
