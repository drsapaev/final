...
- **PR-UI-17-5 ModernTabs→Tabs rename + stylelint spacing gate + ESLint forbidden imports** (#2955 → `e7649b287`, 31.08.2026): renamed `navigation/ModernTabs.tsx` → `Tabs.tsx` (33 consumers updated); added stylelint `declaration-property-value-allowed-list` rule for spacing (rejects hardcoded 3px/5px/7px/13px/18px outside `--mac-spacing-*` scale); added ESLint forbidden-imports rule (rejects imports of deleted/legacy modules: `forms/Modern*`, `components/Icon`, `assets/iconsMap`, etc.) — forward-protection against regression. — plan §PR-UI-17 items 8, 9, 11. **Post-merge VERIFIED (5 invariants @ `e7649b287`, verified on origin/main @ `08253d097`):** origin/main contains squash SHA ✓; parent of squash = `b247f00aca` (PR-UI-17-4t tm_ tail, in main history) ✓; scope = 21 files +1216/−813 (ModernTabs→Tabs rename 33 consumers + stylelint gate + ESLint forbidden-imports rule + CSS reformatting) ✓; CI 19/19 terminal = 16 success + 3 expected skipped + 0 failures, combined=success ✓; Regression Audit Gate = success on merge commit ✓.
...
**M-8 consolidation inventory (живое состояние):** 2FA — живы Manager (×3)/Wizard (×2)/SMSEmail2FA (×1)/TwoFactorVerify (×1), dead×2 удалены; PWA install prompt — все 3 копии + barrel + транзитивно-мёртвый ConnectionStatus удалены, живых не осталось; role-гварды — каноническая тройка routeGuards/RoleGate/useRoles жива, dead×2 + barrels×2 удалены; **Telegram Manager — root `components/TelegramManager.tsx` (2 496 LOC) runtime-DEAD** (0 runtime-импортёров; читается только 2 fs-тестами) — план считал его «root-живым»; кандидат на удаление вместе с fs-тестами в cleanup-хвосте (отдельное решение).
...
**Итог:** PR-UI-17 ✅ COMPLETE, 5 SP кредитованы. Портфолио **18/19 PR · 97/103 SP**. Sprint 6 остаток — PR-UI-18 (🟡 IN PROGRESS параллельной сессией). Follow-up-кандидаты (вне PR-UI-17): root TelegramManager decommission; common/Modal консолидация (split/MIGRATE); macos/Icon → lucide-миграция (247 потребителей).

**Post-merge VERIFIED (consolidated, v2.7.1, 31.08.2026, verified on origin/main @ `08253d0979fb1652c84421394657a99f44c983d0`):** All 6 PR-UI-17 increments pass 5 invariants per rule §16. INV1 (squash SHA in main history) ✓ for all six: `94ebcb04c` / `2e1fba60d` / `d9ffa2133` / `62e95e298` / `b247f00ac` / `e7649b287`. INV2 (single-parent, parent in main history) ✓ for all six; parent chain: `d461a06cd9` → `94ebcb04c7` → `2e1fba60df` → `d9ffa2133b` → ... `baf0b9492a` (PR-UI-18-1 baseline) → `62e95e2984` → `b247f00aca` → `e7649b287a` (each ancestor-of-main). INV3 (exact scope, non-empty files set + insertions/deletions match) ✓ for all six (17/33/18/4/4(−355)/21 files respectively). INV4 (CI terminal 0 failures, combined=success) ✓ for all six: CI runs = 33/35/35/33/35/19; failures = 0/0/0/0/0/0; combined state = success. INV5 (Regression Audit Gate = success on merge commit) ✓ for all six. PR-UI-17 fully **MERGED; VERIFIED** at `08253d0979fb1652c84421394657a99f44c983d0`. (Note: origin/main advanced past `08253d097` to `88af44c48` after verification was performed — PR-UI-18-5 #2964, Plan-SSOT v2.7 #2968, §4.1.18 addendum #2973, ops functional smoke #2974 merged by parallel sessions; verification at `08253d097` is persistent: presence of all 6 squash SHAs in main history is invariant under subsequent merges.)
