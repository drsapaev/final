...
| **3. State patterns** | 5-6 | PR-UI-07, 08, 09 | Loading/Empty/Error + убрать glass + DataTable | 17 SP | 07 ✅ (+серия 07a) · 08 ✅ (#2838) · 09 🟡 (09a ✅ #2843 → §4.1.6; 09b ✅ #2848 → §4.1.7; 09c ✅ COMPLETE — #2857/#2860/#2861/#2862 → §4.1.8; 09d ✅ #2870 → §4.1.9; 09e-1 ✅ #2872 → §4.1.9; 09e-2 LOC-редукция — future-work) |
...
1. **09e-2 LOC-редукция таблиц** (EAT 2 004 → ~400; QueueTable 259 → ~100; RefundRequestsTable 432 → ~150): отдельное архитектурное решение пользователя; read-only assessment выполняется вне плана; автоматический старт НЕ разрешён.
...
3. Sidebar-рендер читает label через `useTranslation()`; **fallback**: ключ отсутствует → текущий русский текст (навигация не ломается при пропущенном ключе)
...
- ✅ Sidebar отображает локализованные labels минимум в ru + en (browser-проверка), переключение языка меняет навигацию без перезагрузки
...
| **H-10** Dark-тема непокрыта (182 isDark-ветвления) | Токен-миграция доменов | Sprint 5 (ongoing) | 🟡 ACTIVE | ratchet noFallback 12003→11883 монотонно |
...
| **M-2** ~2 600 inline-стилей | Ratchet-политика | ui-baseline `--check` gate | ✅ ACTIVE | noFallback 12003→11883, 15+ merges без ухудшения; baseline 163/330/12003 не тронут |
...
| **M-8** 4 role-гварда / 6 файлов 2FA / 3 PWA / 2 TelegramManager | Консолидация по одному на сущность | PR-UI-17 | ⬜ PLANNED | §7 PR-UI-17 ownership-сводка |
...
**Итог v1.8:** 36 findings → 0 ORPHANED. Из них: ✅ DONE/ACTIVE — 12; 🟡 PARTIAL — 15 (все с владельцем); ⬜ PLANNED — 15 (все в спринтах); 🔁/📌 расхождения с аудитом — 3 (H-2, H-3, H-6 — все задокументированные решения, не потери; H-3 — кандидат пересмотра в Sprint 5). C-6 — единственная находка, бывшая orphaned с 25.08, закрыта введением PR-UI-19.
