...
### 3.12. Dentist visit follow-up (PR #3434)

| Файл | LOC | Действие | Причина | PR |
|---|---:|---|---|---|
| `src/components/dental/DentalVisitScreen.tsx` | 1302 | **KEEP** | Единый экран приёма хранит разделы протокола в `specialty_data.visit_protocol`; использует общий нормализатор из `VisitProtocol.tsx`. | #3434 |
| `src/components/dental/VisitProtocol.tsx` | 1130 | **KEEP — shared helper** | `normalizeVisitProtocolRecord` остаётся зависимостью встроенного редактора после удаления отдельного окна протокола. | #3434 |

Эти записи дополняют существующую структуру dentist-модулей из PR-UI-15; Sprint/SP scope и исторические LOC-метрики не меняются.
...
**Остаток после v2.7:** PR-UI-18 ✅ **COMPLETE** — 5 SP кредитованы (items 1/2/4 исполнены 5 инкрементами двух сессий; item 3 амендирован — mandate с ложной предпосылкой, функциональный интент закрыт существующим блокирующим Playwright visual-diff-гейтом, см. §4.1.18). Портфолио **19/19 PR · 102/103 SP** (residual 1 SP — историческая бухгалтерия ранних срезов v1.x, не покрытие: все 19 PR COMPLETE). **Sprint 6 CLOSED. Все 19 PR UI_REMEDIATION_PLAN исполнены.** Follow-up-кандидаты (вне плана): ~~root TelegramManager decommission~~ — ОТЗЫВАН v2.8 re-audit'ом (компонент LIVE, не runtime-dead; см. §4.1.19; пере-классифицирован в кандидат LOC-декомпозиции god-компонента 2 496 LOC); ~~common/Modal консолидация~~ — исполнена v2.9 (track 2, A+: runtime-dead декомиссия провайдера 513 LOC + тримминг мёртвых экспортов hooks/useModal 304→59 LOC; см. §4.1.20); ~~macos/Icon → lucide-миграция~~ — ИСПОЛНЕНА v2.10-v2.12 (track 3 CLOSED: 3-1 навигационный контракт §4.1.21 + 3-2 доменные потребители §4.1.22 + 3-3 декомиссия Icon.tsx 546 LOC + barrel + ESLint-гард §4.1.23; Icon-систем 2→1, end-state §3.3 достигнут); ~~color-contrast remediation pixel-locked поверхностей с re-capture baseline (axe-находки §4.1.18)~~ — ИСПОЛНЕНО v2.13 (track CC CLOSED: §4.1.24; a11y-baseline → {}); ~~axe-расширение на авторизованные QA-harness поверхности~~ — ИСПОЛНЕНО v2.14 (track AXE-EXP CLOSED: §4.1.25; a11y-baseline-auth → {}); RoleGate Receptionist policy (§4.1.10 finding #1, по-прежнему deferred).

**Остаток после v2.15:** портфолио неизменно **19/19 PR · 102/103 SP** (residual 1 SP — историческая бухгалтерия v1.x). ~~RoleGate Receptionist policy (§4.1.10 finding #1)~~ — ЗАКРЫТО v2.15 (track REC: §4.1.26 — роль выводится как deprecated alias, write-freeze + alias removal; гейты route/RoleGate согласованы на deny, widening НЕ применён). Следующие кандидаты очереди (все read-only inventory / policy decisions, требуют явного старта): (1) roles-table subsystem inventory (public.roles = 0 rows — кто реально использует; dead/deferred subsystem?); (2) Manager RBAC vocabulary inventory (production Manager = 1); (3) P-014 policy decision (Cashier/Lab/специалисты на /clinical/appointments: route пускает, page RoleGate deny); (4) Nurse policy (алиас nurse→doctor, frontend-only); (5) E-4 backend Receptionist-alias removal (после compatibility audit, gate см. §4.1.26).

**Остаток после v2.16:** портфолио неизменно **19/19 PR · 102/103 SP** (residual 1 SP — историческая бухгалтерия v1.x). Закрыты этой сессией: ~~elevated-контрастный долг~~ (ELEV, §4.1.27 — dark secondary #a6a6ad, elevated 4.69:1, guard-ассерт, visual zero-delta); ~~E-4 backend Receptionist-alias removal~~ (§4.1.27 — removal gate верифицирован 4/4, alias демонтирован по всем 10 классам поверхностей, deny-контракты пинят, миграционная нормализация сохранена; Receptionist полностью выведен из RBAC-словаря). ~~roles-table subsystem inventory~~ и ~~Manager RBAC vocabulary inventory~~ — ИСПОЛНЕНЫ (read-only, вердикты в §4.1.27: roles-table = де-факто DEAD mounted-but-unused-поверхность с behavior-neutral decommission-entailment; Manager write-frozen + grants-zero, M-2 заблокирован только ops-решением по smoke_manager id=20 — DEACTIVATE + верификация is_superuser=false). **Остающиеся кандидаты очереди (требуют явного бизнес-выбора пользователя; брифы decision-ready в §4.1.27):** (1) P-014 policy decision — options A (widen page + data-layer решение) / B (tighten route, регресс PR-UI-04b cashier UX) / C (документировать); (2) Nurse policy — options N-1 (keep) / N-2 (keep + scope-limit) / N-3 (REC-3-style removal); **prerequisite для N-3**: production Nurse count никогда не записывался; (3) ops-хвост Manager M-2 (вне кода): деактивация smoke_manager; (4) roles-table decommission — отдельный behavior-neutral трек по entailment-списку §4.1.27 (требует явного старта).

**Остаток после v2.17:** портфолио неизменно **19/19 PR · 102/103 SP** (residual 1 SP — историческая бухгалтерия v1.x). Закрыто: ~~ops-хвост Manager M-2~~ — деактивация smoke_manager исполнена ops-шагом 04.09.2026 (host-side guarded transaction; логин 401, контроль smoke_doctor 200) и ~~M-2 vocabulary closure~~ — кодовая часть исполнена этим инкрементом (§4.1.28: enum + filter-окно + зеркала + тест-переработка; post-deploy хвост — Stage 0 SQL-инвентарь остатков данных + опциональный token-purge + canonical redeploy, вне репо-кода). **Остающиеся кандидаты очереди (требуют явного бизнес-выбора пользователя):** (1) P-014 policy decision (options A/B/C, бриф §4.1.27); (2) Nurse policy (N-1/N-2/N-3; prerequisite — production Nurse count); (3) roles-table decommission (behavior-neutral, entailment-список §4.1.27); (4) backend request-state auth-семейство (require_staff/require_active_user/GET /users/{id}/profile 401 — найдено M-2 tombstone-тестом, pre-existing, владелец backend/auth-трек).
...
**Codex review round (PR #2983, round 1, head `2b8837ef`):** 0 замечаний — «Didn't find any major issues. Hooray!» (chatgpt-codex-connector, триггер `@codex review`; инлайн-комментариев 0; CI 8/8 workflows terminal success — PR/PR Lifecycle/PR Review Quality Gate/Regression Audit/i18n/Unified/Gitleaks/Security Scan). Итераций ревью: 1; fixes: не требуются. Drift на цикле: c40733a6 (#2982, backend-only, 0 frontend-файлов) — DISJOINT, продолжено по FAST-протоколу без ребейза.


### 4.1.22 Follow-up track 3-2: domain consumers — macos-Icon → прямые lucide-импорты (v2.11, 01.09.2026)

**Контекст:** инкремент 2 из 3 трека (решение пользователя: strategy B, доменные батчи в одном PR если управляемо — исполнено одним PR; strokeWidth 2 = lucide-default; Q3=bugfix YES; MacOSDemo мигрируется). Скоуп: все доменные потребители macos-Icon вне навигационного контракта 3-1.

**Исполнение (33 файла, scripts t3_2_phase_a/b/c.py, assertion-guarded):** Phase A (generic-трансформер: литеральные name= + условные name={c ? 'a' : 'b'} + size 16/20/24/32 + aria-hidden + import-хирургия) — 25 файлов; Phase B (конфиг-строки → LucideIcon refs + рендеры) — patientSections.ts, wizardUtils.ts, QueueTable STATUS_MAP+empty-states, AdminSecurityDashboard/LabPanel табы, PatientPanel табы/секции, ModernStatistics карточки, MacOSDemo демо-табы ×3 + icon-gallery (строки → компоненты с labels), PanelEmptyState API `icon?: LucideIcon` + 4 файла-вызывающих, AppointmentWizardV2 string-comparison-цепочка (4 ветки) → прямая компонента; Phase C — 3 reverse-guard контракт-теста переписаны в forward-guards (LabResultsSection: lucide-обязательство + запрет macos-Icon + 4 канонических компонента; LabQueueWorkbench: EyeOff/Eye; ContentTab: FileDown) с REVERSAL-документацией (UX-AUDIT-FIX6/11 эпоха → §3.3/§4.1.17 end-state → Track 3 явное supersession — архитектурный интент «одна каноническая система» сохранён, направление скорректировано).

**Инцидент качества (пойман red-check):** Phase A v1 regex `<Icon\b(.*?)/>` c DOTALL проглотил 1 516 строк DermatologistPanelUnified через комментарий `// S-M-2 ... macos <Icon>` до первого настоящего `/>`. Файл восстановлен из HEAD (`git checkout`); фикс: `[^>]*?` без re.S + pre-pass переписывания 7 stale-комментариев (grep-clean: без литерала `<Icon`) + SUSPECT-флаг на attr-регионы с import/>15 новых строк. Урок: JSX-regex поверх кода с иконками-в-комментариях требует pre-pass очистки комментариев; diff-stat аудит обязателен между раундами.

**Латентные баги закрыты (intentional UX-bugfix):** flask/syringe/clipboard (wizardUtils — иконки категорий визарда рендерились questionmark), checkmark.shield (ASD compliance-таб — questionmark), `chart('misc.ms_bar')` (ModernStatistics — сломанный template-literal в name= → questionmark в заголовке статистики). Плюс визуальная семантика: 3 late-находки из 3-1 отчёта (clock.arrow.circlepath→History и др.).

**Гейты (Tier-1, все зелёные):** tsc 0; eslint 0 errors (33 файла, warnings pre-existing класс); vitest **1 642/1 642** (200 файлов, 0 дельта — включая переписанные контракт-тесты); build 30.8s; storybook PASS; ratchet PASS (0 regressions); icon-controls 0 findings; UX/Visual canonical 6 specs × chromium retries=0 — **93/93 PASS, 0 baseline-обновлений** (иконки ≤0.9% пикселей < порога 0.01; causality DOM-probe: /lab = 23 lucide-SVG / 0 mac-icon, probe удалён). Machine-verify: **0 импортёров macos-Icon в src** (multiline-aware скан обоих видов импортов), 0 `<Icon name=` литералов вне Icon.tsx.

**Codex review round (PR #2985, round 1, head `a9027ff9`):** 0 замечаний — «Didn't find any major issues. :+1:» (chatgpt-codex-connector, триггер `@codex review`; инлайн-комментариев 0; CI 8/8 terminal success). Итераций ревью: 1; fixes: не требуются.


### 4.1.23 Follow-up track 3-3: macos/Icon decommission + forward-guards — трек CLOSED (v2.12, 01.09.2026)

**Контекст:** финальный инкремент трека 3. Перед декомиссией — machine-пруфы consumer-dead: (P1) 0 импортёров macos-Icon в src (multiline-скан СТАТИЧЕСКИХ named + barrel + ДИНАМИЧЕСКИХ lazy()-импортов — урок §4.1.19); (P2) 0 `<Icon name=` литералов вне реверс-документаций 2 контракт-тестов (намеренные строки-«было» в REVERSAL-комментариях); (P3) 0 SF-конфиг-строк `icon: '<sf>'` в src; (P4) 0 fs-читателей Icon.tsx в тестах (iconSource-чтение удалено в 3-2 Phase C).

**Исполнение (t3_3_decommission.py, assertion-guarded):** git rm `src/components/ui/macos/Icon.tsx` (546 LOC: 116-entry ICONS-карта inline-SVG-путей, size/color-маппинги, multicolor/monochrome, questionmark-fallback, мёртвые useTheme/useTranslation-вызовы); barrel `ui/macos/index.ts` — экспортная строка заменена комментарием-эпитафией с указанием канона; ESLint `no-restricted-imports` register (блок PR-UI-17-5, eslint.config.js) — новый pattern `**/components/ui/macos/Icon` level error с сообщением «use direct lucide-react imports (§3.3)» — forward-гард против реинтродукции. Alias/шим НЕ сохранён (явное решение пользователя: permanent shim FORBIDDEN).

**Гейты (Tier-1, все зелёные):** tsc 0; eslint 0 errors (включая самопроверку нового forbidden-правила — 0 нарушений); vitest **1 642/1 642**; build 28.8s; storybook build PASS; ratchet PASS (inlineStyles 2 427→2 424, −3 — выражения удалённого файла; inlineStyleFiles 210, −1; unreferencedFileCount 93 — файл был referenced через barrel, исчез из скана без счётной дельты); icon-controls 0 findings; UX/Visual canonical 6 specs × chromium retries=0 — **93/93 PASS, 0 baseline-обновлений** (baseline-файлы нетронуты — AC «baseline file itself unchanged»).

**Финальные AC трека 3 (user decision framework):** macos/Icon.tsx absent ✓; barrel export absent ✓; live importers = 0 ✓ (pre-proven P1 + post-deletion rescan в скрипте); `<Icon name=` consumers = 0 ✓; SF-nav-строки = 0 ✓ (3-1); questionmark-fallbacks из инвентаря 3-х треков = 0 ✓ (5+5+10 имён закрыты); reverse-guards rewritten ✓ (3 теста, 3-2); forward ESLint guard active ✓; tsc/tests/e2e/a11y/visual/ratchet GREEN ✓; baseline files unchanged ✓. **Icon-систем 2→1 — end-state §3.3 и Приложения A (lucide-react единственная система) достигнут.**

**Codex review round (PR #2986, round 1, head `e7308ffb`):** 1 inline P2 — ВАЛИДЕН и исполнен: ESLint-паттерн `**/components/ui/macos/Icon` ловил только прямой импорт удалённого модуля, но НЕ named-import из barrel (`import { Icon } from '../ui/macos'`) — фактический путь регрессии (баррель был реальным маршрутом потребителей 3-2). Фикс: 9 точных `paths`-entries с `importNames: ['Icon']` по всем barrel-спецификаторам репо + exact `./Icon`; верификация в обе стороны (negative: легитимные импорты 0 errors; positive: probe-файл импорта `{ Icon }` из barrel + `./Icon` → 2 no-restricted-imports errors, probe удалён). Ответ в PR-ветке; finalize-commit в CI round 2. Round-2 CI поймал lint-failure: importNames-рестрикции флагуют и `export * from './macos'` (star-импорт статически не исключить) — ui/index.ts:13 реэкспортировал кит star'ом = вектор тихой пропагации. Фикс-2: star → ЯВНЫЙ список 44 имён + 4 типа (реинтродукция Icon в macos-баррель больше не пропагирует в ui-баррель вовсе); importNames расширены на ui-баррель-спецификаторы (итого 14 entries). Реверифицировано: tsc 0, lint 0 errors, vitest 1 642/1 642, build PASS; positive-probe: 3/3 пути регрессии (macos-barrel, ui-barrel, ./Icon) флагуются.


### 4.1.24 Follow-up track CC: color-contrast remediation — трек CLOSED, a11y-baseline → {} (v2.13, 02.09.2026)

**Контекст:** follow-up-кандидат из §4.1.18 (остаток после v2.7): «color-contrast remediation pixel-locked поверхностей с re-capture baseline (axe-находки §4.1.18)». Трек исполнен 5 инкрементами (CC-1..CC-5), каждый независимо смержен и верифицирован по §16.

**Диагностика (старт трека):** 71 axe-узел color-contrast на 7 route:theme-ключах; корневые причины: (1) легаси `dark-theme-visibility-fix.css` форсил `color: var(--dark-text-primary) !important` (= --mac-bg-secondary #2c2c2e — ФОНОВОЕ значение как чернила) на все кнопки в dark + `button * { color: inherit !important }`, при непарном background-color → инки 1.04–3.47:1 на всём приложении в dark (проб /admin: 41/41 кнопок dark-on-dark; pr18-admin-dark sidebar 4% bright-пикселей); (2) white-ink филлы на base-акцентах ниже 4.5:1 (#007aff 4.02 / #ff453a 3.41 / #30d158 2.02 / #ff9f0a 2.06); (3) landing-декор: акцент/muted-инки на акцент-tint'ах (~2:1); (4) display-board light/medical-темы смешивали константные тёмные чернила с app-theme-зависимыми токенами (dark-приложение → 1.04:1); (5) MUI textSecondary на warning-tint (1.59:1); (+) dark --mac-text-secondary #8e8e93 на bg-content #2c2c2e = 4.27:1.

**Инкременты (все MERGED-VERIFIED, squash-SHA канонические):**
- **CC-1** (#3011 → `4de425eb0`): декомиссия 2 button-правил легаси-патча + dark --mac-text-secondary #8e8e93→#98989d (4.85:1); Sidebar активный пункт → --mac-nav-item-active-text (blue-on-blue 1.0:1 был невидим в ОБОИХ темах — PR-49 !important solid bg + inline accent-инки, пиксельно доказано); strong-токены (#0066d6 5.42 / #d92c20 4.85 / #147a38 6.14) + QueueJoin recovery-кнопки; +4 fs-contract guard-теста (darkThemeInk.contract.test.ts); 13 baseline PNG re-capture (A/B-причинность + byte-stability). a11y-baseline: queue-join:light удалён. 71→42 узла.
- **CC-2** (#3013 → `b3178cded`): Button.tsx primary/danger + success/warning/danger/info overrides → strong-токены (link оставлен на base accent — dark-линки регрессировали бы 3.45→2.41, отдельный латентный класс, документирован); .mac-button--primary класс; --mac-warning-strong #a86200 (4.76); landing workflow-marker. 29 PNG force-regen (дельта ниже per-pixel YIQ-порога гейта 0.084<0.2 — machine-пруф: 40/40 против СТАРЫХ baseline; пиксельные пары ровно #007aff→#0066d6). a11y-baseline: payment-success:light+dark удалены. 42→18.
- **CC-3** (#3014 → `4478ed4ab`): landing tint-чипы → primary-инки (workflow-node ×6/тему, live-badge span — зелёный остаётся на иконке, flow-card p — комбинированный селектор РАЗДЕЛЁН, обычные карточки на muted, language-option--active — латент того же класса). 4 pr16-landing PNG re-capture. a11y-baseline: landing:light+dark удалены. 18→4.
- **CC-4** (#3015 → `f70cdef8a`): --displayboard-{light,dark,medical}-* токены (canonical tokens.css; значения = resolved-значениям light-приложения — pr18-display-board-light 0 пикселей дельты против ОРИГИНАЛА, machine-пруф); DisplayBoardUnified themes → var()-ссылки (tsxHex ratchet не тронут — первая итерация с hex-константами дала tsxHex 373→374 REGRESSION → переработано в токены); displayboard.css инки-константы по фону (announcement/badges/current-call/contrast-mode — было var(--mac-bg-primary), флипало с app-темой). pr18-display-board-dark re-capture (единственный FAIL >1%). a11y-baseline: display-board:dark удалён. 4→1.
- **CC-5** (payment-cancel reason): MUI textSecondary на warning-tint → primary-инки; a11y-baseline → **{}** (все 12 аудитов 6 маршрутов × light/dark = 0 нарушений WCAG 2.x A/AA); _meta.resolution фиксирует монотонную ужимку 7→0.

**Итог трека CC:** axe color-contrast 71→0 узлов; e2e/a11y-baseline.json — ПУСТОЙ (гейт = zero-violation, shrink-only ratchet в силе); 3 новых guard-контракта (кнопочные инки, парность !important, dark text-secondary ≥4.5:1); палитра расширена canonical-токенами (strong-семейство + board-палитры) без единого нового файла; все визуальные базлайны координированно re-capture с A/B-причинностью и byte-stability. **Трек CLOSED.** Остаток follow-up-кандидатов (вне плана): axe-расширение на авторизованные QA-harness поверхности (латентные находки: цветной текст статусов board на нейтральных карточках ~1.9–3.4:1, link-инки 3.45–4.02:1, MUI-тексты на tint'ах), RoleGate Receptionist policy (§4.1.10 finding #1, по-прежнему deferred).

### 4.1.25 Follow-up track AXE-EXP: axe-гейт на авторизованных QA-harness поверхностях — трек CLOSED, a11y-baseline-auth → {} (v2.14, 02.09.2026)

**Контекст:** follow-up-кандидат из §4.1.24 (остаток после v2.13): «axe-расширение на авторизованные QA-harness поверхности». Трек исполнен 6 инкрементами (AXE-EXP-1..6), каждый независимо смержен и верифицирован по §16; инкременты 1–5 исполнены параллельной сессией, закрывающий инкремент 6 — данной сессией.

**Диагностика (старт трека, AXE-EXP-1 probe на main `aca7d96a8`):** 236 violation-узлов / 34 аудита (17 авторизованных маршрутов × light/dark: 6 role-homes + 2 specialty + 9 admin-family; через `installAuthenticatedQaHarness` — mocked, backend-less, deterministic): 200 color-contrast (⌘K-чип поиска ×34, палитра-триггер и sidebar-бейдж в dark, `.mac-badge` white-on-color филлы, inactive tab/segment лейблы, lab lqw-*, misc) + 36 ARIA (aria-required-parent 22: MacOSTab role=tab без tablist; aria-valid-attr-value 6: SegmentedControl dangling aria-controls; aria-required-children 4: AppointmentSummaryBar; aria-prohibited-attr 2: admin-kpi-grid aria-busy; label 2: cashier date inputs).

**Инкременты (все MERGED-VERIFIED, squash-SHA канонические):**
- **AXE-EXP-1** (#3017 → `f73273a325b5ab25dbb836a02bb87c3c92660381`): новый axe-аудит `e2e/a11y-axe-authenticated.spec.ts` (17 маршрутов × 2 темы = 34 аудита, WCAG 2.x A/AA tags, shrink-only baseline-ratchet: fail на NEW правилах И на stale-записях; diagnostic NDJSON-dump через `A11Y_AUTH_DUMP`; контракт mounted-shell/data-theme/style-application/content-mount против false-clean при параллельной нагрузке) + seeded baseline `e2e/a11y-baseline-auth.json` (236 узлов, каждая запись документирована в `_meta`) + попутный фикс WebhookManager infinite load-loop.
- **AXE-EXP-2** (#3018 → `06be8b8ddb7db7ee040ad609e57e6b7f12ea812a`): универсальный app-chrome color-contrast класс — ⌘K hint-чип (на всех 34 аудитах), ⌘K command-palette триггер (его фон-переменная была UNDEFINED → light-литерал в ОБЕИХ темах, 2.6:1), sidebar inactive count-badge; канонический токен **`--mac-text-on-tertiary`** (tokens.css, 5 блоков incl. prefers-contrast:high; light = значение text-secondary — zero visual delta в light, dark #c7c7cc 6.74:1). Baseline: 34→24 ключа, 236→172 узла.
- **AXE-EXP-3** (#3020 → `34e1b833e00de876d13c22618c8878c3aa0f3427`): badge-fill класс — крупнейший кластер (~112 из 236 узлов): Badge filled-варианты несли WHITE ink на BASE-акцентах (4.01/2.05/2.02/3.41:1) → CC-2 strong-семейство (5.42/4.85/6.14/4.76:1, white ink сохранён); `info`-вариант: white на light-tint 1.33:1 → theme-independent тёмно-синий `--mac-accent-blue-active` (5.9:1); MacOSTab tab.badge того же класса. Baseline: 24→22 ключа, 172→60 узлов.
- **AXE-EXP-4** (#3021 → `9f9bf2677dedb8aed0599756b48dc2ae2266dfe4`): tab/link-инки — канонический токен **`--mac-link-ink`** (light #0066d6 5.42:1 / dark #5ac8fa 8.97:1; закрывает латентный link-ink класс, документированный с CC-2): patient `.pp-tab-active`; inactive tab/segment → `--mac-text-on-tertiary`; lab.css status-filter/sort-label; admin.css tertiary-утилиты → secondary-инки. Scope-note: `admin-services:dark` (2 узла) — ОТДЕЛЬНАЯ корневая причина (легаси white-table патч), оставлен в baseline для закрывающего инкремента. Baseline: 22→17 ключей, 60→38 узлов.
- **AXE-EXP-5** (#3022 → `e80b5a35afe595af5e348d2d2e095625addf2d09`): все 36 ARIA-узлов: MacOSTab кнопки в role=tablist контейнере (22); SegmentedControl без dangling aria-controls (6; опциональный `controlsPrefix` prop); AppointmentSummaryBar refresh-кнопка вынесена из role=list (display:contents обёртка — pixel-identical, 40/40 visual PASS против UNCHANGED baselines); admin KPI loading-grid role=status (2); cashier date-inputs aria-label ×3 i18n-ключа ×5 локалей (2+latent). Baseline: 17→1 ключ, 38→2 узла.
- **AXE-EXP-6** (данный PR, закрывающий): корневая причина последнего ключа `admin-services:dark` (2 узла — «DataTable white-in-dark surface class»): легаси-патч dark-theme-visibility-fix.css красил КАЖДЫЙ голый `<table>` в белый в classic dark (`--dark-card-bg` = `var(--mac-text-primary)` = **#ffffff** — ТЕКСТОВЫЙ токен, ошибочно подключённый как фон карточки; `!important` поверх inline-стилей canonical DataTable, маскируя var()-themed dark-поверхности PR-UI-12..15) + мёртвый инвалидный inline `color-mix` в DataTable variantStyles (градиент как операнд color-mix → IACVT → transparent с самого начала в обеих темах). Фикс: патч сужен до легаси-классового скоупа (.table-wrapper/.data-table — load-bearing, без изменений), канонические таблицы освобождены (th/td принудительные инки сужены тем же скоупом); инвалидный color-mix → явный `transparent` (byte-identical computed value в light, канонический dark-фон карточки сквозь таблицу в dark; th/td инки DataTable — `--mac-table-header-text`/`--mac-text-primary` — корректно themed). Baseline: 1→**0 ключей, 2→0 узлов — ПУСТОЙ**; _meta.resolution фиксирует монотонную ужимку 236→172→60→38→2→0.

**Итог трека AXE-EXP:** 34/34 аудита (17 авторизованных QA-harness маршрутов × light/dark) — **0 нарушений WCAG 2.x A/AA**; оба a11y-гейта (публичный PR-UI-18-5 + авторизованный AXE-EXP-1) теперь zero-violation со shrink-only ratchet в силе; канонические токены расширены (`--mac-text-on-tertiary`, `--mac-link-ink`); класс-фикксы ARIA-структуры (tablist, aria-controls, list ownership, role=status, input labels) улучшают семантику для скрин-ридеров; легаси white-table патч декомиссирован в части канонических таблиц (CC-1-прецедент-паттерн, вторая декомиссия из того же файла). **Трек CLOSED.** Остаток follow-up-кандидатов (вне плана): RoleGate Receptionist policy (§4.1.10 finding #1, по-прежнему deferred).

### 4.1.26 Follow-up track REC: Receptionist deprecation — canonical Registrar, write-freeze + alias removal; RoleGate finding §4.1.10 #1 закрыт БЕЗ widening RoleGate (v2.15, 03.09.2026)

**Business-решение (пользователь, финальное):** `Registrar = каноническая продуктовая роль`; `Receptionist = deprecated legacy input alias`. Отдельного Receptionist UI / workflow / permission model не существует (легаси user_mgmt default_permissions — единственное место с отдельным набором, не канонический RBAC-путь), поэтому сохранять вторую RBAC-роль для той же должности нет смысла. Production SQL evidence (read-only сессия, 02.09.2026): **users.role='Receptionist' = 0; нормализованных вариантов = 0; Receptionist-linked Doctors = 0; Registrar = 2; public.roles = 0 rows** → data-migration НЕ нужна.

**Phase 0 read-only inventory (62 файла):** writers (9 поверхностей: 2 backend-схемы regex, openapi pattern ×5, UserModal/UserManagement опции, i18n-ключи ×5, user_mgmt легаси-словарь, writer-тест); readers/aliases (backend 17 — security.require_roles SSOT-нормализация, гварды file_system/doctor_integration/registrar_wizard и пр.; frontend 10 — ROLE_ALIASES зеркала, HeaderNew/GNC/GlobalSearchBar/clearAuthCache/PatientPickup/WizardV2/common-RoleGuard); внешний consumer — Telegram Mini App initData (принимает 'receptionist' на входе, нормализует в registrar). Полный инвентарь зафиксирован в worklog Task 14.

**Инкременты (исполнены одним coherent PR по решению пользователя; E-4 backend-alias removal — ОТЛОЖЕН):**
- **REC-1 canonical write-freeze:** backend — `_USER_MANAGEMENT_ROLE_PATTERN` (schemas/user_management.py; покрывает UserCreate+UserUpdate) и оба inline-паттерна schemas/authentication.py (Register/Update — мёртвые схемы, 0 endpoint-импортёров) лишены 'Receptionist'; `_users.py` supported_roles-словарь вычищен; openapi.json — 5 pattern-строк синхронизированы (1:1 механическая замена, эквивалент регенерации; файл свеж на main `905456504`); frontend — опции UserModal (create/edit fallback-список: prod roles-таблица пуста → статический список и есть рабочий writer) и UserManagement (fallback-список + badge-варианты: 'warning' переназначен каноническому Registrar; /admin/users не входит в visual-baselines → пиксельного риска нет); i18n `um_role_receptionist`/`umdl_role_receptionist` удалены ×5 локалей (симметрично, locale-parity соблюдён); UserModal.rolePayload.test переписан в write-freeze контракт (absence-ассерт + Registrar payload); bootstrap-фикстура переведена на 'Registrar'. **Принцип: canonical writes only, legacy reads temporarily accepted.** **Амандмент Codex review P1 (PR #3025):** миграция `app/scripts/migrate_users_to_postgres.py` писала legacy-роль verbatim (INSERT и UPDATE), обходя схемный regex — единственный оставшийся writer-путь для stored Receptionist; закрыто нормализацией `_normalize_legacy_role` на границе чтение→запись (legacy-вход читается как есть, канонический Registrar пишется в оба пути); тест миграции инвертирован в write-freeze контракт (источник сохраняет легаси-написание, ассерты требуют канонический выход; локально 84/84 unit + 1/1 integration alias-тест зелёные). **Амандмент Codex re-review P2 ×2 (PR #3025):** (1) read/write словари разведены — `_USER_MANAGEMENT_ROLE_PATTERN` (write: create/update/bulk-change-role, канонический без Receptionist) и новый `_USER_MANAGEMENT_ROLE_FILTER_PATTERN` (read: UserSearchRequest.role + GET /users role-параметр — канонический + Receptionist для запросов к legacy-строкам совместимых деплоев; ?role=Receptionist остаётся валидным фильтром); openapi.json: 2 read-поверхности → compat-паттерн, 3 write-поверхности — канонический (2-строчный дифф); (2) supported_roles health-метаданные: Receptionist заменён каноническим Registrar (список ранее вообще не содержал front-desk роль).
- **REC-2 QA/e2e canonicalization:** 5 e2e-моков `role:'Receptionist'` → `'Registrar'` (registrar-time/ux-audit/workflow/visual-regression/capture-landing) + UserProfile fixture; документирующий комментарий visual-regression (дивергенция RoleGate/ROUTE_ALIASES) переписан под новое состояние. **Обязательный proof исполнен:** те же разрешённые маршруты (registrar-поверхности рендерятся), нет /forbidden-redirect, **visual 40/40 PASS против UNCHANGED базлайнов (0 PNG в диффе = zero-pixel-delta)** — причинность: HeaderNew/GNC не рендерят сырую роль (только логика), нормализация 'receptionist'→'registrar' давала тот же результат, что и канонический 'registrar' напрямую.
- **REC-3 frontend alias removal:** ROLE_ALIASES (routeRegistry.ts + types/roles.ts зеркало) — удалён ТОЛЬКО ключ receptionist (nurse→doctor — отдельная policy, НЕ тронут); consumers вычищены: HeaderNew ×2 нормализатора, GlobalNotificationCenter, GlobalSearchBar (role→home map), clearAuthCache ('receptionist' исключён из validRoles → легаси-кэш-профиль теперь purged с редиректом на fresh login), PatientPickupView (canEdit), AppointmentWizardV2 (hasRole), common/RoleGuard (isRegistrar + permission-ветка); i18n-текст `aw_insufficient_rights_description` ×5 локалей (упоминание Receptionist убрано; экран insufficient-rights не входит в baselines). **Proof закрытия deferred-finding:** rbacRouteParity.test инвертирован — `hasRouteAccess({role:'Receptionist'}, /registrar | /registrar-panel | /clinical/appointments) === false` (deny) + канонический 'Registrar' === true; **route-gate и page RoleGate теперь согласны: Receptionist deny на обоих слоях — finding §4.1.10 #1 закрыт БЕЗ расширения RoleGate** (роль выводится, а не легализуется).

**Гейты:** tsc 0; eslint 0 errors (37 пре-существующих color-warning); stylelint 0; vitest **202 файла / 1652 теста ALL GREEN** (1651 + 1 новый parity-тест; пре-существующий worker-flake useDoctorPanel воспроизведён 1 раз под нагрузкой, standalone 12/12, чистый повтор full-suite зелёный); e2e: registrar-time 1/1 + CI UX/Visual batch **127/127** (вкл. a11y public + auth, оба baseline {}) + registrar-workflow 6/6, capture-landing 6 skipped (env-gated, штатно); check-theme OK; icon-controls PASS (0/0); ratchet PASS (улучшение varUsagesNoFallback 11299→11295); build ✓; git diff --check clean; 29 файлов +111/−55, 0 PNG.

**E-4 backend alias removal — ОТЛОЖЕН (совместимость):** внешний Telegram Mini App initData boundary принимает 'receptionist' на входе; backend read/input-нормализация (security.require_roles, deps, 17 reader-поверхностей) остаётся на compatibility window. Будущий removal gate: `stored Receptionist = 0 ∧ new writes = 0 ∧ frontend callers = 0 ∧ Telegram/external callers = 0` — только после отдельного compatibility audit. Дёшево и безопасно держать входную нормализацию до тех пор.

**Новые follow-up-кандидаты (вне плана, read-only инвентари, НЕ исполнены):** (1) roles-table subsystem inventory — public.roles = 0 rows в production; кто реально использует таблицу (DB_ROLES TODO-комментарии намекают на deferred Phase 0.5/4); (2) Manager RBAC vocabulary inventory — production users.role='Manager' = 1; UI/workflow/writers/readers/permissions, canonical vs legacy; (3) P-014 policy decision — Cashier/Lab/cardio/derma/dentist допускаются route-гвардом на /clinical/appointments (P-014 widening, registry L1240-1243), но page-level RoleGate(['Admin','Registrar','Doctor']) им рисует denyBox (у cashier пункт «Записи» живёт в sidebar) — отдельное policy-решение о фактическом доступе; (4) Nurse policy — алиас nurse→doctor (frontend-only) оставлен как есть.

### 4.1.27 Follow-up tracks ELEV + E-4: elevated-contrast debt closure; Receptionist backend-alias removal — compatibility window закрыт (v2.16, 04.09.2026)

**ELEV (elevated-контрастный долг, остаток CC-трека):** dark `--mac-text-secondary` `#98989d` измерялся **3.95:1** на elevated-поверхности `--mac-bg-tertiary` `#3a3a3c` (отклонение от авторизации исходного чата, зафиксировано в v2.15). Исполнено минимальным системным фиксом: оба dark-блока tokens.css → `#a6a6ad` (изначально авторизованное значение): **7.03:1** на `#1c1c1e`, **5.76:1** на `#2c2c2e`, **4.69:1** на `#3a3a3c` — WCAG AA держится на каждой канонической dark-поверхности; quaternary `#48484a` строго улучшен (3.18→3.77, вне скоупа долга; secondary-на-quaternary текстового класса нет — quaternary используется как фон пузырей/выделений с primary-чернилами). Guard-контракт `darkThemeInk.contract.test.ts` +1 ассерт: оба dark-блока ≥4.5:1 на elevated (окна window/content/elevated запинены все). Остаток вне скоупа (задокументирован): `--mac-text-tertiary` `#636366` — декоративный ярус (1.89:1 на elevated). **Гейты:** tsc 0; eslint/stylelint чисто; ui-ratchet PASS; guard vitest 5/5; **visual 40/40 против НЕИЗМЕНЁННЫХ базлайнов — zero pixel delta** (координированный re-capture дал байт-идентичные PNG: secondary-чернила не рендерятся ни на одной захваченной поверхности — zero-delta паттерн REC-2); axe public 12/12; axe auth 34/34.

**E-4 (backend Receptionist-alias removal):** removal gate §4.1.26 верифицирован все 4 условия: **stored=0** (production SQL evidence 02.09.2026, §4.1.26) ∧ **new writes=0** (REC-1 write-freeze + миграционная нормализация) ∧ **frontend callers=0** (REC-3; grep — только комментарии/зеркала/deny-тесты) ∧ **external callers=0** (machine-audit: Telegram staff scope выводит роль из **DB-записи пользователя** `staff_user.role`, а не из содержимого initData — внешний вход = stored spelling, а stored=0). Демонтаж исполнен: (a) RBAC SSOT `core/security.py` — receptionist→registrar input-normalization И registrar→receptionist allowed-list expansion; (b) `core/roles.py` — enum member `RECEPTIONIST`, `STAFF_ROLES`, hierarchy level 3 (в отличие от Manager read-compat-окна нет: stored=0); (c) `deps.require_staff` — legacy spelling удалена (список иначе не тронут — widening вне скоупа); (d) inline-гард appointments (`APPOINTMENT_BROAD_ACCESS_ROLES_LOWER` без алиас-расширения); (e) грант-листы: file_system ×14, file_upload_json, lab_reporting, registrar_wizard cancel/workflow/require_roles, registrar_integration REGISTRAR_CANCEL_ROLES, doctor_integration ×8; (f) reader-фильтры: registrar_notification_service (×2 → `== "registrar"`), notification_platform `_policy.py` + `_base.py` (ROLE_ALIASES + DEPARTMENT_ROLE_ALIASES); (g) Telegram alias-карты: `telegram_mini_app_init_data._STAFF_ROLE_ALIASES`, `admin_telegram/_helpers`; (h) `authorization/staff.py` `_REGISTRAR_ROLE_ALIASES`; (i) user_mgmt vocabulary: default_permissions-ключ удалён (×2, write-only dead vocabulary — см. roles-table inventory), stats-loop spelling → канонический Registrar (прецедент REC-1 supported_roles); (j) read/filter vocabulary `_USER_MANAGEMENT_ROLE_FILTER_PATTERN` — `Receptionist` удалена, **Manager оставлена** (её окно чтения открыто до ops-деактивации, M-1); openapi.json — 2 pattern-строки синхронизированы (1:1 механика, эквивалент регенерации). **Сохранено намеренно:** `_normalize_legacy_role` в migrate_users_to_postgres.py (граница чтения ЛЕГАЦИОННОЙ внешней БД при миграции — data preservation, не runtime RBAC; test_manager_deprecation пинит). **Тесты инвертированы в deny-контракты** (паттерн REC-3): `test_department_schedule_receptionist_alias_reads_any` → `test_department_schedule_receptionist_spelling_denied` (stored 'Receptionist' → **403** на department-schedule — route/page/backend согласованы на deny); `normalize_role('receptionist')` → `'unknown'`. Фронтенд-зеркало `types/roles.ts` синхронизировано (BackendRole / STAFF_ROLES / ROLE_LEVEL / isBackendRole); `docs/ROLES_AND_ROUTING.md` role-normalization обновлён (Receptionist NO LONGER treated as registrar; Nurse — отдельная policy, в силе). **Гейты:** unit 1601 passed (+2 пред-существующих authorization_service red'а — воспроизведены на базовом коммите, backend-drift вне скоупа); целевые integration 246 passed (rbac_matrix, appointments ×3, registrar ×2, files ×2, telegram mini app, notification platform, user management); tsc 0; eslint чисто; parity+UserModal vitest 6/6; ui-ratchet PASS. **Трек E-4 CLOSED; Receptionist полностью выведен из RBAC-словаря (canonical vocabulary only).**

**Follow-up inventory v2.15-очереди (read-only, исполнено этой сессией; полные брифы — отчёт сессии, repo-external):** (1) **roles-table subsystem — вердикт: де-факто DEAD** (mounted-but-unused: `/api/v1/admin/permissions` 13 маршрутов + `/api/v1/roles` 6, смонтированы api.py:216/449; единственный writer — POST /roles без frontend-вызовов; role_permissions вообще без runtime-writer — CRUD-заглушки; 0 seed'ов/INSERT'ов в миграциях; tables заполняет только тест; `useRoles`-фоллбэк — фактический источник опций ролей при public.roles=0; DB_ROLES «Phase 0.5/4» — план в репо ОТСУТСТВУЕТ, TODO унаследован из wip-jules-2026 `3c0037a4b`; decommission entailment: 2 mount'а, ~7 таблиц, 9 TODO, 1 admin-таб deep-link-only, 2 теста — behavior-neutral); (2) **Manager RBAC — write-frozen + grants=0 (M-1D complete)**, enum read-compat до ops cleanup + M-2; production = 1 строка `smoke_manager` (id=20, is_active=true, синтетический nightly-smoke аккаунт с известным автоматическим паролем, 0 человек); residual exposure = authentication-not-authorization; M-2 (enum + filter + тесты + зеркало) заблокирован только ops-решением: DEACTIVATE (рекомендация кода) + верификация `is_superuser=false` (superuser обошёл бы ВСЕ role-чеки); merge-risk watch: unmerged `feat/admin-doctor-onboarding-ux` `712681235` однажды восстановил Manager в write-union; (3) **P-014 — трёхслойная дивергенция подтверждена**: route 8 ролей (registry L1248) / page RoleGate 3 (alias-blind) / backend data ~2 (даже Doctor 403 — страница не шлёт `doctor_id`, appointments.py L216-221); P-014-widening на этой поверхности декоративен; cashier sidebar «Записи» — задокументированный PR-UI-04b UX (tighten = регресс записанного use-case; `/registrar/visits` уже отдаёт данные server-side); options: A) widen page + data-layer решение (fetch /registrar/visits | расширить APPOINTMENT_BROAD_ACCESS_ROLES | doctor_id), B) tighten route (−cashier sidebar item), C) документировать + C-lite; (4) **Nurse — алиас поднимает level-2 роль на весь Doctor-веб-набор, включая admin-group маршрут `/admin/file-management`** (registry L992); Telegram Mini App Nurse исключает (inter-surface inconsistency); без алиаса — full lockout (home-фоллбэк `/clinical/search` → deny → `/forbidden`), backend-гранты (patients, files ×12, visits, AI) остаются, становясь недостижимыми из UI; production Nurse count НЕ записан — **prerequisite любого решения**; REC-3-механика purge (`autoClearStaleCache`) — dead code (0 callers); options N-1 keep / N-2 keep+scope-limit / N-3 REC-3-style removal (материально слабее прецедента: Nurse — живая каноническая роль без deprecation rationale). **P-014 и Nurse — НЕ исполнены (policy decisions, требуют бизнес-выбора пользователя; брифы decision-ready).**

### 4.1.28 Manager RBAC M-2: vocabulary closure — enum + read/filter окно + зеркала закрыты (v2.17, 05.09.2026)

**Гейт удаления (E-4-style, 4 условия, все верифицированы):** **stored-legacy-tombstoned** — ops-деактивация smoke_manager id=20 исполнена 04.09.2026 host-side (guarded UPDATE `is_active` true→false, affected=1, BEGIN/timeout/FOR UPDATE/COMMIT; логин 401, контроль smoke_doctor 200; tombstone сохранён — DEACTIVATE, не DELETE) ∧ **writes=0** (M-1C freeze, пиннится тестами) ∧ **frontend callers=0** ('manager' role-key поверхностей в registry/sidebar/homeForRoles нет; tsc-компиляция после удаления union — 0 ошибок) ∧ **external callers=0** (require_roles не принимает spelling — гранты сняты M-1D; Telegram initData не зависит от enum). **Демонтаж (9 файлов кода/тестов + 2 артефакта):** (a) `core/roles.py` — enum member `MANAGER` + hierarchy entry `Manager: 8` (raw 'Manager' строка теперь scores 0 — канонический-only table, тот же режим, что E-4 дал Receptionist level 3; ZERO итераций/lookups по enum — machine-verified: `list(Roles)`/`__members__`/`Roles("Manager")` вызовов в app-коде нет); (b) `schemas/user_management.py` — `_USER_MANAGEMENT_ROLE_FILTER_PATTERN` УДАЛЁН, `UserSearchRequest.role` → canonical `_USER_MANAGEMENT_ROLE_PATTERN` (write == read, окно закрыто; hasattr-гард против resurrection — в тесте); (c) `_users.py` GET /users role-Query → canonical pattern; (d) `_helpers.py` re-export удалён; (e) `openapi.json` — 2 pattern-строки синхронизированы (1:1 механика, эквивалент регенерации — прецедент E-4/REC-1); (f) frontend `types/roles.ts` — BackendRole union / ROLE_LEVEL / isBackendRole guard (tsc-гарантия полноты зеркала); (g) комментарии M-1 → closure-заметки M-2 (schemas/_users/_helpers/ensure_smoke_users; исторические grant-rationale в webhooks.py/reports.py не тронуты — документируют решение M-1D, всё ещё точное); (h) `test_rbac_matrix.py` filter-семья — 'Manager' перенесён из accept-цикла в reject-цикл (422 == junk). **Tombstone-совместимость (доказано тестом):** `users.role` = String(20) сырой строкой; `UserResponse.role: str` — Admin search по username возвращает tombstone-строку (role='Manager', is_active=false, is_superuser=false) — аудито-история видима, сериализация не сломана удалением enum. **Тесты:** `test_manager_deprecation.py` перепрофилирован M-1→M-1+M-2 closure (54 теста: deny-матрица M-1D 403 ×6 поверхностей — regression-pin; grant-preservation; write-freeze 422 ×3; filter-accept инвертирован → 422; enum-defines инвертирован → not-hasattr + `Roles("Manager")` ValueError + vocabulary-list pin; vocab-split → unified (+ hasattr-гард); NEW tombstone-serialization; source-contracts ensure_smoke_users/nightly; migration-verbatim сохранён). **Гейты:** backend targeted 147/147 (sqlite, CI-exact fastapi 0.140.13: manager_deprecation 54 + rbac_matrix 23 + onboarding + analytics_contracts + codex_security_fixes); фронт tsc 0; eslint 0; vitest **1671/1671** (--no-file-parallelism, 204 файла); 2 pre-existing red'а authorization_service воспроизведены на базовом коммите (relationship_check_error ≠ patient_scope_mismatch — вне скоупа, владелец backend/auth; см. §4.1.27 прецедент). **Находка (pre-existing, вне скоупа):** request-state auth-семейство (`require_authentication`/`require_active_user`/`require_staff` → `get_current_user_from_request` → `request.state.user_id`) НЕ получает state ни от одного middleware — GET /users/{id}/profile 401 «Требуется аутентификация» для всех вызовов (probe M-2 tombstone-теста); добавлено в очередь кандидатов. **Post-deploy хвост (ops, вне репо):** Stage 0 READ-ONLY SQL-инвентарь остатков данных (row 20 вкл. is_superuser, refresh_tokens/user_sessions, audit_logs actor_role, public.roles+M2M) + опциональный guarded token-purge + canonical redeploy + nightly proof.

**M-2b (Codex review follow-up на #3049, 05.09.2026):** Codex-ревью приземлилось в 02:26Z (квота восстановилась между моими чеками; перед merge не перепроверено — ошибка сессии, задокументировано) с 2 находками; обе закрыты follow-up-инкрементом: **P1** — миграция `migrate_users_to_postgres` копирует is_active/is_superuser verbatim → активная/superuser legacy-'Manager'-строка могла бы воскреснуть при ре-ране; фикшн tombstone-принуждением в `LegacyUserRow.__post_init__` (role verbatim по no-alias правилу, is_active=False, is_superuser=False, case-insensitive; пиннится тестом); **P2** — DB-backed role-каталог (`/api/v1/roles`) принимал name='Manager' → строка утекала в /roles/options → UserModal предлагал снятое написание → 422 при сабмите; фикшн SSOT-множеством `RETIRED_ROLE_SPELLINGS` (core/roles.py: manager+receptionist, case-insensitive) + RoleCreate field_validator (422 на границе схемы) + фильтр в /roles/options (defense-in-depth для существующих строк). Тесты: +3 (catalog-reject ×3 spellings, options-filter с живой Role-строкой в SQLite, migration-tombstone ×2 spellings + canonical-контроль); ruff чист (I001 собственного импорта починен; 2 UP017 — pre-existing нетронутых строк, семейство M-1).

### 4.1.29 Dentist workflow safety follow-up — COMPLETE, reconciled at merged main `897b6b191` (26.09.2026)

Этот follow-up — отдельный утверждённый продуктовый план из пяти последовательных PR; он не добавляет SP к историческому UI-remediation portfolio.

- **Step 1 — PR #3429, MERGED:** отключены локальные фиктивные действия фотоархива и legacy-route переведён на выбор пациента.
- **Step 2 — PR #3431, MERGED:** сохранение черновика EMR обязательно до закрытия визита; ошибки и конфликты блокируют завершение.
- **Step 3 — PR #3434, MERGED-VERIFIED:** разделы протокола встроены в экран визита; отдельная модалка и дублирующая команда завершения удалены; переход «К очереди» ждёт сохранения и не закрывает очередь.
- **Step 4 — PR #3439, MERGED:** добавлен защищённый `/api/v1/dental/media` поверх `files`: JPG/PNG/PDF, метаданные, просмотр по авторизации, мягкое удаление и проверки пациента/визита; модель хранения и схему БД не меняли.
- **Step 5 — PR #3443, MERGED-VERIFIED (merge SHA `0a606b5da586b7937e4f04c36bc2e9a1982dba86`):** подключён frontend API для списка/загрузки/просмотра/метаданных/удаления; серверный поиск пациентов стал единственной точкой выбора; вкладка фотоархива возвращена; AI-алиас направлен в экран приёма, лишняя AI-вкладка и недействующие действия удалены. Покрытие включает UI/API unit-тесты и Playwright на синтетических данных.
- **Post-merge evidence for #3434:** merge SHA `76747537a29d0c145865250439e0e1ba05f14235` совпадал с `origin/main`; squash stat совпал с одобренным diff (8 файлов, +502/−79). На merged tree прошли Tier 1: 285 frontend test files / 2,642 tests, type-check, lint (0 ошибок), theme, icon-controls, build и Linux Frontend E2E. Main push CI также завершил успешно backend и integration tests, Docker, parity, security, Gate D, CodeQL и regression audit.
- **Step 5 synthetic browser evidence (local Playwright Chromium; без staging и PHI):** финальный прогон 4/4: legacy tab aliases + серверный поиск, архив (просмотр/загрузка/повторное чтение после reload), адаптивность/темы для поиска и архива. Замер от перехода на вкладку пациентов до появления результата после поиска синтетического пациента: 375px light 2489/1430 ms, dark 1346/1948 ms; 768px light 1876/1875 ms, dark 1833/1443 ms; 1280px light 1842/1877 ms, dark 1901/1899 ms. На странице архива и экране поиска нет document-level горизонтального переполнения во всех шести сочетаниях ширины/темы. Визуальная проверка выявила отдельное ограничение общей оболочки: на 375px правая часть верхней панели приложения обрезана; вынесено в responsive follow-up общей оболочки. Автоматически найден и исправлен конфликт Escape у окна просмотра с очисткой пациента.
- **Оставшийся Tier 2 сценарий — ЗАКРЫТ PR #3446 (merge SHA `3d225a61d705`):** явное действие начала визита для пациента из `call-next`; EMR открывается только после ответа start-visit с валидными `patient_id`/`visit_id`; null-patient колбэк удалён. Непрерывный путь очередь → пациент → приём → завершение запинен браузером в `frontend/e2e/dentist-panel-plan.spec.ts` ('queue call starts the returned visit, saves EMR, then completes the queue entry') с жёстким порядком запросов `['call-next', 'start-visit', 'save-emr', 'complete-queue']` на синтетических данных. Queue/clinical-lifecycle follow-up, заявленный ниже как отдельный, исполнен этим PR с обязательным gate.
- **Remaining — НЕТ (трек закрыт 26.09.2026 на main `897b6b191`):** пост-merge серия этого же трека — review-P1 фиксы к #3439 слиты как #3441 (merge `46b475e54`: protected-domain file boundary + ID-space authorization + IAM SSOT role gates); PHI-лог серия #3447 (`0dff931b7`, диалоги), #3449 (`e8aa49ea5`, протокол визита), #3450 (`a34c48779`, URL-гидратация), #3451 (`897b6b191`, worklist) — все merged. Финальная сверка против исходного плана «Исправление панели стоматолога»: 5/5 пунктов подтверждены кодом/тестами — EMR-завершение: `DentalVisitScreen.persistence.test.tsx` (немедленное сохранение перед закрытием очереди, последовательные save с row_version, блокировка при ошибке/409/отсутствии visit_id, retry, изоляция черновиков при переключении пациента; снимок данных один и тот же в K04/K10-гейт и команду очереди); архив: `backend/tests/test_dental_media_api.py` (несовпадение пациент/визит, неподдерживаемые форматы, автор/админ-удаление, лечащий врач видит снимки другого автора, аутентификация просмотра, без путей хранения); навигация/поиск: e2e-алиасы + серверный поиск `/patients/` с role-gate. Остаток вне git: staging-чеклист `docs/runbooks/STAGING_VALIDATION.md` обязателен до любого заявления о развёртывании/полной проверке; responsive-ограничение общей оболочки (375px, верхняя панель) остаётся отдельным треком. Production и реальные данные пациентов не использовались.

Метрики исторического UI-remediation portfolio и Sprint/SP итоги не переписывались; это отдельный клинический follow-up. API/file permission implementation remains subject to its own mandatory repo gate.
