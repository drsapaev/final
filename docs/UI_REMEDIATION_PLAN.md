...
### 4.1.29 Dentist workflow safety follow-up — progress at merged main `de9ae6f3b` (25.09.2026)
...
- **Step 5 — IN IMPLEMENTATION (`codex/dentist-ui`; PR ожидает создания):** подключён frontend API для списка/загрузки/просмотра/метаданных/удаления; серверный поиск пациентов стал единственной точкой выбора; вкладка фотоархива возвращена; AI-алиас направлен в экран приёма, лишняя AI-вкладка и недействующие действия удалены. Покрытие включает UI/API unit-тесты и Playwright на синтетических данных.
...
- **Оставшийся Tier 2 сценарий:** непрерывный путь очередь → пациент → приём → завершение пока не подтверждён браузером. Текущая очередь показывает записи, но не предоставляет перехода строки к началу визита; `ModernQueueManager` владеет вызовом следующего пациента, а стоматологический завершитель требует уже связанный queue-entry/visit. Это отдельный queue/clinical-lifecycle follow-up с обязательным gate; не считается закрытым этим UI PR.
- **Remaining:** Step 5 ожидает целевых проверок, ревью и слияния; после него повторно проверить оставшийся сквозной сценарий и обновить ledger. Production и реальные данные пациентов не использовались.
