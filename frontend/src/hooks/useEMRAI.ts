...
export const useEMRAI = (useMCP = true, provider = 'deepseek') => {
  ...
  const getICD10Suggestions = useCallback(async (symptoms, diagnosis, specialty: unknown = null) => {
  ...
  const analyzeImage = useCallback(async (imageFile, imageType, options: Record<string, unknown> = {}) => {
    setLoading(true);
    setError('');
  ...
  const analyzeSkinLesion = useCallback(async (imageFile, lesionInfo: unknown = null, patientHistory: unknown = null) => {
    setLoading(true);
    setError('');
