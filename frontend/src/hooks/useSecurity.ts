...
const useSecurity = () => {
  ...
  const filteredThreats = useMemo(() => (securityData.threats?.filter(threat => {
    const matchesSearch = !searchTerm || 
      String(threat.type).toLowerCase().includes(searchTerm.toLowerCase()) ||
      String(threat.description).toLowerCase().includes(searchTerm.toLowerCase()) ||
      String(threat.source).toLowerCase().includes(searchTerm.toLowerCase()) ||
      String(threat.target).toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const matchesDateRange = !filterDateRange || (() => {
      const threatDate = new Date(threat.timestamp as string);
  ...
  const filteredLogs = useMemo(() => (securityData.logs?.filter(log => {
    const matchesSearch = !searchTerm || 
      ...
      String(log.ip).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const getSecurityStats = () => {
    const stats = {
      totalThreats: Number((securityData.overview as Record<string, unknown> | undefined)?.totalThreats) || 0,
      ...
      blockedIPs: Number((securityData.overview as Record<string, unknown> | undefined)?.blockedIPs) || 0,
      activeSessions: securityData.overview?.activeSessions || 0,
      failedLogins: securityData.overview?.failedLogins || 0,
      securityScore: Number((securityData.overview as Record<string, unknown> | undefined)?.securityScore) || 0
    };
  ...
  const getSecurityTrends = () => {
    const trends = {
      threats: {
        current: Number((securityData.overview as Record<string, unknown> | undefined)?.totalThreats) || 0,
        previous: 18,
        change: ((Number((securityData.overview as Record<string, unknown> | undefined)?.totalThreats) || 0) - 18) / 18 * 100
      },
      blockedIPs: {
        current: Number((securityData.overview as Record<string, unknown> | undefined)?.blockedIPs) || 0,
        previous: 12,
        change: ((Number((securityData.overview as Record<string, unknown> | undefined)?.blockedIPs) || 0) - 12) / 12 * 100
      },
      securityScore: {
        current: Number((securityData.overview as Record<string, unknown> | undefined)?.securityScore) || 0,
        previous: 82,
        change: ((Number((securityData.overview as Record<string, unknown> | undefined)?.securityScore) || 0) - 82) / 82 * 100
      }
    };
