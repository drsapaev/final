...
function normalizeNotification(input, source = 'api') {
  ...
  return {
    id: input.id || `${source}-${Date.now()}-${Math.random()}`,
    type: input.type || input.event_type || 'system',
...
export function NotificationCenterProvider({ children }) {
  ...
  const appendNotification = useCallback((event, source = 'ws') => {
    ...
    setInbox((current) => {
      ...
      if (existingIdx >= 0) {
        const next = [...current];
        next[existingIdx] = { ...next[existingIdx], ...enriched };
        return next;
      }
  ...
  const loadNotifications = useCallback(async (params = {}) => {
    try {
      const payload = await notificationsService.getNotifications(params);
      ...
      setInbox((current) => {
        const merged = [...normalizedList, ...current];
        const unique = [];
        const seen = new Set();
        merged.forEach((item) => {
  ...
  const markAsRead = useCallback(async (notificationId) => {
    ...
    try {
      ...
      logger.warn('[NotificationCenter] markAsRead failed', error);
    }
  }, []);
  ...
  const markAllAsRead = useCallback(async (role = null) => {
    ...
    try {
      ...
      logger.warn('[NotificationCenter] markAllAsRead failed', error);
    }
  }, []);
