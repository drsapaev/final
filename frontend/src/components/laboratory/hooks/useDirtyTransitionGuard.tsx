...
export interface DirtyGuardTransitionOptions {
  ...
  sourceIds?: string[];
  /**
   * PR 3351 (review round 8, P1): коллбек ПРИ pending-блоке — вызывается
   * ВМЕСТО onCancel, когда переход заблокирован незавершённой операцией
   * (а не отменён пользователем в dirty-диалоге). urlIntent-флоу панели
   * использует его, чтобы ОТЛОЖИТЬ намерение до завершения операций
   * (адресная строка остаётся за intent-ом, откат не выполняется —
   * откатывал бы внешнюю запись ПОД sentinel-ом и ломал смежность twin:
   * collapse после завершения операции приземлялся бы на неоткаченную
   * внешнюю запись и «воскрешал» заблокированный intent произвольным
   * supersede). Обычные переходы (клик пациента) коллбек не передают —
   * им достаточно тоста «сохраняется…».
   */
  onPendingBlock?: () => void;
}
