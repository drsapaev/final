...
const barBase: CSSProperties = {
  ...
  opacity: 0.55,
  animationName: 'mac-table-skeleton-pulse',
  animationDuration: 'var(--mac-duration-normal)',
  animationTimingFunction: 'var(--mac-ease)',
  animationIterationCount: 'infinite',
  animationDirection: 'alternate',
};

/**
 * Inline `<style>` injection is intentionally avoided to keep the component
 * portable across shadow-DOM / SSR boundaries. The keyframes are emitted once
 * per mount via a `<style>` tag inside the `<tbody>` so consumers don't have to
 * ship a separate CSS file. Playwright visual-regression tests already mock
 * CSS variables at the document level, so the pulse animation works in E2E.
 */
const KEYFRAMES = `@keyframes mac-table-skeleton-pulse {
  0% { opacity: 0.35; }
  100% { opacity: 0.7; }
}`;
        ...
        >
          {Array.from({ length: columns }).map((_, colIndex) => (
            ...
            <td
              ...
              style={{
                ...cellBase,
                borderRight: colIndex === columns - 1 ? 'none' : '1px solid var(--mac-border)',
              }}
            >
              <span style={{ ...barBase, display: 'block' }} role="presentation" aria-hidden="true" />
            </td>
          ))}
        </tr>
      ))}
