/**
 * Cashier payment contract — pure helpers + types for the cashier domain.
 *
 * PR-UI-14-1: verbatim extraction from pages/CashierPanel.tsx (module-level
 * block). No behavior change: every function body, ordering and comment is
 * ported as-is; CashierPanel now consumes this module. Contract tests
 * (pages/__tests__/CashierPanel.contract.test.tsx) pin these helpers via
 * source markers — the markers moved here together with the code.
 *
 * Contents (single responsibility — "payment contract"):
 *  - row/data shapes: CashierPaymentRow / CashierPaymentRowOrId / CashierPaymentData
 *  - date presets + labels (i18n factory)
 *  - payment id/method resolution, receipt print payload builders
 *  - payment status meta/label resolution
 *  - backend visit/payment availability predicates (fail-closed)
 *  - grouped cashier payment command (backend-owned allocation contract)
 *  - backend payment-action gate (PAYMENT_ACTION_CAN_FIELD / hasBackendPaymentAction)
 */
import { api } from '../../api/client';  // PR-53: replace raw fetch with axios
import tokenManager from '../../utils/tokenManager';
import type { Appointment } from '../../types/domain/clinic';
import { parseRegistrarTimestamp, formatRegistrarDate, formatRegistrarTime } from '../../utils/dateUtils';
