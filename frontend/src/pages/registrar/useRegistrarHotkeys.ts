...
export const useRegistrarHotkeys = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // E-054 leftover 1 (wizard-пятёрка #3079, superseded): при открытом
      // мастере или slots-модале Ctrl-хоткеи не переключают вкладки/панели
      // ЗА модальным окном — Ctrl+K не переоткрывает мастер, Ctrl+1..5 не
      // уводят пользователя с открытой формой записи. Escape не гейтится:
      // slots-модал закрывается панелью (ниже), мастер — собственной
      // защитой requestClose (подтверждение несохранённых данных).
      // Ctrl+P остаётся нативным print (этот хук никогда не звал
      // preventDefault для него).
      if ((showWizard || showSlotsModal) && e.ctrlKey) {
        return;
      }
      ...
      if (e.key === 'Enter') {
        ...
        // Fix F: панельный Escape больше НЕ закрывает мастер напрямую —
        // раньше это обходило собственную защиту мастера (подтверждение
        // при несохранённых данных, блокировка закрытия во время сохранения).
        // Мастер обрабатывает Escape сам (requestClose с защитой).
