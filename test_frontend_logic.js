// Тест frontend логики обработки данных очередей
...
// Симулируем исправленную логику из RegistrarPanel.jsx
function testFrontendLogic() {
  ...
  console.log('=== ТЕСТИРОВАНИЕ ИСПРАВЛЕННОЙ ЛОГИКИ FRONTEND ===');
  ...
  mockServerData.queues.forEach(queue => {
    ...
    if (queue.entries && Array.isArray(queue.entries)) {
      queue.entries.forEach((entry, index) => {
        try {
          const fullEntry = entry.data || entry; // ✅ ИСПРАВЛЕНО: Данные могут быть в entry.data
...
// Запускаем тест
const result = testFrontendLogic();
